...
| 3a | **patch load / swap — the DEVICE half** (slot-keyed camera + output sessions) | RENDERER owns doc, graph, streams and the node-keyed registries that hold them; helpers untouched | all of them — no process dies | **nothing** *for a slot in use*. The reserved ids are never deleted, so no registry entry is retired and no stream re-acquired. An UNUSED slot holds no engine at all (lazy engines, row 11) and so has nothing to keep — the guarantee applies exactly where a session exists | **NONE for a slot.** No re-acquisition, no camera blink, no permission re-prompt. An UNRESERVED camera still dies — that is the interruption, and it is now the positive control rather than the behaviour | ✅ **graph tap:** `graph/device-slots-ydoc.test.ts` runs the REAL loader + REAL reconciler and asserts NO `removeNode` for any reserved id across a load, with an unreserved camera in the same rack asserted to die. ⏳ **receiver side:** `e2e` camera-pixels-advance across a load — see the P1 report. ⚠ Neither subsumes the other: the graph tap cannot see a frame, the e2e cannot see the engine |
...
| 11 | **first bind / first patch of an UNUSED slot** (new with lazy engines) | RENDERER owns the slot node, which exists from boot; the ENGINE node does not exist until first use | n/a — nothing is connected yet | the slot's engine node is created on the next snapshot after a device is bound or a cable lands | **the mount itself.** A slot that has never been used has no session, so there is nothing to keep continuous — but the first-use moment is a real add, and on a software renderer it is a visible one. No OTHER slot may flinch while it happens | ✅ `device-slots-ydoc.test.ts` › *lazy engines* — asserts the unused population holds NO engine against a positive control that does, that binding and patching each bring one up, that unbinding releases it, and that a BOUND slot still survives a load untorn. ⚠ What it cannot see: the wall-clock cost of the mount itself, and whether a neighbouring live slot glitches during it — no instrument watches that, at any tier |
