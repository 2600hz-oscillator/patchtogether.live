// art/scenarios/moog921b/sine-c4.test.ts
//
// ART scenario for the MOOG 921B Oscillator (slave VCO driven by a 921A bus).
// Renders the 921B at C4 (freq_bus = 0.0) with default knobs and compares the
// output to a baseline .f32 in art/baselines/, pinned to the worklet source
// SHA. The render harness is the Phase-1 deterministic stub (see
// art/setup/render.ts) — the load-bearing pin here is the source SHA, which
// asserts the .f32 baseline was regenerated whenever the 921B worklet OR its
// shared DSP lib changed (memory: ART SHA-pin regenerate LAST). NOTE: the SHA
// hashes ONLY the worklet's own top-level moog921b.ts source — the shared
// lib/moog-vco-dsp.ts is inlined by esbuild bundle:true, so a lib-only change
// doesn't move the .sha (but DOES change the built .js). Regenerate the .sha
// as the final step after ALL worklet + lib edits.
//
...
describe('moog921b / sine-c4', () => {
  const scenarioId = 'moog921b/sine-c4';
  ...
  it('SHA matches between source and built artifact', async () => {
    const srcSha = await moduleSourceSha('moog921b');
    const built = await builtSha('moog921b');
  ...
  it('matches baseline (RMS tier B)', async () => {
    ...
    expect(
      ...
      `Baseline SHA (${existingSha}) doesn't match source SHA (${srcSha}).\n` +
        `Run \`npm run art:update -w art\` if the change to moog921b.ts was intentional.`,
    ).toBe(srcSha);
