...
export const ART_EXCLUDED: Readonly<Record<string, string>> = {
  ...
  cvBuddy: 'lane→ES-9 CV/gate/clock sender — its OUTPUT ports are cv/gate (auto-routed to the physical ES-9 jacks), not audio-family; the only audio is the ES-9 hardware RETURN (external rack signal, no deterministic offline source), same class as es9. Passthrough/slot/clock math pinned by the cv-buddy slot-alloc + clock-math + es9-reconcile unit suites; the audio-thread clock emitter (the cv-clock worklet that drives the jacks in a real browser) by the dsp cv-clock-core suite + the cv-buddy-clock-worklet stall suite',
  ...
  linnstrument: 'live MIDIAccess device stream from a LinnStrument in User Firmware Mode — every OUTPUT port is cv (six retained XY axes) or polyPitchGate (two buses) driven entirely by Note On/Off, CC X lo/hi pairs, CC Y and poly pressure arriving from external hardware, so there is no audio-family port to capture and no deterministic offline source to drive one. Same class as trails / midiLane / midiclock. The wire decode is pinned by raw-decode.test.ts and vectors.test.ts against the synthetic V01–V16 corpus, the reducer by selection-reducer.test.ts, and the real chain (simulated device on the real onmidimessage seam → keys_poly → CUBE → SCOPE; r_x → VCA → SCOPE) by the e2e linnstrument.spec.ts',
