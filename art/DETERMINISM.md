...
| Random seed (ART audio profiles) | `0xC0FFEE` (`PROFILE_NOISE_SEED`) | `art/setup/drivers.ts` (`seededNoise`, and passed to seedable cores, or injected via `processorOptions.seed` where the worklet exposes a seed seam: slewswitch's step PRNG) | Every ART audio-profile scenario | Profile renders MUST be bit-identical run-to-run (they pin raw `.f32` baselines), so any RNG-based core is driven from an explicit seed — never `Math.random`. Driver phase/epoch are pinned to sample 0. A module whose RNG has NO injectable seed can't be profiled until a seam lands (e.g. cloudseed's `Math.random()` mod-phase init in its allpass/delay constructors). |
...
- **MIDI device list** — varies across CI runners. HELM + MIDICLOCK + MIDI-CV-BUDDY specs mock the MIDIAccess API; cards depending on device state are EXEMPT_FROM_VRT.
  ⚠ **This line used to end "until a deterministic stub lands", and that exit condition was never a stub.** `e2e/tests/_per-port-drivers.ts` has mocked `requestMIDIAccess` and pumped a deterministic clock stream since the per-port sweep was built — the sentence above even says the specs mock MIDIAccess — and nobody connected it to the exemption it was gating. The real exit condition is narrower and does not need a mock at all: **a card whose PRE-CONNECT state is the whole capture is already deterministic**, because `requestMIDIAccess` is not called until a user gesture and VRT performs none, so the roster does not exist rather than merely being empty. `midiclock` was drained on that basis (2026-08-24), and its two face scenes plus its card scene are baselined. `midiCvBuddy`, `midiOutBuddy` and `midiLane` remain exempt: each paints a different post-Connect surface, so the argument has to be made per module rather than inherited from the one it was written about. **Baselining a POST-connect state is the thing that still needs work** — it would mean installing the per-port driver's mock in the VRT harness.
