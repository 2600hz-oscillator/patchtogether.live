...
// ⚠ `cardStub()` IS GONE. It emitted a `<Type>Card.svelte` skeleton — a
...
// ⚠ `addVrtExemption()` USED TO LIVE HERE, AND ITS ABSENCE IS THE POINT.
//
// A scaffolded module got an `EXEMPT_FROM_VRT` entry reading "pending baseline"
// so the per-module legacy-CARD VRT sweep would not fail on a card nobody had
// photographed yet. That sweep is deleted and its exemption tables with it, so
// there is no list to write into — and, more usefully, nothing to exempt FROM:
...
function scaffold(opts: ScaffoldOpts): ScaffoldResult {
  ...
  // Pre-flight: refuse if the module's def file already exists. The type union
  // is open and the registry is glob-driven, so the def file IS the source of
  // truth for "already exists" — it was the second half of this check even when
  // the card-map test enumeration was the first. Keeps re-runs from creating
  // duplicates.
  ...
  // ⚠ NO CARD IS SCAFFOLDED. A `<Type>Card.svelte` stub used to land here,
  // resolved glob-driven by the PascalCase convention. The fleet is deleted and
  // `ModuleShell` is the only module UI, so writing one would create a file
  // nothing renders — and it is precisely how the removal would have been undone
  // one module at a time. A new module's surface is its `face`, which
  // `module-face-lint` requires and which a human authors.
  // 3) Shape test.
  ...
  // ⚠ AND THERE IS NO SHARED TOUCH LEFT AT ALL. The last one was a single line
  // in `modules-card-map.test.ts`'s `EXPECTED_NODE_TYPES`, which guarded that no
  // module silently lost its card. That file and the map it tested are gone, so
  // scaffolding a module now edits only its OWN files plus the manifest prose.
