...
// ⚠ NO LONGER COMMITTED (#1858). The generated punch-list was a committed copy of a
// derivable artifact, gated only on being a faithful copy — it could fail on process
// and never on substance. This module survives as a LIBRARY: the ci.yml-derived gating
// assertions in test-ledger.test.ts call bucket3() in memory. Generate on demand if a
// human wants to read it; do not commit the output.
//
// The three buckets:
//
...
export function generateLedger() {
  ...
  p('the collab job\'s report, enforcing the deny-by-default per-lane budget in');
  p('`scripts/e2e-skip-budget.mjs`: a reasonless or unknown-reason skip reds the');
  p('shard, and `e2e` is a REQUIRED lane, so it blocks the merge. `collab` was');
  p('armed 2026-09-01 (#2294) — before that its skips were audited by nothing, so');
  p('all eleven two-peer DOOM multiplayer tests could stand down at runtime and');
  p('the job still reported green; it reds that job now, but stays OFF the');
  p('umbrella, so it does not block a merge. Both directions are anchored by');
  p('`scripts/e2e-skip-budget.test.ts`.');
