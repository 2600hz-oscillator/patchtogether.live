...
Docs: <https://patchtogether.live/docs> (in-app, auto-generated from the module registry on every build). Public on every tier — the beta gate explicitly carves `/docs/*` out.
...
packages/web        SvelteKit app (Svelte 5, @xyflow/svelte canvas, Web Audio)
                    └─ src/routes/docs/* — in-app docs site (Tron theme, prerendered)
...
19 audio modules currently registered. Full per-module I/O diagrams + param tables are auto-generated from the registry: <https://patchtogether.live/docs/modules>. Each module's right-click context menu has a "Docs" entry that opens its dedicated page in a new tab.
