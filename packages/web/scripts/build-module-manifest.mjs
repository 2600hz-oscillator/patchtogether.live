...
const TESTING_OUT = join(OUT_DIR, 'testing-manifest.ts');
...
// Walks art/ and e2e/ to capture spec lists for the /docs/testing page.
// Generated alongside the module manifest so the docs route can import a
// committed TS module instead of doing fs scans at SvelteKit build time
// (which run inside Vite and don't see the repo root reliably).
function buildTestingManifest() {
  const artScenariosDir = join(REPO_ROOT, 'art', 'scenarios');
  const artBaselinesDir = join(REPO_ROOT, 'art', 'baselines');
  const e2eTestsDir = join(REPO_ROOT, 'e2e', 'tests');
  const vrtDir = join(REPO_ROOT, 'vrt');
  ...
  const vrtImplemented = existsSync(vrtDir);
  const e2eSpecs = listSafe(e2eTestsDir).filter((f) => f.endsWith('.spec.ts')).sort();

  const data = { artScenarios, artBaselines, vrtImplemented, e2eSpecs };
  const ts = `// packages/web/src/lib/docs/testing-manifest.ts
//
// AUTOGENERATED by packages/web/scripts/build-module-manifest.mjs.
// DO NOT EDIT BY HAND. Regenerate with \`flox activate -- task docs:manifest\`.
...
export const testingManifest: TestingManifest = ${JSON.stringify(data, null, 2)} as const;
`;
  writeFileSync(TESTING_OUT, ts);
  console.log(
    `[build-module-manifest] wrote testing manifest ` +
      `(${artScenarios.length} ART scenarios, ${artBaselines.length} baselines, ${e2eSpecs.length} E2E specs) -> ${TESTING_OUT}`,
  );
}
