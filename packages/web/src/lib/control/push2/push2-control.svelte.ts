...
import { laneAssignedModules, laneColorEff, laneOf, slotOf, readClip, readClipAudio, clipPlaybackIsRecorded, audioRecState, laneRecArm, laneRecMode, type ClipPlayerData } from '$lib/audio/modules/clip-types';
...
export function pushDisplayOps(): PushDrawOp[] {
  ...
  if (nodeId && launchpadLegendContext().mode === 'audio') {
    ...
    const clip = readClip(data, index), take = readClipAudio(data, index), rec = audioRecState(data, lane);
    ...
    return renderPushAudio(
      ...
      `${refusal ? 'NOT RECORDING' : phase.toUpperCase()}${phase !== 'idle' ? ` → SLOT ${target + 1}` : ''}  •  ${laneRecMode(data, lane) === 'endless' ? 'ENDLESS' : '1 LOOP'}  •  ${take && !take.live ? 'RECORDED' : clip ? 'NOTES' : 'EMPTY SLOT'}`,
      refusal || clipplayerAudioFeedback(nodeId) || 'Record this clip’s audio layer. NOTES / RECORDED keeps the same clip.',
    );
  }
