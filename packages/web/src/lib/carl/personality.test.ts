...
describe('RackspaceCarl personality', () => {
  it('emits addNode with the idPrefix when starting from empty', () => {
    ...
    // First weighted pick from an empty patch should be addNode (only
    // addNode + sleep have nonzero weights when patch is empty + no
    // owned nodes; addNode = 40 vs sleep = 5).
  ...
  it('counts only owned nodes against maxOwnedNodes', () => {
    ...
    // One owned node already + one foreign node — addNode weight = 0
    // because owned has hit cap, so over many ticks we never see addNode.
  ...
  it('produces only legal addEdge candidates (matching cable types)', () => {
    ...
    // Hammer through many ticks; gather all addEdge intents and verify
    // every one has sourceCableType === targetCableType.
    for (let i = 0; i < 500; i++) {
      const intent = carl.next(rng, patch);
  ...
  it('is deterministic for a given seed', () => {
    ...
    // Same seeds, same fresh personalities, same patch → identical intent
    // streams for 20 ticks. (After the 1st intent the patch would diverge
    // if applied, so we keep it constant here just to test seed determinism.)
    for (let i = 0; i < 20; i++) {
