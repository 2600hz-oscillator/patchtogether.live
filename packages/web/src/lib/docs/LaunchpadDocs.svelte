<script lang="ts">
  ...
  // ── The COMMAND DECK 8×8 (both modes paint the same deck frame):
  //   row 0 = EDIT · COPY · PASTE · P-REV · BUF(dark until a copy) · DBL · LEN · NOW
  //   row 1 = the KEYS-entry hold pads: note-REC (K●) + note-OVERDUB (KO).
  const deckPads = [
  ...
  // Deck top row. The session deck LIGHTS only REC · SONG · PLAY · ALL (CC 95 =
  // SHIFT is functional but unlit in the session deck — it lights in the
  // editor). Pair: CC 98 dark. Single: CC 98 = the cyan VIEW marker.
  const pairDeckTop = [
  ...
  function keysUnitPads(unit: 'L' | 'R') {
    ...
    // bottom-row controls (unit L / the single device)
    if (unit === 'L') {
  ...
  const SINGLE_MAP: { what: string; addr: string }[] = [
    ...
    { what: 'ARM ROW (CLIP view top row)', addr: 'CC 91 = NEW · 92 = COPY (re-tap = clear buffer) · 93 = PASTE · 94 = PASTE-REV · 95 = NOW (sticky) · 96 = LENGTH · 97 = DOUBLE — arm, then tap a clip. Auto-disarms after ~4 s' },
    { what: 'double-tap (CLIP view)', addr: 'two quick taps of the same clip pad (~¼ s) → open its note editor + flip to CONTROL, without changing whether the clip plays' },
    { what: 'deck globals (CONTROL view top row)', addr: 'CC 91 = REC (arranger) · 92 = SONG (SES⇄ARR) · 96 = PLAY (transport) · 97 = ALL (stop-all) · 95 = SHIFT (editor ×8)' },
    { what: 'editor nav (CONTROL view top row)', addr: 'CC 91 ▲ · 92 ▼ · 93 ◀ · 94 ▶ (±1; hold SHIFT/CC 95 = ±8) · 96 = VEL (hold + tap) · 97 = SCALE' },
    { what: 'editor FOLLOW', addr: 'scene column, 4th pad from the top (under EXIT · DBL · LEN) — green = following, violet = frozen' },
    { what: 'KEYS entry', addr: 'CONTROL view: hold note-REC (deck row 1 col 1) or note-OVERDUB (col 2) → CC 98 to CLIP (the hold survives) → double-tap a clip' },
    { what: 'KEYS layout', addr: 'top row = 8-cell whole-clip playhead · 6 keyboard rows (cols 1–8) · bottom row = EXIT · QUEUE-REC · OVERDUB · LEN' },
  ];
  const PAIR_MAP: { what: string; addr: string }[] = [
    ...
    { what: 'deck globals (R top row)', addr: 'CC 91 = REC (arranger) · 92 = SONG (SES⇄ARR) · 96 = PLAY (transport) · 97 = ALL (stop-all) · 95 = SHIFT (editor ×8)' },
    { what: 'editor nav (R top row)', addr: 'CC 91 ▲ · 92 ▼ · 93 ◀ · 94 ▶ (±1; hold SHIFT/CC 95 = ±8) · 96 = VEL (hold + tap) · 97 = SCALE · 98 = FOLLOW' },
    { what: 'KEYS entry', addr: 'hold note-REC (R deck row 1 col 1) or note-OVERDUB (col 2) + DOUBLE-TAP a clip on L (REC = overdub off · OVERDUB = overdub on)' },
    { what: 'KEYS layout', addr: 'top rows = 16-cell playhead (L 1–8, R 9–16) · 6 keyboard rows continuous across the L|R seam · bottom row (L) = EXIT · QUEUE-REC · OVERDUB · LEN' },
  ];
...
<section class="mode-section" id="single-launchpad">
  ...
  <h4>The arm row — deck actions without leaving the matrix</h4>
  <p>
  ...
  <ul class="tight">
    <li><strong>NEW (CC 91):</strong> arm, tap an <em>empty</em> pad → a fresh clip + the note editor
      opens (the device flips to CONTROL). A loaded pad is a no-op — it won't clobber.</li>
  ...
  <LaunchpadDiagram
    ...
    accent={hex(RGB_VIEW)}
    caption="SINGLE · CONTROL view (command deck). Bottom row = the function pads: EDIT (orange), COPY/PASTE/P-REV (green), BUF (dark until you copy), DBL + NOW (purple), LEN (yellow). Row above = the KEYS-entry holds K● / KO. Right column = per-lane STOP (bright red where a lane plays). Top row: REC · SONG · PLAY · ALL + the cyan VIEW button."
  />
  <ul class="tight">
    ...
    <li><strong>PLAY (CC 96)</strong> toggles the transport; <strong>ALL (CC 97)</strong> queues a stop
      on every lane.</li>
    <li><strong>K● / KO (row above EDIT):</strong> the KEYS-entry holds — see
      <a href="#single-keys">KEYS on one device</a>.</li>
  </ul>
  ...
  <LaunchpadDiagram
    ...
    accent={hex(RGB_VIEW)}
    caption="SINGLE · note editor, FOLLOW ON. X = step (an 8-step window = half a 16-step block), Y = pitch (in-key, bottom = lowest). The amber column is the playhead. Right column: EXIT · DBL · LEN · FOL (green = following). CC 98 stays the cyan VIEW button."
  />
  <LaunchpadDiagram
    pads={editorPads}
    top={sEditTop}
  ...
  <LaunchpadDiagram
    ...
    caption="SINGLE · KEYS. Top row = the playhead — the WHOLE clip compressed into 8 cells, so the moving dot never leaves the device. Middle 6 rows = the isomorphic keyboard (root cyan, in-scale green, out-of-scale dim, pressed white). Bottom row = EXIT (red), QUEUE-REC (yellow → red), OVERDUB (purple), LEN (yellow). While KEYS is open, CC 98 is inactive — EXIT is the way out."
  />
  <p class="muted">
  ...
  <ol class="steps">
    <li>In <strong>CONTROL view</strong>, <strong>hold K●</strong> (note-REC — enter with overdub OFF /
      true-replace) or <strong>KO</strong> (note-OVERDUB — enter with overdub ON / additive). They sit on
      the deck row just above EDIT/COPY.</li>
    <li>Still holding, press <strong>CC 98</strong> to flip to <strong>CLIP view</strong> — the hold
      survives the flip. While held, single taps <strong>don't launch</strong>. Now
      <strong>double-tap a clip</strong> → KEYS opens for it (an empty pad makes a fresh clip). The clip
      starts <strong>playing</strong>, the keyboard is <strong>live</strong>, and recording is
      <strong>armed-but-idle</strong> until you press QUEUE-REC. (Let go before the double-tap and
      nothing happens — the hold simply clears.)</li>
  </ol>
  <h4>Record a loop</h4>
  <ul class="tight">
...
<section class="mode-section" id="two-launchpads">
  ...
  <LaunchpadDiagram
    pads={matrixPads}
    scene={matrixScene}
    callouts={matrixCallouts}
    caption="PAIR · UNIT L. Rows = the 8 instrument lanes (top→bottom, matching the on-screen card — lane 1 is the top row), columns = the 8 clip slots. Tap a clip to launch it / stop its lane (next quantize boundary; hold NOW on R to fire instantly). Right column = scene launch (amber). L's top row is unused (dark)."
  />
  <ul class="tight">
  ...
  <LaunchpadDiagram
    ...
    scene={deckScene}
    callouts={deckCallouts}
    caption="PAIR · UNIT R (session deck). Bottom row = the function pads: EDIT (orange), COPY/PASTE/P-REV (green), BUF (copy-buffer indicator, dark until you copy), DBL + NOW (purple), LEN (yellow). Row above = the KEYS-entry holds K● / KO. Right column = per-lane STOP (bright red where a lane plays). Top row: REC · SONG · PLAY · ALL."
  />
  <ul class="tight">
  ...
  <LaunchpadDiagram
    ...
    scene={pairEditScene}
    callouts={editCallouts}
    caption="PAIR · UNIT R flips here while editing, FOLLOW ON. X = step (an 8-step window = half a 16-step block), Y = pitch (in-key, bottom = lowest). The amber column is the playhead. FOL (CC 98) is green while the window auto-scrolls with the playhead. Right column: EXIT · DBL · LEN. Unit L keeps the live matrix the whole time."
  />
  <LaunchpadDiagram
    pads={editorPads}
  ...
  <div class="diagram-pair">
    <LaunchpadDiagram
      ...
      accent={hex(RGB_QREC_REC)}
      caption="PAIR · KEYS, UNIT L. Top row = playhead cells 1–8 (of 16). Middle 6 rows = the keyboard's left half (root cyan, in-scale green, out-of-scale dim, pressed white). Bottom row = controls: EXIT (red), QUEUE-REC (yellow → red), OVERDUB (purple), LEN (yellow)."
    />
    <LaunchpadDiagram
      pads={keysRPads}
