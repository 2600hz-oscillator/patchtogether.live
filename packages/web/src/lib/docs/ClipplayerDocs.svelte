...
<article class="clip-guide" data-testid="clipplayer-guide">
  ...
  <section aria-labelledby="session">
    ...
    <p>The screen grid shows the first eight slots. The selected-clip LANE / SLOT selectors reach all {SCENE_STRIDE} slots without launching. Hardware banking reaches the extended slot range. The selected-clip controls and recording status sit directly below the grid and wrap within its width; scene repeats for the first eight scenes follow beneath them in their own SCENES block, so the grid keeps its place at the top of the session page.</p>
