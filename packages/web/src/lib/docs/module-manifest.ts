...
const DESCRIPTIONS: Record<string, string> = {
  ...
  audioIn:
    'System audio input source. Stream from a user-selected mic/line-in/interface via getUserMedia; L+R outputs are fanned out from mono sources or split from stereo. Pick the device and switch it on from the module\'s own faceplate — on the rack tile or in the expanded view; the permission is asked for on that click, never on patch load, and a prior grant re-opens the input on the next visit. ⚠ The last sentence here used to read "Card owns the permission prompt + device dropdown + devicechange refresh" and every clause of it is now false: the STREAM belongs to the node ($lib/ui/modules/node-audio-input-registry, #1590) and the ROSTER + the devicechange watch belong to the app ($lib/audio/input-device.svelte), so no surface unmount can stop a capture.',
  ...
  trails:
    'The Bela TRAILS eurorack module read over its USB-C MIDI port — a quad touch-gesture recorder whose four channels each loop a recorded finger gesture. Twelve jacks (X/Y per channel plus a contact gate) and a divided clock from the device\'s own MIDI transport. The card mirrors the physical 85x85mm pad one to one, with a coloured dot and a fading trail per channel. RANGE picks unipolar pad coordinates or bipolar about the centre; SMOOTH glides the X/Y jacks; CLOCK DIV divides the MIDI clock. Touch data is live engine state and never reaches the saved patch.',
  linnstrument:
    'A LinnStrument 200 in User Firmware Mode as three instruments at once: a 16x8 fourths MPE keyboard, a column of eight control cells (R/G/B joystick toggles plus ARP, HOLD, OCT-, OCT+, PANIC) and an 8x8 XY/MPE pad. The first finger on the pad steers every selected joystick; six bipolar CV jacks carry the three retained X/Y pairs, held between gestures and stored in the patch. Two independent 16-lane poly buses (keys, pad) with per-voice pitch slide, and an arpeggiator per region clocked by TIMELORDE. Touches are live engine state and never reach the saved patch; the device binding lives on this computer.',
  clockedRunner:
  ...
  // ⚠ THREE CLAUSES OF THIS SENTENCE WERE STALE AND USER-FACING. It described a
  // HELD Numpad+/Numpad- octave modifier that no longer exists (octave up/down
  // are remappable KEYS in the same keymap now, and `midiForKey`'s
  // `modifierOctave` is hard-coded 0 at its only call site); it said the
  // listener captures "Numpad* event.codes" when the keymap has been rebindable
  // to ANY physical key for some time; and it counted EIGHT outputs, omitting
  // the POLY bus that landed later.
  numpadPlus:
  ...
  skifree:
    'The classic SKIFREE (ski downhill, dodge trees / rocks / jumps, get chased and EATEN by the abominable snowman) as a CV-controlled game module — a thin wrapper around the upstream skifree.js engine (MIT, Daniel Hough 2013). Single-instance per rack (maxInstances:1). Two bipolar CV inputs (x / y, −1..+1) synthesize the mouse cursor the skier steers TOWARD: x = cursor X across the canvas (0 = left edge, +1 = right edge), y = cursor Y (the skier always heads downhill; cursor position sets the steering angle + speed). When x and y are BOTH unpatched, mouse steering is available on the module\'s own surface: CLICK the slope to take the controls, then move the pointer over it and the skier heads for wherever you are pointing (pointing BELOW the skier is what sends it downhill). Any patched CV OVERRIDES the mouse, on the next pointer move, without a mode to switch (the factory writes the CV cursor each scheduler tick and the surface stops writing). The pointer position is mapped as a RATIO of the displayed picture\'s rect, so the 320 px dock slope and the 104 px lane thumbnail mean the same gesture; the vendored bundle\'s own enableMouse path is NOT used, because its handlers take their rect from the factory\'s detached canvas and therefore fed raw viewport pixels into a 0..320 space. One gate output: a rising-edge 10 ms pulse on every CRASH (hitting a tree / rock / snowboarder / failed jump) OR when the yeti EATS the skier — hooked to the engine\'s hasHitObstacle callback (upstream fires it for crashes and, via isEatenBy, for eats), so the game becomes a trigger source for envelopes / sequencers. One video output (out): the game canvas mirrored each video frame via the cross-domain audio→video bridge — patch SKIFREE → VIDEO OUT / BENTBOX / any video module to send the ski-slope render downstream (mirrors the DOOM `out` pattern). The game itself belongs to the NODE — the factory loads the bundle, builds the controller against a DETACHED canvas and disposes it when the node leaves the graph — so a rack nobody has expanded is still skiing, still crashing and still firing its gate, and the faceplate BLITs from that canvas rather than owning it. The faceplate carries the slope in two places: a steerable 320 px picture at the head of the dock full view, and a read-only thumbnail on the lane tile, both with a SCREEN switch semantic that stops the PICTURE and never the game. The `out` video port is the way to send the slope anywhere else. No audio worklet — the gate is a ConstantSourceNode pulsed on the event (PONG\'s pattern); the game logic runs at rAF cadence inside the bundle. Bundle committed pre-built at /skifree/skifree.bundle.js (~24 KB, esbuild IIFE of the upstream js/ classes + a thin embed wrapper); sprite sheets at /skifree/*.png. See packages/web/native/skifree/README.md for the regeneration recipe + attribution.',
  seqtris:
    'An 8x8 Tetris played on a Novation Launchpad and wired into the rack as a clocked note source. The pad grid IS the well and the right-hand scene column is the controller (top to bottom: reset board, two dead buttons, drop piece, rotate left, rotate right, move left, move right); the card mirrors the same picture and the same eight buttons so it plays without hardware. CONNECT grants Web MIDI and lists the attached Launchpads. The piece set is the normal seven with two shapes cut down for a short well — the straight line is a 2-cell domino and the two L pieces are 3-cell corners, so nothing is ever taller than three rows. Each piece carries one TRACKED SQUARE, fixed at spawn as the leftmost cell of its top row and immutable through rotation; its column picks the octave and its row picks a degree of a C major scale running DOWNWARD from the top row, so the leftmost column reaches down to C0 and the rightmost up to C8. PIECE (poly) fires that note on every move and on every spawn; a hard drop instead sends every row it falls through at once under one held gate — a tie. SPAWN pulses per new piece, LINE pulses once per cleared row. Gravity is counted in CLOCK PULSES (one row every GRAV pulses), and each cleared line steps GRAV down to the next whole number about ten percent faster, so the game accelerates without ever leaving the clock grid. QUANT latches button presses to the next clock pulse (default) or plays them free. BOARD (poly) is declared and silent — stubbed for now.',
  slewSwitch:
...
const PORT_NOTES: Record<string, string> = {
  ...
  'cube.base_vol': 'Per-voice VCA FLOOR the ADSR rides on top of: gain = base + (1-base)·env per ACTIVE (gated-or-releasing) voice. 0 (default) = pure ADSR (silent between notes); 0.5 floors at 0.5 and rises to 1.0 at the env peak; 1 = full, the envelope does nothing. A never-gated voice stays silent regardless of base. With nothing patched (raw VCO) there is no note to shape and CUBE runs at unity whatever this is set to — the legacy byte-identical drone.',
  ...
  'scope.out':
    'Mono-video output: pixel-equivalent of the faceplate\'s 2D scope render — both channels, scale/offset, range, XY/split mode, timeMs window. Driven by the cross-domain video bridge calling SCOPE.drawFrame() each video frame, so every scope control affects what downstream video modules see.',
  'scope.timeMs':
    'CV -> time window (ms across canvas width). Mirrors the Time fader 1:1 — the bridge re-reads the same params record so the faceplate\'s and video-out renders converge.',
  ...
  'timelorde.gate':
    'Level-sensitive show/hide for the beat-pulsing owl display. HIGH = owl on, LOW = off (same on/off state as the faceplate\'s owl button).',
  'timelorde.video_in':
  ...
  // ⚠ SAID `ff=0.95`, WHICH IS A VERSION OUT OF DATE. `DYNAMIC_SCALE.ff` is
  // 1.045, and `score-data.ts` records the change in its own comment
  // ("ff 0.95 -> 1.045"). That is not a rounding difference: it is the ONE
  // dynamic that scales the ENV output ABOVE unity, which is exactly the fact a
  // reader would want from this note, and the stale value said the opposite.
  'score.env': 'Envelope out: ADSR x dynamic (mf=0.55, ff=1.045 — the one marking that pushes ENV above unity).',
  ...
  'writeseq.rec':   'Record start/stop gate — a rising edge TOGGLES the record-arm latch (same as the faceplate\'s RECORD button).',
  ...
  'wavecel.base_vol': 'Per-voice VCA FLOOR the ADSR rides on top of: gain = base + (1-base)·env per ACTIVE (gated-or-releasing) voice. 0 (default) = pure ADSR (silent between notes); 0.5 floors at 0.5 and rises to 1.0 at the env peak; 1 = full, the envelope does nothing. A never-gated voice stays silent regardless of base. With nothing patched (raw VCO) there is no note to shape and WAVECEL runs at unity whatever this is set to — the legacy byte-identical drone.',
  ...
  'buggles.external_clock': 'Gate input. When patched and pulsing, replaces the internal woggle scheduler — every rising edge advances state. Detected on the main thread through the shared windowed edge counter, polled every 33 ms, so external clocks above ~30 Hz coalesce into one woggle event per poll.',
  ...
  'midiLane.cc_a':        'Learn-assignable CC tap A → 0..1 CV. Hit LEARN on the faceplate + wiggle a CC to bind (defaults to CC1 = mod wheel). Drives audio params directly or video params via the cross-domain bridge.',
...
export function buildModuleManifest(
  ...
  const audioEntries = Object.entries(sources)
    ...
    .filter(({ file }) => {
      ...
      // NUMPAD+'s WRITE seam — the same shape as kria-writes.ts one entry up:
      // the one origin-tagged, granular path into `node.data.layers` and
      // `node.data.keymap` that the legacy card, both faceplate panels and the
      // FACTORY's live recording all call. Not a ModuleDef (the def lives in
      // numpad-plus.ts).
      if (file === 'numpad-plus-writes.ts') return false;
      ...
      // SCORE's three non-def siblings, split out when the faceplate landed and
      // each for a different reason. `score-layout.ts` is the STAFF'S PIXEL
      // GEOMETRY, shared so the legacy card and the faceplate's panel cannot map
      // a click to one cell and paint the notehead in another.
      // `score-writes.ts` is the WRITE seam — the one origin-tagged path into
      // node.data both surfaces call, kept out of the pure `score-data.ts` so
      // that file stays store-free and unit-testable, exactly like kria's split
      // above. `score-transport-deps.ts` is the quicksave dependency object,
      // extracted because a second copy would reintroduce the stale-nodeId bug
      // its own comment records. None is a ModuleDef; the def lives in score.ts.
