...
const DESCRIPTIONS: Record<string, string> = {
  ...
  colourofmagic:
    'COLOUR OF MAGIC — a multi-colorspace video PROCESSOR. One video IN runs through THREE parallel colorspace blocks — RGB, YDbDr (the SECAM luma+chroma broadcast space), and HSV-or-HSL — each block encoding the picture into that space, adjusting each component, then decoding back to RGB. Every block has, per channel: a BIAS knob (additive offset, identity at 0) that also sums CV patched into its cv input; a MONO OVERRIDE input that REPLACES that channel with an incoming grayscale/video stream (bias still adds on top, so a patched key can still be offset); and an OVER/CLAMP toggle — CLAMP clips out-of-range to 0..1 (a legal clip) while OVER wraps it around via fract() (the LZX chroma-wrap look where over-driven values fold back through the spectrum). YDbDr chroma rides a 0.5 pedestal so a bias pushes the blue-yellow (Db) / red-cyan (Dr) axes symmetrically; HUE always wraps regardless of its toggle (which is advisory). The third block switches HSV↔HSL. EIGHT outputs run in parallel: pass (the untouched source), rgb / ydbdr / hsvhsl (each block colorized), and r / g / b / luma (mono taps of the ADJUSTED RGB block — the individual channels + their Rec.601 luma). The RGB block also has a palette REPLACE mode: three colour-picker swatches remap the adjusted R/G/B channels to chosen output colours (a duotone/tritone recolour) before the rgb out — at the default pure-red/green/blue picks it is a passthrough. With nothing patched all outputs are opaque black. All colorspace math is a 1:1 mirror of the pure, unit-tested $lib/video/colourofmagic-colorspace core. The card has a live preview (with an 8-way output selector), the three block columns of knobs + OVER/CLAMP pills, the HSV/HSL toggle, and the palette REPLACE swatches; all CV + mono-override jacks + the 8 outputs live in the sectioned yellow drill-down PATCH PANEL (IN / RGB / YDbDr / HSV-HSL / OUT, no raw side jacks).',
  tempest:
