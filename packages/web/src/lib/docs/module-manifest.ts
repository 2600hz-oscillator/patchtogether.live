...
const DESCRIPTIONS: Record<string, string> = {
  ...
  circles:
    'CIRCLES — stateful particle video generator (LZX-style primitive source). A GATE event (or the internal RATE clock) spawns a circle at a SEEDED-random position in a 1024-px field; each circle latches its diameter / vector / speed at spawn and moves in that direction, BOUNCING when its CENTER-POINT hits a wall (the velocity reflects; no edge/radius collision math, so a fat disc may briefly overhang). Model: a JS list of active circles {x,y,vx,vy,diameter} integrated on the engine rAF. Controls — D: circle DIAMETER (5..90 px); V: spawn VECTOR ANGLE (full range = 0..360°, every angle reachable); SPD: SPEED (0 = static scatter, up to 300 px/s ≈ crosses the field in ~3 s); RATE: KNOB-ONLY internal clock (0 = spawn ONLY on gate events; turning up spawns faster, hard-capped at 1 circle / 500 ms). Inputs — gate (a rising edge spawns one circle), d / v / spd (per-param CV), video (sampled by the mapped output). Outputs (4, all derived from a per-pixel overlap-COUNT of the active circles): OVERLAP (mono-video) white wherever ≥1 circle covers the pixel, black else; CONTOUR (mono-video) circle OUTLINES only, ring width = 10% of that circle\'s diameter (min 2 px) so many circles read as "ripples in a pond"; COMBINE (colour video) the overlap region colourized by overlap COUNT through a hue ramp (1 overlap = one hue; 2,3,4… cycle the spectrum) with brightness + saturation rising as more circles stack; MAPPED (colour video) shows the VIDEO input\'s contents wherever ≥2 circles overlap, black elsewhere. Bounded sim: circles bounce forever and accumulate, so a hard cap of 200 active circles culls the OLDEST first to keep per-frame cost bounded. Determinism: the random spawn position is from a seeded mulberry32 PRNG (fixed default seed; never Math.random), so VRT / per-port / behavioral sweeps are reproducible. Usage: patch a clock/sequencer GATE in (or just turn RATE up) and route OVERLAP / CONTOUR to a SCOPE or mono consumer, COMBINE to OUTPUT for the coloured stack, and a video source → VIDEO in + MAPPED → OUTPUT to punch that source through the ≥2-overlap region.',
  pentemelodica:
