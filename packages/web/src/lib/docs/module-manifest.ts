...
const DESCRIPTIONS: Record<string, string> = {
  ...
  wavecel:
    'Stereo wavetable oscillator with morph, stereo spread, and a West-Coast wavefolder — a more advanced sibling of WAVETABLEVCO. MORPH scans the wavetable frame position, SPREAD detunes/widens the stereo image, FOLD adds wavefolding harmonics. Ships factory wavetables and accepts runtime upload of E352-format WAV wavetables (frames ride the Y.Doc out to every rack-mate). The card offers a 3D wavetable visualization in addition to the standard scope view. A POLY input (polyPitchGate) accepts the 5-voice chord bus from MIDI LANE (mode=poly) or POLYSEQZ: when any lane is gated WAVECEL renders one wavetable voice per gated lane at that lane\'s pitch and sums them — the morph/spread/fold timbre is shared across all voices. With nothing patched to poly the mono `pitch` path runs unchanged. A per-voice amplitude ADSR (Attack / Decay / Sustain / Release) shapes each voice: in poly mode each lane\'s gate edge drives its own envelope (one envelope per voice, soft/click-safe retrigger — re-gating a still-releasing voice attacks from the current level, never pops); the mix is normalized over env-AUDIBLE voices (1/sqrt(N)) so a sustain=0 held note doesn\'t pump the level and a releasing tail doesn\'t pop. A mono TRIGGER gate input drives lane-0\'s envelope when poly is unpatched: the FIRST rising edge ever seen turns WAVECEL into a gated voice (attack on note-on, release on note-off); before any note — and whenever TRIGGER is unpatched — the envelope is skipped entirely and WAVECEL free-runs as a byte-identical drone (a save/reload of a patched-but-never-gated TRIGGER returns to the drone). The four ADSR params read live (continuous k-rate) across all stages, so a held chord rides sustain/release in real time and a fresh note attacks at the value present at its trigger moment. Edge detection is block-rate (retrigger granularity floor ≈ one audio block).',
  foxy:
...
const PORT_NOTES: Record<string, string> = {
  ...
  'cube.trigger': 'Mono TRIGGER gate for the per-voice amplitude ADSR (drives lane-0\'s envelope when POLY is unpatched). A level gate, not a pulse: rising edge = note-on (attack), falling edge = note-off (release). The FIRST rising edge ever seen turns CUBE into a gated voice thenceforth; before any note — and whenever this is unpatched — the envelope is skipped and CUBE free-runs as a byte-identical drone.',
  ...
  'wavecel.trigger': 'Mono TRIGGER gate for the per-voice amplitude ADSR (drives lane-0\'s envelope when POLY is unpatched). A level gate, not a pulse: rising edge = note-on (attack), falling edge = note-off (release). The FIRST rising edge ever seen turns WAVECEL into a gated voice thenceforth; before any note — and whenever this is unpatched — the envelope is skipped and WAVECEL free-runs as a byte-identical drone.',
