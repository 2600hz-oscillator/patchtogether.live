...
const DESCRIPTIONS: Record<string, string> = {
  ...
  circles:
    'CIRCLES — stateful particle video generator (LZX-style primitive source). A GATE event (or the internal RATE clock) spawns a circle at a SEEDED-random position in a 1024-px field; each circle latches its diameter / vector / speed / decay at spawn and moves in that direction, BOUNCING when its CENTER-POINT hits a wall (the velocity reflects; no edge/radius collision math for the WALL, so a fat disc may briefly overhang). With the COLLIDE gate HIGH, circles ALSO bounce off EACH OTHER: an EDGE-detection elastic collision — two circles collide when the distance between their CENTERS ≤ (r1 + r2), i.e. their painted discs touch (unlike the center-based wall bounce, this uses the radii), and an equal-mass ELASTIC response swaps the velocity components along the center-to-center normal and separates the pair so they don\'t stick (each keeps its independent latched SPEED as far as elastic physics allows). Gate LOW / unpatched → circles PASS THROUGH each other (the default). It is a LIVE GLOBAL mode (read every frame), NOT spawn-latched. The inter-circle pass is O(n²) over the active list, bounded by the 200-circle cap (~10k pair tests/frame). Every per-circle property is LATCHED at spawn from the live knob+CV — crucially SPEED: a circle integrates from its OWN latched velocity for its whole life, so turning SPD after a circle exists affects ONLY newly-spawned circles, never the ones already flying. Model: a JS list of active circles {x,y,vx,vy,diameter,decayS,ageS,alpha} integrated on the engine rAF. Controls — D: circle DIAMETER (5..270 px); V: spawn VECTOR ANGLE (full range = 0..360°, every angle reachable); SPD: SPEED (0 = static scatter, up to 300 px/s ≈ crosses the field in ~3 s; latched independently per circle); DECAY: per-circle FADE-OUT time — 0 = NO decay (the circle PERSISTS, the static-field case, FIFO-capped) ramping up to a 10 s fade where the circle\'s alpha ramps 1 → 0 and it is removed; RATE: KNOB-ONLY internal clock (0 = spawn ONLY on gate events; turning up spawns faster, hard-capped at 1 circle / 500 ms). Inputs — gate (a rising edge spawns one circle), collide (a LIVE gate: HIGH = circles bounce off each other elastically via edge detection, LOW/unpatched = pass through), d / v / spd / decay (per-param CV), video (sampled by the mapped output). Outputs (4, all derived from a per-pixel overlap-COUNT of the active circles, each circle\'s contribution scaled by its DECAY fade alpha so a fading circle counts less / draws lighter): OVERLAP (mono-video) white wherever ≥1 circle covers the pixel (dimming as the covering circle fades), black else; CONTOUR (mono-video) circle OUTLINES only, ring width = 10% of that circle\'s diameter (min 2 px) so many circles read as "ripples in a pond" (rings lighten as they decay); COMBINE (colour video) the overlap region colourized by overlap COUNT through a hue ramp (1 overlap = one hue; 2,3,4… cycle the spectrum) with brightness + saturation rising as more circles stack and dimming as the stack fades; MAPPED (colour video) shows the VIDEO input\'s contents wherever ≥2 circles overlap, black elsewhere. Bounded sim: circles bounce forever and accumulate, so a hard cap of 200 active circles culls the OLDEST first (a safety net even with DECAY) to keep per-frame cost bounded. Determinism: the random spawn position is from a seeded mulberry32 PRNG (fixed default seed; never Math.random), so VRT / per-port / behavioral sweeps are reproducible. Usage: patch a clock/sequencer GATE in (or just turn RATE up) and route OVERLAP / CONTOUR to a SCOPE or mono consumer, COMBINE to OUTPUT for the coloured stack, and a video source → VIDEO in + MAPPED → OUTPUT to punch that source through the ≥2-overlap region. Leave DECAY at 0 for an accumulating static field; turn it up to make circles fade and vanish for a trail/dissolve look. Gate the COLLIDE input HIGH (any clock/gate/LFO over the high threshold) to switch the field from a soft overlapping wash into a billiards-like cluster where the discs knock each other around.',
  pentemelodica:
