...
const DESCRIPTIONS: Record<string, string> = {
  ...
  blood:
    'Interactive BLOOD video source — the NBlood (Build-engine) port of Blood, compiled to WebAssembly and rendered as a video module. Owner-only and single-instance, like DOOM: the rack owner spawns it and plays; the video output is the Build software-rendered framebuffer, aspect-correct letterboxed into the canvas. CV-typed gate inputs (up/down/left/right, fire, altfire, use, jump, crouch, weapnext/weapprev, esc, enter) drive the marine from cables, and the focused card also captures the keyboard. Stereo audio_l/audio_r outputs are wired but silent in v1 (the PCM bridge is stubbed). Game data (BLOOD.RFF / GUI.RFF / SOUNDS.RFF / TILES000.ART) is USER-SUPPLIED and not redistributable — there is no out-of-box play; the card shows a "Blood data missing — run task setup:blood" overlay until you provide a copy you own (GOG/Steam One Unit Whole Blood or Fresh Supply).',
  bluebox:
