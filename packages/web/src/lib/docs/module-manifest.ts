...
const DESCRIPTIONS: Record<string, string> = {
  ...
  timelorde: 'Singleton master clock. Internal or external BPM, twelve clock-divider outputs. The card carries a big beat-pulsing neon WIZARD display; patch a feed into VIDEO IN and the display becomes a live monitor while VIDEO OUT passes the feed through (TIMELORDE can sit inline in a video chain).',
  toybox:
...
const PORT_NOTES: Record<string, string> = {
  ...
  'timelorde.video_in':
    'Patch a video feed here and the big card display becomes a LIVE MONITOR of it (the wizard steps aside); also routed to video_out so TIMELORDE passes the feed through inline. Unpatched = the wizard shows.',
  'timelorde.video_out':
    'The picture the big display shows — the live VIDEO IN feed when one is patched, else the beat-pulsing wizard. Lets TIMELORDE sit inline in a video chain (in -> display -> out).',
