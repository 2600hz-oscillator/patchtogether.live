...
const DESCRIPTIONS: Record<string, string> = {
  ...
  clipplayer:
    'CLIP PLAYER — a clip-launcher sequencer (Ableton Session-view model) built around a 64-slot "clip page". v1 clips are NOTE/PATTERN clips: each clip is a small step pattern, and the module is a sequencer whose pattern is the LAUNCHED clip. It is the dedicated companion to a monome grid 128 controller (browser-native WebSerial, no native helper — see lib/grid) but is fully usable from the card alone. Model: launching a clip queues it; on the next clip-loop boundary it takes over and its pattern runs out the pitch/gate/velocity outputs (v1 plays ONE clip at a time — quantize-switching between the 64 — with simultaneous multi-track playback as a follow-up). The active/queued clip syncs over the Y.Doc so collaborators (and a second grid) see the same session; grid LED + serial I/O stay per-user local. Card: an 8×8 launch grid (click empty = create a clip, click loaded = launch/queue, click the playing pad = stop; a STOP ALL button) plus a Deluge-style note editor for the selected clip (X = step, Y = pitch, in-key scale-degree rows by default, octave shift, a clock-locked playhead column). Inputs — CLOCK IN: external clock; each rising edge advances one step (patch TIMELORDE here to lock the grid to the system tempo; uses the canonical windowed edge counter, no double-count), STOP ALL: a rising edge stops the playing clip. Outputs — PITCH: the launched clip\'s pitch as a poly-capable V/oct cable (chords fill lanes; a mono pitch sink auto-receives lane 0), GATE: high while a note sounds, VELOCITY: per-note velocity as a 0..1 CV (patch into a VCA/ADSR amount), CLIP GATE: a trigger pulse on each clip downbeat (when a clip starts post-quantize, and at every loop) for downstream sync. Params — BPM: internal step tempo when CLOCK IN is unpatched; QNT (quantize): when on, a queued clip takes over only at the active clip\'s loop boundary (off = launch immediately); OCT: octave transpose applied to all pitches; GATE: per-step gate duty cycle. All ports live on the yellow drill-down PATCH PANEL (no side jacks); knobs are MIDI / control-surface assignable. Usage: build a few note clips, patch PITCH/GATE into a voice (VCO + VCA/ADSR) and CLOCK IN from TIMELORDE, then launch/quantize-switch clips from the card (or a monome grid) to perform.',
  riotgirls:
