...
const DESCRIPTIONS: Record<string, string> = {
  ...
  gibribbon:
    'GIBRIBBON (video) — a CV-controlled rhythm line-runner in the spirit of Vib-Ribbon, cast with DOOM shareware sprites as fair-use artistic parody (owner-ruled full rewrite, 2026-08-28). A white vector ribbon scrolls right-to-left on black; the COURSE IS DERIVED from whatever is patched into the four channel inputs by an ADAPTIVE prominence extractor — each channel is measured against its own rolling recent range (relative peaks, rank competition, a starvation boost), so any varying source at any gain yields a playable stream, and a silent or stuck-flat channel spawns nothing. Four event kinds map to the ABXY buttons (loop=A pit-V, jump=B hump, imp=X, zombie=Y — imps/former-humans render as real TROO*/POSS* sprites, the marine as PLAY*, decoded at load from the setup-fetched shareware DOOM1.WAD with a line-art fallback when absent). ONE clock drives everything: an external CLOCK edge advances one course tick, or the TEMPO knob clocks the same path unpatched; judgement is anchored to the tick-derived phase so no frame rate can move the timing window. Hits score with a combo multiplier and climb a DOOM-style health ladder to SUPER; misses degrade to GAME OVER; the new restart gate input (plus the R key and the RESET action) starts a fresh run so a rack can loop the game. An idle unpatched module self-plays in ATTRACT mode, labelled IN the picture; any input starts a real run, and the ATTRACT toggle disables self-play. Score/combo/state are painted INTO the frame by the module rasteriser (no DOM HUD). Outputs: out (video), evt_hit/miss/fire/kill/gameover (10 ms gates), health_cv. The pure engine (gibribbon-engine.ts) is deterministic — seed + scheduler-tick count only — and the module is VRT-baselined via its __gibribbonVrtSeed/__gibribbonVrtTicks pins. DOOM shareware terms apply (same file as the DOOM module, read-only).',
  twotracks:
