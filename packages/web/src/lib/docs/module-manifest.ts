...
const DESCRIPTIONS: Record<string, string> = {
  ...
  toybox:
    'Four-layer video compositor — the foundation of a planned shader/video/OBJ engine (editable combine graph, CV targets, presets, node editor). PHASE 2 renders all 4 layers + a combine stage. Each layer (kind shader/gen/video/off) resolves to a texture per frame: shader/gen compile a bundled MIT shader (GEN: noise-fbm, worley-cells; FX: hsv-plasma, cos-gradient) into the layer FBO; video samples the matching layer0_in..layer3_in video INPUT port; off feeds black. The combine stage composites the 4 textures via a data-driven op graph (node.data.combine) — P2 ships a fixed default chain: fade(L0,L1) → map(screen-blend, that, L2) → lumakey(fg=L3, bg=that) → output. Ops: fade = mix(A,B,t); lumakey = luminance key (threshold/softness/invert); chromakey = hue key (key colour + tolerance/softness/spill); map = selectable blend mode (screen/multiply/add/darken/lighten/difference/overlay) + mix. Each op is one fullscreen pass through a 3-FBO scratch pool, topo-walked (Kahn); the output texture blits to the module out. A P1-shaped node (only layer 0, no combine) short-circuits to layer 0 straight — byte-identical to P1. Content catalog + param schema live in a static manifest (packages/web/static/toybox/manifest.json); GLSL fetched lazily on selection (never JS-bundled), compiled + cached per content. Inputs layer0_in..layer3_in (video); one video output (out).',
