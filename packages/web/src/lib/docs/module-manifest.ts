...
const DESCRIPTIONS: Record<string, string> = {
  ...
  toybox:
    'Multi-layer video compositor. FOUR layers, each rendered into its own framebuffer then reduced to the output by a combine DAG (fade / lumakey / chromakey / map). A layer kind selects its source: SHADER/GEN = a fragment-shader content entry from the bundled MIT bank (GEN: noise-fbm domain-warped simplex FBM, worley-cells animated cellular noise; FX: hsv-plasma sine-interference plasma → HSV→RGB, cos-gradient IQ cosine-palette rings) with iTime + iResolution + its declared float params on faders. OBJ = a 3D mesh layer (Phase 3): a bundled CC0 OBJ (Spot the cow, Utah teapot, chess pawn — parsed by an in-house ASCII OBJ parser with auto-center/auto-scale + computed flat normals) OR a built-in procedural primitive (cube / sphere / torus / hypercube tesseract; no asset file), matcap-shaded with depth testing — the matcap is synthesized procedurally in-shader (chrome / clay / neon styles, zero asset-license surface) and the layer carries rotX/rotY/rotZ, scale, spin (auto-rotate) and an RGB tint. The content/model catalogs + per-shader param schema live in a static manifest (packages/web/static/toybox/manifest.json); GLSL + OBJs are fetched lazily on selection (never JS-bundled) and cached. Phase 5 adds a FIXED pool of 8 generic CV input ports (cv1..cv8) plus a CV-routing tab: each generic port is assigned (via node.data.cvRoutes) to any addressable param — a shader uniform on a layer, an OBJ-layer transform/tint field, or a combine op node float param — and setParam re-scales the incoming ±1 CV across THAT param declared min/max (reusing the same cv-scale helper the cv-bridge uses) so a ±1 modulator sweeps the routed param full range. Persistence: node.data.layers (4-length array of { kind, contentId, params, material }) + node.data.combine + node.data.cvRoutes. Eight CV inputs (cv1..cv8) + one video output (out).',
