...
const DESCRIPTIONS: Record<string, string> = {
  ...
  mappy:
    'MAPPY — a multi-surface MANUAL projection mapper. Spawns up to SIX SURFACES; each surface is fed by a distinct video input (in1..in6) and warped onto its own DRAGGABLE QUAD in the output frame, then composited (painter\'s order, OVER) into ONE video output → a projector. Use it to DE-SKEW one awkwardly-angled projection (drag the four corners to match the physical screen) or to map up to 6 feeds onto the faces of a white cube (only ~3-4 faces are ever visible from one projector angle). WARP: each surface owns a 4-corner QUAD in NORMALIZED [0,1] output space (corner order TL, TR, BR, BL). The homography (unit-square → that quad) defines the projective warp; the shader runs per OUTPUT texel, applies the INVERSE homography to find the matching SOURCE uv, and samples the input there (sampling only where the source uv is inside [0,1], else transparent so under-layers show through). Surfaces composite in input order (in1 first … in6 last) with OVER blend. DEFAULT: every surface starts FULL-FRAME (the unit quad), so connecting any single input immediately fills the frame with that feed — then drag its corners in to skew/shrink the footprint. SHOW GRID: a toggle that draws a NUMBERED calibration grid (a per-surface-tinted checker + border + cross-hairs + a tally encoding the surface number) warped into each connected surface\'s quad INSTEAD of the input — line the grid up to the physical surface, then turn it off. CARD: a live composite PREVIEW with an SVG overlay of every connected surface\'s draggable corner handles + quad outline (coloured per surface; the focused surface\'s handles come to front), a legend with per-surface focus + "reset corners to full-frame" buttons, and the grid toggle. All ports live on the yellow drill-down PATCH PANEL (no raw side jacks). v1 is the MANUAL mapper — the camera-assisted AUTO-align (point a camera at the projection, solve the homography from detected features) is a LATER phase; there is no camera input and no CV in v1 by design. The pure 2D projective math lives in $lib/video/mappy-homography (DLT solve / apply / invert / column-major-for-GLSL), shared by the shader + the unit tests.',
  cellshade:
