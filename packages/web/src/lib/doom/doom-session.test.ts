...
import { shouldOpenMultiplayer, guestWaitingState, GS_LEVEL } from './doom-session';
// computeMpLive + the Join-affordance helpers are authoritatively in
// doom-gating; the end-to-end deadlock scenario pulls them from there.
import { computeMpLive, joinAffordance } from './doom-gating';
...
describe('joinAffordance (no Join deadlock — owner spec: shown, disabled until mpLive)', () => {
  it('shows Join to an unjoined non-host guest (host not live → disabled, never hidden)', () => {
    const j = joinAffordance({ isHost: false, alreadySeated: false, full: false, mpLive: false });
    expect(j.show).toBe(true);
    expect(j.show && j.enabled).toBe(false);
  });

  it('hides Join from the host and from a seated player', () => {
    expect(joinAffordance({ isHost: true, alreadySeated: false, full: false, mpLive: true }).show).toBe(false);
    expect(joinAffordance({ isHost: false, alreadySeated: true, full: false, mpLive: true }).show).toBe(false);
  });

  it('ENABLES Join the instant the host is running a live MP game', () => {
    const j = joinAffordance({ isHost: false, alreadySeated: false, full: false, mpLive: true });
    expect(j.show && j.enabled).toBe(true);
  });

  it('disables Join when the game is full even if the host is live', () => {
    const j = joinAffordance({ isHost: false, alreadySeated: false, full: true, mpLive: true });
    expect(j.show && j.enabled).toBe(false);
  });
});
...
describe('end-to-end deadlock scenario', () => {
  it('host in-level with members present ⇒ mpLive true ⇒ guest Join enabled, no false wait', () => {
    ...
    // Guest (unjoined, sees hostMpLive): Join is SHOWN + ENABLED, and the
    // copy is NOT the false "Waiting for host to start…".
    const guestJoin = joinAffordance({
      isHost: false,
      alreadySeated: false,
      full: false,
      mpLive: hostMpLive,
    });
    expect(guestJoin.show).toBe(true);
    expect(guestJoin.show && guestJoin.enabled).toBe(true);
