...
describe('buildDoomPatchPanelSections — shape contract', () => {
  it('emits 4 per-player sections (P1..P4) + a final Cheats section regardless of mySlot', () => {
    for (const slot of [null, 0, 1, 2, 3]) {
      ...
      expect(sections.slice(0, 4).map((s) => s.label.startsWith('Player '))).toEqual([true, true, true, true]);
      expect(sections[4]!.label).toBe('Cheats');
    }
  });

  it('renders 38 input ports total (4 slots × 9 gates + 2 cheat gates) with cable=cv', () => {
    const sections = buildDoomPatchPanelSections(0);
    const allInputs = sections.flatMap((s) => s.inputs);
    expect(allInputs).toHaveLength(4 * CV_GATE_PORT_IDS.length + 2);
    expect(allInputs).toHaveLength(38);
    for (const input of allInputs) {
      expect(input.cable).toBe('cv');
      // Every input id is either a per-slot gate (p1_up, …, p4_enter) or one
      // of the two global cheat gates (iddqd_in / idkfa_in).
      expect(input.id).toMatch(/^(p[1-4]_(up|down|left|right|space|ctrl|alt|esc|enter)|(iddqd|idkfa)_in)$/);
    }
  });
  ...
  it("labels the local player's section ' (you)' so the operator can spot their slot", () => {
    for (const me of [0, 1, 2, 3]) {
      ...
      // Only the first 4 sections are per-player; the final 'Cheats' is global.
      for (const [idx, section] of sections.slice(0, 4).entries()) {
  ...
  it('renders the full output set on section 0 in def declaration order', () => {
    ...
    expect(sections[4]!.outputs).toBeUndefined();
  ...
  it('total handle count = 36 per-slot CV + 2 cheat inputs + N outputs (one per doomDef output)', () => {
    ...
    // 36 per-slot CV (4 slots × 9 gates each) + 2 cheat inputs from the Cheats section.
    expect(inputs.length).toBe(36 + DOOM_CHEAT_INPUTS.length);
    expect(outputs.length).toBe(doomDef.outputs.length);
  });

  it('Cheats section carries IDDQD + IDKFA gate inputs in declaration order', () => {
    const sections = buildDoomPatchPanelSections(0);
    const cheats = sections[4]!;
    expect(cheats.label).toBe('Cheats');
    expect(cheats.inputs).toEqual(DOOM_CHEAT_INPUTS);
    expect(cheats.inputs.map((p) => p.id)).toEqual(['iddqd_in', 'idkfa_in']);
    expect(cheats.inputs.map((p) => p.label)).toEqual(['IDDQD', 'IDKFA']);
    for (const p of cheats.inputs) expect(p.cable).toBe('cv');
  });
  ...
  it('input ids match the def-declared per-slot gate ids (no drift)', () => {
    ...
    expected.push('iddqd_in', 'idkfa_in');
