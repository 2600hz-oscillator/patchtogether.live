...
export const trailsDef: AudioModuleDef = {
  ...
  async factory(ctx, node): Promise<AudioDomainNodeHandle> {
    ...
    const unsubscribeDisconnect = subscribeTrailsDisconnect(() => {
      decoder.reset();
      tickCounter = 0;
      const at = ctx.currentTime;
      for (const ch of TRAILS_CHANNELS) {
        channelStates[ch - 1]!.gate = false;
        writeGate(trailsGatePortId(ch), false, at);
        writeGate(trailsTrigPortId(ch), false, at);
        for (const voice of polySenders.get(ch)!.voices) {
          voice.gateSrc.offset.cancelScheduledValues(at);
          voice.gateSrc.offset.setValueAtTime(0, at);
        }
      }
      writeGate(TRAILS_CLOCK_PORT_ID, false, at);
    });

    // Device ownership outlives the face. Restore a saved native input even
    // when this module is loaded collapsed or no editor is mounted.
    void restoreNativeTrails();
