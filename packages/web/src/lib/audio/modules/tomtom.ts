...
export const tomtomDef: AudioModuleDef = {
  ...
  docs: {
    ...
    controls: {
      ...
      noise: "Membrane ↔ breath balance (0–1): seeded noise through a band-pass centred 2.5× above the settled pitch (floored at 300 Hz, so below roughly 120 Hz TUNE it stops tracking the drum), riding a decay derived from — and always shorter than — the main DECAY. The SDS-V tone/noise mix law: 0 = pure membrane; full up the breath dominates and the membrane sits back at 30 %, never silent. noise_cv sums in.",
