...
describe('snaredrumDef — module def shape', () => {
  it('module id is "snaredrum" + LOWERCASE label "snare drum"', () => {
    expect(snaredrumDef.type).toBe('snaredrum');
    expect(snaredrumDef.label).toBe('snare drum');
    expect(snaredrumDef.label).toBe(snaredrumDef.label.toLowerCase());
  });

  it('declares the 6 frozen inputs with the declared edge semantics', () => {
    expect(snaredrumDef.inputs.map((i) => i.id)).toEqual([
      'trigger_in', 'gate_in', 'roll_speed_cv', 'accent_in', 'pitch_cv', 'choke_in',
    ]);
    const byId = Object.fromEntries(snaredrumDef.inputs.map((p) => [p.id, p] as const));
    // The STRIKE fires once per rising edge — a trigger.
    expect(byId.trigger_in).toMatchObject({ type: 'gate', edge: 'trigger' });
    // The DRUMROLL acts WHILE high — a gate.
    expect(byId.gate_in).toMatchObject({ type: 'gate', edge: 'gate' });
    // CHOKE is level-sensitive (both edges) — a gate.
    expect(byId.choke_in).toMatchObject({ type: 'gate', edge: 'gate' });
    expect(byId.roll_speed_cv!.type).toBe('cv');
  ...
  it('declares separate stereo audio_l / audio_r outs + the stereo pair', () => {
    expect(snaredrumDef.outputs.map((o) => o.id)).toEqual(['audio_l', 'audio_r']);
    expect(snaredrumDef.outputs.every((o) => o.type === 'audio')).toBe(true);
    expect(snaredrumDef.stereoPairs).toEqual([['audio_l', 'audio_r']]);
  });

  it('declares all 22 params with the frozen defaults/ranges/curves', () => {
    const byId = Object.fromEntries(snaredrumDef.params.map((p) => [p.id, p] as const));
    expect(byId.tune).toMatchObject({       defaultValue: 180,  min: 90,   max: 400,  curve: 'log' });
    expect(byId.tone).toMatchObject({       defaultValue: 0.5,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.damping).toMatchObject({    defaultValue: 0.4,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.head_decay).toMatchObject({ defaultValue: 180,  min: 30,   max: 600,  curve: 'log' });
    expect(byId.body_decay).toMatchObject({ defaultValue: 110,  min: 20,   max: 300,  curve: 'log' });
    expect(byId.pitch_amt).toMatchObject({  defaultValue: 3,    min: 0,    max: 12,   curve: 'linear' });
    expect(byId.pitch_time).toMatchObject({ defaultValue: 18,   min: 3,    max: 80,   curve: 'log' });
    expect(byId.wire).toMatchObject({       defaultValue: 0.7,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.wire_tone).toMatchObject({  defaultValue: 4500, min: 1500, max: 9000, curve: 'log' });
    expect(byId.wire_decay).toMatchObject({ defaultValue: 260,  min: 40,   max: 700,  curve: 'log' });
    expect(byId.crack).toMatchObject({      defaultValue: 0.4,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.crack_tone).toMatchObject({ defaultValue: 3200, min: 800,  max: 7000, curve: 'log' });
    expect(byId.damp).toMatchObject({       defaultValue: 0.2,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.roll_speed).toMatchObject({ defaultValue: 0.5,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.bounce).toMatchObject({     defaultValue: 0.35, min: 0,    max: 1,    curve: 'linear' });
    expect(byId.humanize).toMatchObject({   defaultValue: 0.2,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.spread).toMatchObject({     defaultValue: 0.5,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.drive).toMatchObject({      defaultValue: 0.2,  min: 0,    max: 1,    curve: 'linear' });
    ...
    expect(byId.width).toMatchObject({      defaultValue: 0.4,  min: 0,    max: 1,    curve: 'linear' });
    expect(byId.level).toMatchObject({      defaultValue: 0,    min: -24,  max: 12,   curve: 'linear' });
    expect(Object.keys(byId)).toHaveLength(22);
  });

  it('every port + param is documented (STRICT_DOCS completeness)', () => {
    const d = snaredrumDef.docs!;
    for (const i of snaredrumDef.inputs) expect(d.inputs?.[i.id], `input ${i.id}`).toBeTruthy();
    for (const o of snaredrumDef.outputs) expect(d.outputs?.[o.id], `output ${o.id}`).toBeTruthy();
    for (const pr of snaredrumDef.params) expect(d.controls?.[pr.id], `control ${pr.id}`).toBeTruthy();
  });
});
