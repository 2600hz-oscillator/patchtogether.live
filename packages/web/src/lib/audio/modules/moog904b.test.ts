...
describe('moog904bDef: module def shape', () => {
  it('declares type=moog904b, label="Moog 904B VCF", category=filters, schemaVersion=1', () => {
    expect(moog904bDef.type).toBe('moog904b');
    expect(moog904bDef.label).toBe('Moog 904B VCF');
    expect(moog904bDef.category).toBe('filters');
    expect(moog904bDef.schemaVersion).toBe(1);
  });

  it('is categorized under Moog → SYS55 and uses the Moog904bVcfCard', () => {
    expect(moog904bDef.palette).toEqual({ top: 'Moog', sub: 'SYS55' });
