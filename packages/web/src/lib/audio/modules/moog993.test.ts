...
describe('moog993Def: module def shape', () => {
  it('declares type=moog993, label="Moog 993 Trig", category=modulation, schemaVersion=1', () => {
    expect(moog993Def.type).toBe('moog993');
    expect(moog993Def.label).toBe('Moog 993 Trig');
    expect(moog993Def.category).toBe('modulation');
    expect(moog993Def.schemaVersion).toBe(1);
    expect(moog993Def.palette).toEqual({ top: 'Moog', sub: 'SYS55' });
  });
