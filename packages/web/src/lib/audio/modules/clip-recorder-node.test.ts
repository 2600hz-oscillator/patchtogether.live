...
describe('coerceClipRecorderMsg', () => {
  ...
  it('accepts well-formed chunk and done messages', () => {
    ...
    expect(
      coerceClipRecorderMsg({ type: 'done', lane: 7, frames: 96_000, startFrame: 5_760, gapFrames: 0 }),
    ).toEqual({ type: 'done', lane: 7, frames: 96_000, startFrame: 5_760, gapFrames: 0 });
    ...
    expect(
      coerceClipRecorderMsg({ type: 'done', lane: 3, frames: 0, startFrame: 12_800, gapFrames: 0 }),
    ).toEqual({ type: 'done', lane: 3, frames: 0, startFrame: 12_800, gapFrames: 0 });
    // A PADDED take — the render clock skipped and the worklet filled the hole
    // with silence — carries the hole's size through, truncated to a frame.
    expect(
      coerceClipRecorderMsg({ type: 'done', lane: 3, frames: 96_000, startFrame: 5_760, gapFrames: 480 }),
    ).toEqual({ type: 'done', lane: 3, frames: 96_000, startFrame: 5_760, gapFrames: 480 });
  });
  it('rejects everything malformed instead of throwing mid-take', () => {
    ...
    expect(
      coerceClipRecorderMsg({ type: 'done', lane: 0, frames: -1, startFrame: 0, gapFrames: 0 }),
    ).toBeNull();
    ...
    expect(coerceClipRecorderMsg({ type: 'done', lane: 0, frames: 128, gapFrames: 0 })).toBeNull();
    expect(
      coerceClipRecorderMsg({ type: 'done', lane: 0, frames: 128, startFrame: NaN, gapFrames: 0 }),
    ).toBeNull();
    // ⚠ And so is `gapFrames`, for the same reason: a default of 0 would
    // report "no device-glitch silence" for a take nobody measured — the
    // silent repair, back again.
    expect(coerceClipRecorderMsg({ type: 'done', lane: 0, frames: 128, startFrame: 0 })).toBeNull();
    expect(
      coerceClipRecorderMsg({ type: 'done', lane: 0, frames: 128, startFrame: 0, gapFrames: -1 }),
    ).toBeNull();
    expect(
      coerceClipRecorderMsg({ type: 'done', lane: 0, frames: 128, startFrame: 0, gapFrames: NaN }),
    ).toBeNull();
...
describe('attachClipRecorderSink', () => {
  it('routes chunks through the REAL drain: positions = firstFrame × 8, in order, dropped === 0, done after the last chunk', async () => {
    ...
    const handler = attachClipRecorderSink({ port } as unknown as Pick<AudioWorkletNode, 'port'>, {
      ...
      onDone: (lane, frames, startFrame, gapFrames) => done.push([lane, frames, startFrame, gapFrames]),
    });
    ...
    handler({
      data: { type: 'done', lane: 1, frames: 4, startFrame: 640, gapFrames: 1 },
    } as MessageEvent);
    await drain.flush();
    ...
    // compares against the start it asked for to see a slid take — and so does
    // the padded gap, which is how the registry sees a glitched one.
    expect(done).toEqual([[1, 4, 640, 1]]);
  });
