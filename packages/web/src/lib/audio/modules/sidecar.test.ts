...
describe('sidecarDef — module def shape', () => {
  it('declares 7 inputs (audio L/R, sc L/R, threshold_cv, env_mag_cv, input_level_cv)', () => {
  ...
  it('declares all 9 params with the documented ranges + defaults', () => {
    const byId = Object.fromEntries(sidecarDef.params.map((p) => [p.id, p] as const));
    expect(Object.keys(byId).sort()).toEqual([
      'attack', 'envMag', 'inputLevel', 'knee', 'makeup', 'ratio', 'release', 'sc_hpf', 'threshold',
    ]);
    ...
    // inputLevel is the sidechain input volume: 0..2 (0%..200%), default 1.
    expect(byId.inputLevel).toMatchObject({ min: 0,   max: 2,    curve: 'linear', defaultValue: 1 });
  ...
  it('CV inputs target threshold + envMag + inputLevel with linear cvScale', () => {
    ...
    const lvl = sidecarDef.inputs.find((p) => p.id === 'input_level_cv')!;
    expect(lvl.paramTarget).toBe('inputLevel');
    expect(lvl.cvScale).toEqual({ mode: 'linear' });
  });
