...
// (octave or 5th doubling)). Wires directly into DX7 (when it
...
export const polyseqzDef: AudioModuleDef = {
  ...
  docs: {
    ...
    outputs: {
      poly:
        "The current step's chord as a 5-voice POLY cable (each lane carries its own pitch CV + gate). Patch into a poly-aware voice (DX7 / CUBE / any module with a poly input) so each chord tone gets its own voice; a mono pitch input automatically receives just lane 0, the chord's root.",
      gate:
