...
export const samsloopDef: AudioModuleDef = {
  ...
  docs: {
    ...
    controls: {
      ...
      "samsloop-denoise-{n}":
        "Removes steady background noise or tape hiss from the sample, in place. The noise profile is learned from the sample itself (the quietest fifth of its frames, per frequency band), then a spectral gate with a −12 dB floor attenuates each band toward that profile — hiss in the gaps drops by up to 12 dB, the voice or instrument on top is left within a dB. It REFUSES, with a reason under the waveform, when there is no steady floor to find: a pad, a drone, a clean take or a phrase with no pauses is a level change, not a denoise, and is left untouched. A second press on an already-denoised sample refuses too (it would only dull the take). The rewrite is NOT UNDOABLE — like REC, export first is the way back — and it lands as mono PCM at the source's own bit depth and rate (an 8-bit take stays 8-bit; a mp3 upload becomes a WAV on export). Run it BEFORE normalize.",
      "samsloop-normalize-{n}":
