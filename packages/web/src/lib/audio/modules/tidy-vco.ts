...
//
// ── Model ────────────────────────────────────────────────────────────────
// A complete 2-oscillator VA voice in one card: OSC1 + OSC2 (a band-limited
// SINE → TRIANGLE → SQUARE SHAPE morph — polyBLEP on the square's edges,
// polyBLAMP on the triangle's corners, every anchor RMS-calibrated so the
// knob is a timbre control; shared PW/PWM on the square leg, OSC2 octave +
// detune) + a −1-oct
...
export const tidyVcoDef: AudioModuleDef = {
  ...
  docs: {
    ...
    inputs: {
      ...
      pwm_cv: 'Pulse-width modulation CV: ±0.45 duty per volt on the shared PW, audio-rate. The effective width is clamped 0.05–0.95, so CV reaches wide duties past the knob’s 0.5 (square) ceiling — and because 0.5 IS the square, sweeping either side of it is symmetric. Classic PWM strings come from an LFO here with SHAPE at (or near) the square end, where the pulse leg is in the mix.',
      ...
      shape1_cv: 'OSC1 SHAPE CV: ±1 V sweeps the full sine→triangle→square morph (0..1) on top of the knob. GLOBAL (all 5 voices), block-rate; cv = 0 is a no-op.',
      shape2_cv: 'OSC2 SHAPE CV: ±1 V sweeps the full sine→triangle→square morph (0..1) on top of the knob. GLOBAL (all voices), block-rate.',
    ...
    controls: {
      shape1: 'OSC1 waveform morph, with three REAL landmarks: 0 = a pure SINE, 0.5 = an exact TRIANGLE, 1 = a SQUARE (the pulse leg, at PW’s duty). Between them it crossfades continuously, so the harmonics grow smoothly — none at all at sine, a soft 1/n² odd series at triangle, the full 1/n odd series at square — with no click, no level jump and no harmonic discontinuity anywhere in the travel. All three are band-limited (polyBLEP on the square’s edges, polyBLAMP on the triangle’s corners) and RMS-matched to each other, so the knob moves timbre, not loudness.',
      shape2: 'OSC2 waveform morph, same law as SHAPE 1 (0 = sine, 0.5 = triangle, 1 = square).',
      pw: 'Duty of the SQUARE leg both oscillators share — the top end of the SHAPE morph. At its 0.5 default (and maximum) that leg is a TRUE square; turning PW down narrows it toward 0.05, a thin reedy sliver that also thins out (a real pulse loses energy as its duty shrinks — the classic PWM level taper, deliberately left uncompensated). It has nothing to act on while SHAPE sits at the sine/triangle end, where the pulse leg is out of the mix. The PWM CV jack modulates around the knob and can push the effective width past its square ceiling, up to 0.95.',
