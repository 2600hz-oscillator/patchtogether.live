...
export const cloudsDef: AudioModuleDef = {
  ...
  docs: {
    ...
    controls: {
      ...
      freeze: 'Freeze toggle (0/1): when 1, the module stops writing new audio into the buffer and loops the captured snapshot, so the texture sustains indefinitely with no fresh input. A LATCH — it stays engaged where you leave it, and clicking again releases it. Mirrored by the FREEZE button on the faceplate and by the FRZ gate input, which toggles the same latch on each rising edge. Freezing before anything has been recorded holds the buffer at digital silence.',
