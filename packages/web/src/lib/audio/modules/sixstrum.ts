...
export const sixstrumDef: AudioModuleDef = {
  ...
  docs: {
    ...
    outputs: {
      out:
        "The mono instrument output — all six strings summed and scaled by 1/√(sounding strings), so a six-string barre sits at the same level as a single note, then through the BODY resonance and LEVEL. Feed a mixer, amp sim or reverb: it is a whole guitar / bass / harp voice on one cable.",
    },
    controls: {
      register:
