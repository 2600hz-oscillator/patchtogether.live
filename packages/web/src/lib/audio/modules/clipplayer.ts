...
export const clipplayerDef: AudioModuleDef = {
  ...
  docs: {
    ...
    controls: {
      ...
      "clipplayer-cell-{n}":
        "A note cell in the piano-roll editor (rows are scale degrees/pitches, columns are steps). Click to toggle a note on or off at that pitch and step; right-click a note for its per-note menu — PROBABILITY (a 0–100% firing chance) and PLAY EVERY (a 1–8 count-divider that plays the note only on every Nth loop of the clip). The two STACK: a note fires only when it's that loop's turn AND it wins its probability roll. The cell colour codes it — white = always fires, a dimmer purple/orange as probability drops, RED (dimmer the higher N) for a play-every note, and the AVERAGE of the two when a note is both probabilistic and play-every. The cells make up the clip you're editing for the selected lane+slot.",
