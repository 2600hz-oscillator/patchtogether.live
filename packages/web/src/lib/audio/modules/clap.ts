...
export const clapDef: AudioModuleDef = {
  ...
  face: {
    ...
    hero: {
      ...
      control: 'spread',
      action: 'strike',
      // THE THREE NUMBERS THE GRAPH IS ABOUT — and ALL THREE are `valueId`,
      // not `paramId`, because not one of them is a knob read back.
      //
      // ⚠ `voice` IS NOT `tail`, and this is the kickdrum-TAIL trap in a
      // different module. A `paramId: 'tail'` readout prints "150 ms" at every
      // SNAP setting; measured on the real core at the defaults, the voice's
      // own −60 dB length is 181 ms at SNAP 0.5, 169 ms at 0.9, 142 ms at 0.99
      // and 50 ms at SNAP 1 — where the room is gone entirely and TAIL still
      // reads 150. SNAP is the negative control, and it is a permanent leg of
      // clap-face-model.test.ts.
      //
      // ⚠ THESE ARE THE ENVELOPE FIGURES, and the difference is stated rather
      // than hidden. Rendered −60 dB length runs ~10–15 ms longer than the
      // formula because of the 20 Hz DC blocker's own residue. Printing the
      // render figure would print a number nobody could reproduce from the
      // knobs.
      readouts: [
        { label: 'burst', valueId: 'clap-burst-ms' },
        { label: 'room at', valueId: 'clap-room-onset-ms' },
    ...
    sidebar: [
      {
        ...
        // def's own header names them). THE THREE NON-DEFAULT ENTRIES ARE THE
        // EXACT SETTINGS e2e/vrt/vrt-clap.spec.ts already pins as composite VRT
        // scenes, so the preset roster and the visual corners cannot drift
        // apart. Each recall stamps all NINE value params through the ordinary
        // commit path — undoable, synced, immediately editable.
        //
        // ⚠ `strike` IS DELIBERATELY NOT IN ANY ENTRY, and that is the one
        // documented exception to kickdrum's "a preset is a FULL recall" rule:
        // it is a momentary PAD, not a setting. Recalling it would stamp a
        // finger position, and `restedParams` would strip it at spawn anyway.
        entries: [
          {
            id: '808',
            label: '808',
            note: '3 × 10 ms',
            values: { pulses: 3, spread: 10, tone: 1000, width: 0.5, tail: 150, color: 0.15, snap: 0.5, drive: 0.2, level: 0 },
          },
          {
            id: '909-dense',
            label: '909 DENSE',
            note: '5 × 5 ms',
            values: { pulses: 5, spread: 5, tone: 2400, width: 0.8, tail: 60, color: 0, snap: 0.85, drive: 0.6, level: 3 },
          },
          {
            id: 'linn-room',
            label: 'LINN ROOM',
            note: '650 ms room',
            values: { pulses: 2, spread: 22, tone: 550, width: 0.3, tail: 650, color: 0.9, snap: 0.2, drive: 0, level: -6 },
          },
          {
            id: 'dry-snap',
            label: 'DRY SNAP',
            note: 'no room',
            values: { pulses: 4, spread: 8, tone: 1400, width: 0.1, tail: 30, color: 0.3, snap: 1, drive: 1, level: 6 },
          },
        ],
      },
      {
        kind: 'readouts',
        label: 'band-pass',
        // PUBLISHING BOTH IS THE INSTRUMENT'S OWN NEGATIVE CONTROL. Q is
        // TONE-INVARIANT (it is a function of WIDTH alone) and BANDWIDTH is
        // not (BW = fc · q), so a derivation that moves both when TONE moves,
        // or neither, is falsified on the spot. Defaults: Q 1.12, BW 890 Hz.
        entries: [
