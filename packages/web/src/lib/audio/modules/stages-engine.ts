// packages/web/src/lib/audio/modules/stages-engine.ts
//
// Pure-math engine for STAGES (Mutable Instruments Stages archetype, MIT-
// licensed). HOST-SIDE COPY of packages/dsp/src/stages-engine.ts — kept
// numerically identical so the host (vitest pass, ART scenarios) and the
// AudioWorklet pass the same math. Following the project convention
// used by peaks / clouds / veils: dsp-side engine for the worklet, web-
// side mirror for hosts that can't import across the worklet bundling
// boundary (the TS path-alias rules and worklet asset pipeline don't
// support importing TS source out of node_modules/@patchtogether.live/dsp/src).
//
// IF YOU EDIT THIS FILE, also edit packages/dsp/src/stages-engine.ts.
