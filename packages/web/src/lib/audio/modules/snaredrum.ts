...
export const snaredrumDef: AudioModuleDef = {
  ...
  docs: {
    ...
    controls: {
      ...
      spread: "ROLL/STEREO: two-hand pan + per-hand detune (0 = mono/centred, 1 = hard L/R hands, each detuned ¾ of a semitone off centre in opposite directions). The left hand pans left, the right pans right, each striking a slightly different spot on the membrane, and the loud wire bed's placement slews (~5 ms) toward whichever hand just struck — a genuine stereo roll image, not a decorrelation trick. It is a ROLL control: a single trigger hit is always dead-centre, so Spread does nothing until the roll gate runs.",
