...
export const cvBuddyDef: AudioModuleDef = {
  ...
  docs: {
    ...
    outputs: {
      ...
      run:
        "A RUN gate driven only by the owner (id-smallest) instance: held HIGH the whole time the rack transport is playing and LOW while it is stopped (it follows play state; it does not pulse). Its ES-9 jack HOLDS +5 V across a stream hiccup: a dropout must never read downstream as a stop-and-restart, because Pam's resets on a RUN edge. Wired to ES-9 jack 7. Patch it to a Pam's RUN/STOP input.",
      clock:
