...
// card owned the only picker; a live defect the moment the DOCK FACE gained one.
//
  ...
  it('reads the saved device id from a REACTIVE position, not only on mount', () => {
    ...
    expect(
      ...
      'no $effect calls readSavedDeviceId() — the card is back to hydrate-once, so a device ' +
        'picked on the DOCK FACE (or by a collaborator) is saved and never acted on until the ' +
        'card remounts',
    ).toBe(true);
  });
