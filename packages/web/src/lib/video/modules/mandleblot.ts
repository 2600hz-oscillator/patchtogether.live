...
export const mandleblotDef: VideoModuleDef = {
  ...
  docs: {
    ...
    outputs: {
      ...
      color_out: "Colour (video) output: the RGB palette pass, full-saturation hue cycled by iteration band (mu), time, and zoom depth; in-set points are forced black to preserve the set's silhouette. This is the surface texture — the faceplate\'s live preview and the default/canonical output handle.",
    },
    controls: {
      zoom: "Zoom (ZOOM knob, log curve, 0..1). Mapped exponentially to a real zoom factor via 10^(6·knob) — 0→1× (whole set), 0.5→~1000×, 0.8→~63k×, 1.0→1e6× (the highp-float ceiling, past which the image goes block-y); default 0.2 (~16×). ⚠ KNOB POSITION IS NOT MAGNIFICATION and the two are not linearly related — half a turn is already ~1000×. The faceplate paints no live ×N magnification readout beneath the knob, because a resting faceplate paints no derived-state text. This sentence and the mapping above it are that finding's home now, together with the value on the control's own aria-valuetext.",
