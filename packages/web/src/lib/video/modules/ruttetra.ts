...
export const ruttetraDef: VideoModuleDef = {
  ...
  docs: {
    ...
    outputs: {
      out: "out (video) - the rendered Rutt/Etra raster: additive horizontal scanlines, luma-displaced into a heightmap, over a black phosphor field. Chainable into any video input and also feeds the faceplate's preview screen.",
    },
    controls: {
      xShape: "X Shape (0..1, default 0) - morphs the horizontal ramp shape that lays the source across each scanline. 0 = linear (1:1, the unwarped raster), ~0.33 = triangle, ~0.66 = soft-fold (raised cosine), 1 = radial (distance from center). The dial is marked at those four anchors and names the nearest one as you sweep it (linear / triangle / soft / radial).",
      ...
      xFreq: "X Freq (0.25..8, default 1) - horizontal ramp frequency: how many shape-ramp cycles span the frame in X. 1 = one pass; higher values repeat/fold the scan pattern across the width. BELOW 1 the ramp never completes, so the picture is COMPRESSED into that fraction of the width and the rest of the frame stays black - at 0.25 the whole raster occupies the left quarter. It sits on the SCAN page beside X Phase.",
      yFreq: "Y Freq (0.25..8, default 1) - vertical ramp frequency: how many shape-ramp cycles span the frame in Y. 1 = one pass; higher values repeat/fold the scanlines down the height. BELOW 1 the scanlines are compressed into that fraction of the height - at 0.25 the whole raster occupies the top quarter. It sits on the SCAN page.",
      xPhase: "X Phase (0..1, default 0) - phase offset added to the horizontal ramp after the frequency multiply and before shaping, sliding the X scan pattern sideways. Panel control only (no CV input); on the SCAN page of the faceplate.",
      yPhase: "Y Phase (0..1, default 0) - phase offset added to the vertical ramp after the frequency multiply and before shaping, sliding the Y scan pattern up/down. Panel control only (no CV input); on the SCAN page of the faceplate.",
    },
  },
