...
export const monoglitchDef: VideoModuleDef = {
  ...
  docs: {
    ...
    inputs: {
      in: "Video source. Its per-pixel luminance (sampled once per scanline at the row center) is what drives every scanline's vertical displacement; with no input the faceplate shows a dark-navy idle sweep. Accepts video/image/keys via the engine's implicit upcasts.",
