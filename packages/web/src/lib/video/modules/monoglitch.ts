 ...
 * in ONE place because TWO surfaces need it: `MonoglitchCard.svelte` (the
 * legacy lane card) and `ui/modules/monoglitch/MonoglitchOutputBody.svelte`
 * (the faced dock body). Both read `node.data.resizedWidth` / `resizedHeight`,
 * so both need the same floors and the same starting size.
 *
 ...
 * other to reach. Same shape and same argument as `RUTTETRA_MONITOR_BOX`, which
 * this follows deliberately rather than inventing a second spelling.
 *
 ...
 * rounded to whole-u 180 px rack tiles), so no rack that was saved before this
 * promotion reopens at a different size than it was left at.
 */
export const MONOGLITCH_MONITOR_BOX = {
