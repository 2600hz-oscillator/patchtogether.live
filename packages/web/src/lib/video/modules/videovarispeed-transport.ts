...
//
// Split out so the math is unit-coverable without a real HTMLVideoElement —
// the card wires the results into <video>.playbackRate (forward) or a
// throttled currentTime scrub (reverse), and the engine module declares the
// matching params. Keeping the rules here means a single source of truth that
// both the card (runtime) and the tests assert against.
//
...
// magnitudes:
//
//   knob 0.0  (7:00, full-left)   -> -4x   (reverse, 4x speed)
//   knob 0.5  (12:00, centre)     -> +1x   (normal forward)
//   knob 1.0  (5:00, full-right)  -> +4x   (forward, 4x speed)
//
...
//
// The slope differs across the centre (10 vs 6) — that asymmetry is
...
// each seek triggers a decode / keyframe seek that stalls the decode pipeline
// AND drops readyState below HAVE_CURRENT_DATA mid-seek, freezing the
// downstream texture. We instead scrub at a THROTTLED cadence (a seek at most
// every REVERSE_SCRUB_INTERVAL_MS) and advance by the full elapsed amount each
// scrub — so reverse is coarser (lower effective fps) but never floods the
// decoder or stalls the main thread, and the downstream output keeps showing
// whatever the last decoded frame is.

/** Minimum wallclock gap between reverse `currentTime` seeks. ~10 fps reverse
 *  scrub: smooth enough to read as reverse motion, sparse enough that the
 *  decoder keeps up and never backs up the main thread. */
export const REVERSE_SCRUB_INTERVAL_MS = 100;

/** Decide whether to perform a reverse scrub this tick, and if so to what
 *  target time. Pure so it's unit-testable. Returns the next currentTime to
 *  seek to (clamped to the window start) plus whether a seek should happen.
 *
 *  `accumulatedMs` is wallclock elapsed since the last scrub; once it crosses
 *  the throttle interval we move back by |speed| * accumulatedSec and reset
 *  the accumulator. This keeps reverse playback rate correct on average
 *  (covers the same ground as a per-frame scrub) while issuing far fewer
 *  seeks. */
export function reverseScrubStep(
  currentSec: number,
  speedAbs: number,
  accumulatedMs: number,
  windowStartSec: number,
): { seek: boolean; toSec: number } {
  if (accumulatedMs < REVERSE_SCRUB_INTERVAL_MS) {
    return { seek: false, toSec: currentSec };
  }
  const back = speedAbs * (accumulatedMs / 1000);
  const toSec = Math.max(windowStartSec, currentSec - back);
  return { seek: true, toSec };
}
