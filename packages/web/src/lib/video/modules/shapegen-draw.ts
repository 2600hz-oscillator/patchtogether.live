...
//   • ring       → wireframe (kept; v2 PR ships front-lit torus)
//   • tetraFrame → wireframe (intrinsically a frame; stays wireframe in solids mode)
//
...
/**
 * Draw one shape in SOLIDS mode. Per-primitive lit rendering for sphere /
 * cube / cylinder / cone. Ring + tetraFrame stay WIREFRAME (ring v2 will
 * ship as a front-lit torus; tetraFrame is inherently a frame).
 */
function drawShapeSolid(
  ...
  switch (shape.type) {
    ...
    // Ring + tetraFrame stay WIREFRAME in solids mode v1 (documented at
    // header) — v2 PR can ship a lit-torus + maintain tetraFrame as a frame.
    case 'ring':       drawShapeWireframe(ctx, shape, x, y, rPx, projZ); return;
    case 'tetraFrame': drawShapeWireframe(ctx, shape, x, y, rPx, projZ); return;
  }
  const _exhaustive: never = shape.type;
  void _exhaustive;
}
...
/** Whether a given primitive type renders with the lit/solid path or
 *  stays wireframe even in SOLIDS mode. Exported for tests. */
export function isLitShapeType(t: FoxyShapeType): boolean {
  return t === 'sphere' || t === 'cube' || t === 'cylinder' || t === 'cone';
}
...
export function drawShapesScene(
  ...
  for (const d of drawables) {
    if (mode === 'solids') drawShapeSolid(ctx, d.shape, d.sx, d.sy, d.rPx, d.projZ);
