...
export const videoOutDef: VideoModuleDef = {
  ...
  docs: {
    ...
    outputs: {
      out: "Pass-through of the input. Each frame the faceplate's draw renders its input texture into its own FBO, and out exposes that FBO's texture — effectively a clean copy of in (input → FBO → out). Wire it onward to chain a monitor into downstream video effects without breaking the signal flow; with no input it carries the static idle pattern.",
    },
    controls: {},
  },
