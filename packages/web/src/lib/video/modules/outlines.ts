...
export const outlinesDef: VideoModuleDef = {
  ...
  docs: {
    ...
    controls: {
      ...
      shape: "SHAPE SELECTOR, 0..1 quantised to six shapes: circle, triangle, square, pentagon, hexagon, octagon. Latched per shape at spawn; the faceplate shows the current shape name.",
      rotation: "ROTATION, a LIVE GLOBAL bipolar spin: center (0.5) = no rotation, left = fast counter-clockwise, right = fast clockwise. Every live shape shares one rotation angle (not latched); the faceplate shows CCW / dot / CW.",
