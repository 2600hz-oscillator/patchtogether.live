...
export const backdraftDef: VideoModuleDef = {
  ...
  docs: {
    ...
    controls: {
      ...
      bezel: "Bezel — labelled BORDER on the card, because 'border thickness' is what people look for (0..1, default 0.4): the width of the screen's frame, in screen-local units, so a level-k bezel automatically lands 3/4-scaled — deeper bezels shrink because they are IMAGES of the real one. The bezel is not decoration: it is the only high-contrast boundary between one nesting level and the next, and without it the nest reads as a smooth zoom rather than as frames within frames. Floored at a non-zero minimum for exactly that reason — a fader whose bottom end deletes the feature would be a bug. Inert while TV MODE is off.",
