...
export const backdraftDef: VideoModuleDef = {
  ...
  noUserControl: [
    ...
    { param: 'freeze',      writer: 'internal', why: 'determinism toggle for VRT capture: at >=0.5 draw() is a no-op so the ring and output hold their last frame. No port targets it and no faceplate control sets it' },
  ],
  ...
  docs: {
    ...
    inputs: {
      ...
      delay_clock: "Gate/clock input (raw passthrough, edge-detected). While a cable is patched here the Delay fader is ignored ENTIRELY: from the moment of patching the effective delay HOLDS its current value (no jump), and once two rising edges land it locks to one clock-pulse duration (capped at 1000ms = one beat at 60 BPM, the same cap as the fader) and tracks the clock from then on. Fader moves while patched change only the stored position; unpatching returns control to the fader at wherever it now sits. The fader shows the override (dimmed, with a CLK badge). A short TRIGGER works exactly as well as a held gate, and a gate cable exactly as well as a CV one (an LFO or envelope patched straight in): the rising edge is counted on the audio thread as the value arrives and replayed by the patch bridge, never sampled once per rendered frame, so a 5ms pulse cannot be missed and the lock does not depend on how fast the renderer is running. The period is measured on the AUDIO clock: each replayed edge carries the audio-thread time it actually rose at, so the lock is sample-accurate however late the main thread gets round to replaying it (a starved page replays edges in bursts; the count and the times survive the burst). Only a write that carries no edge time is stamped when it arrives — well inside the resolution the delay is quantized to anyway (one video frame).",
    ...
    controls: {
      ...
      delayClock: "Delay Clk (0..1, default 0): hidden synthetic param the DELAY CLK CV bridge writes (raw gate swing). No faceplate knob; the module edge-detects it to measure the pulse period that overrides the Delay control.",
      ...
      mirrorXGate: "Mir X Gate (0..1, default 0): hidden synthetic param the mirror_x_gate CV bridge writes (raw gate swing). No faceplate knob; a rising edge flips Mirror X.",
      mirrorYGate: "Mir Y Gate (0..1, default 0): hidden synthetic param the mirror_y_gate CV bridge writes (raw gate swing). No faceplate knob; a rising edge flips Mirror Y.",
      ...
      panicGate: "Panic Gate (0..1, default 0): hidden synthetic param the panic CV bridge writes (raw gate swing). No faceplate knob; a rising edge fires the settings reset the PANIC button also fires.",
    },
  },
