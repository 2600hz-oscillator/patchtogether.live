...
export const colourofmagicDef: VideoModuleDef = {
  ...
  docs: {
    ...
    controls: {
      ...
      over_h: "h wrap: ADVISORY only — hue ALWAYS wraps regardless of this toggle. Declared for UI symmetry with the other channels; disabled on the faceplate.",
      ...
      pal_r: "pal r: the palette colour the RGB red channel maps to under REPLACE (packed 0xRRGGBB, default orange). Set via the faceplate's colour picker (which auto-enables REPLACE).",
      pal_g: "pal g: the palette colour the RGB green channel maps to under REPLACE (packed 0xRRGGBB, default teal). Set via the faceplate's colour picker.",
      pal_b: "pal b: the palette colour the RGB blue channel maps to under REPLACE (packed 0xRRGGBB, default violet). Set via the faceplate's colour picker.",
      preview: "preview: which of the 22 outputs the faceplate's preview canvas shows (discrete 0..21: 0 pass, 1 rgb, 2 ydbdr, 3 hsvhsl, 4 r, 5 g, 6 b, 7 luma, 8 ydb_y, 9 ydb_db, 10 ydb_dr, 11 hsv_h, 12 hsv_s, 13 hsv_v, 14 yiq, 15 yiq_y, 16 yiq_i, 17 yiq_q, 18 ycc, 19 ycc_y, 20 ycc_cb, 21 ycc_cr). Default 1 (rgb). The previewed output is always rendered even when unpatched; other outputs render only when patched downstream.",
      freeze: "freeze: hidden determinism toggle — at 1 the renderer holds its last rendered frame (no redraw) for stable VRT capture. Default 0; no faceplate control.",
    },
  },
