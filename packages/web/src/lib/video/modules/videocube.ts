...
// co-located docs below are hash-transparent by construction, so DOC edits cost
...
export const videocubeDef: VideoModuleDef = {
  ...
  docs: {
    ...
    outputs: {
      ...
      smooth_out: "The slice cross-section (as slice_out, honouring SLICE VIEW) forced through the SMOOTH reader treatment — a trailing frame of each ring — regardless of the faceplate's READER selector. Routed alongside morph_out + chaos_out it lets all three temporal treatments be compared/mixed at once. PER-PORT gated.",
      morph_out: "The slice cross-section forced through the MORPH reader treatment — the newest (crisp) frame of each ring — regardless of the faceplate's READER selector. Differs from smooth_out (which trails) under motion. Honours SLICE VIEW. PER-PORT gated.",
      chaos_out: "The slice cross-section forced through the CHAOS reader treatment — a per-pixel dithered frame across each ring's 60-frame window — regardless of the faceplate\'s READER selector. The noisiest of the triptych; differs from smooth_out/morph_out under motion. Honours SLICE VIEW. PER-PORT gated.",
    },
    controls: {
