...
describe('video — VideoEngine res injection (HD toggle)', () => {
  // The HD toggle rebuilds the engine at a viewport-derived res. The
  // constructor takes an optional {res}; default is VIDEO_RES (byte-for-byte
  // today). We use the same fake-canvas trick as the freeze test — the
  // constructor only reads res for OffscreenCanvas sizing (skipped when a canvas
  // is supplied) + stores it, no GL needed.
  function makeEngine(res?: { width: number; height: number }): VideoEngine {
    const glStub = {} as unknown as WebGL2RenderingContext;
    const canvas = {
      width: 1,
      height: 1,
    ...
    return new VideoEngine(res ? { canvas, res } : { canvas });
  }

  it('defaults res to VIDEO_RES (640×480) when no res is supplied', () => {
    const engine = makeEngine();
    expect(engine.res.width).toBe(640);
    expect(engine.res.height).toBe(480);
    engine.dispose();
  });

  it('honors an injected HD res (1920×1080)', () => {
    const engine = makeEngine({ width: 1920, height: 1080 });
    expect(engine.res.width).toBe(1920);
    expect(engine.res.height).toBe(1080);
    engine.dispose();
  });

  it('honors a 4:3 HD res (1440×1080)', () => {
    const engine = makeEngine({ width: 1440, height: 1080 });
    expect(engine.res).toEqual({ width: 1440, height: 1080 });
    engine.dispose();
  });

  it('falls back to VIDEO_RES on a degenerate (0-dimension) injected res', () => {
    const engine = makeEngine({ width: 0, height: 0 });
    expect(engine.res.width).toBe(640);
    expect(engine.res.height).toBe(480);
    engine.dispose();
  });

  it('context() hands the injected res to module factories', () => {
    const engine = makeEngine({ width: 1920, height: 1080 });
    // The context is private, but addNode passes it to the factory. Drive a
    // stub factory that captures ctx.res.
    let capturedRes: { width: number; height: number } | null = null;
    const stubType = ('res-spy-' + Math.random().toString(36).slice(2)) as
      ModuleNode['type'];
    registerVideoModule({
      type: stubType,
      domain: 'video',
      ...
      params: [],
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      factory: ((ctx: { res: { width: number; height: number } }) => {
        capturedRes = { width: ctx.res.width, height: ctx.res.height };
        return {
          domain: 'video',
          surface: { fbo: null, texture: null, draw: () => {}, dispose: () => {} },
          setParam: () => {},
    ...
    expect(capturedRes).toEqual({ width: 1920, height: 1080 });
    engine.dispose();
  });
});
