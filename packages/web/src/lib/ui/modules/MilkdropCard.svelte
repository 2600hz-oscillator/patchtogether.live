<script lang="ts">
  ...
  const inputs = portsFromDef(milkdropDef.inputs, {
    audio: 'AUD', bass: 'BAS', treb: 'TRB', reactivity: 'RCT', speed: 'SPD',
    presetSelect: 'PST', morph: 'MPH', next: 'NXT',
  });
  const outputs = portsFromDef(milkdropDef.outputs);
