<script lang="ts">
  // packages/web/src/lib/ui/modules/shapes/ShapesOutputBody.svelte
  //
  // The SHAPES dock full-view body: the live stamp picture plus the SCREEN
  ...
  // affordance) is the #1928 defect this slot exists to prevent. `ShapesCard.svelte`
  // has no preview canvas and no `previewCollapsed` key at all; it is a fader row
  // plus two buttons. So nothing is lost by promotion, and this face GAINS a control its card
  ...
  // node stops being pulled. SHAPES HAS NONE — no time uniform, no ping-pong,
  // no accumulator, no RNG: the stamp is a pure per-pixel SDF evaluation of
  // (shape, zoom, rotate, tile, tileN), so it would resume instantly and
  // identically. ⚠ Do NOT copy `peakstate`'s or `vdelay`'s comment here; there
  // is no history to lose.
  //
  // What the mark protects is the OUTPUT, and on a SOURCE that is the sharpest
  // version of the argument: SHAPES has no input at all, so it is the ORIGIN of
  // whatever is downstream. A lapsed mark would not stall a preview, it would
  // MUTE the generator every consumer is sampling — a control labelled SCREEN
  // behaving as a mute on the whole chain.
  function draw() {
    rafId = null;
...
<div class="shapes-output" data-testid="shapes-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="shapes-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="shapes-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the stamp preview is collapsed and its space reclaimed. SHAPES keeps generating and keeps feeding OUT: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the stamp preview off to collapse it and reclaim the vertical space. SHAPES goes on generating and feeding OUT either way.'}
