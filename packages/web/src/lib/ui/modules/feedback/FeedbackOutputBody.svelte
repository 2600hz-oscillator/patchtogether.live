<script lang="ts">
  // packages/web/src/lib/ui/modules/feedback/FeedbackOutputBody.svelte
  //
  // The FEEDBACK dock full-view body: the live feedback loop picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // The mechanics below are `ShapesOutputBody`'s deliberately: same node-data
  // key, same overlay geometry, same watch-mark handling. Copying them is the
  // point — a second spelling of `previewCollapsed` is how these fork. What is
  // NOT copied is the argument for the watch mark, which is per-module and
  // written out below.
  //
  ...
  // RENDERING.
  //
  // ⚠ THIS IS THE ACCUMULATOR CASE, AND IT IS THE STRONGEST ARGUMENT IN THE
  // BATCH — do NOT give this module the "it would resume identically" comment its
  // three batch-mates carry. FEEDBACK IS a recursive accumulator by definition:
  // its own docs describe reading the PREVIOUS frame from a ping-pong
  // framebuffer, warping it, scaling it by Decay and adding the fresh input.
  //
  // So the pull is not merely how the picture stays fresh — it is how the trail
  // EXISTS. A lapsed mark stops the node being pulled, the ping-pong stops
  // advancing, and the accumulated image the player spent the last several
  // seconds building decays out of the patch. Switching SCREEN back on would
  // show a loop that had lost its history, which is the `vdelay` / `bentbox`
  // class of loss and exactly what the ruling ("the module KEEPS RENDERING")
  // forbids.
  function draw() {
    rafId = null;
...
<div class="feedback-output" data-testid="feedback-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="feedback-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="feedback-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the feedback picture is collapsed and its space reclaimed. FEEDBACK keeps looping and keeps feeding OUT: the trail goes on building, so switching it back on shows the LIVE loop rather than the frame you left.'
        : 'SCREEN — turn the feedback picture off to collapse it and reclaim the vertical space. FEEDBACK goes on looping and accumulating either way.'}
