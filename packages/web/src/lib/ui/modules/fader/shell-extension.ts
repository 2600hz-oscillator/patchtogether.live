// packages/web/src/lib/ui/modules/fader/shell-extension.ts
//
// The FADER SHELL EXTENSION — the module-owned end of the extension seam
// (#1512), on the `fullViewBody` slot.
//
// `faderDef.face.extension: 'fader'` declares this file — the id IS this
// directory's name, and the non-eager glob in
// `$lib/ui/workflow/shell-extensions.ts` is the one resolver. ModuleShell loads
// it lazily and never imports a fader component itself, which keeps
// `module-shell-import-guard` green.
//
// ⚠ WHY (#1928): promotion stops BOTH surfaces rendering `FaderCard.svelte`, so a
...
import FaderOutputBody from './FaderOutputBody.svelte';

export default {
  fullViewBody: FaderOutputBody,
