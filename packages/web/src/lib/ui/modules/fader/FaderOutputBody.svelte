<script lang="ts">
  // packages/web/src/lib/ui/modules/fader/FaderOutputBody.svelte
  //
  // The FADER dock full-view body: its live mix picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `FaderCard.svelte` owns the only
  ...
  //
  // ⚠ IT MATTERS MORE HERE THAN ON A SINGLE-OUTPUT MODULE, and that is worth
  // stating. FADER has TWO outputs: `out` (the main mix, which is what this
  // preview shows) and `send` (a copy of the A/B mix feeding an external FX
  // loop that returns through `return`). Both are produced by the same two GPU
  // passes every frame.
  //
  // So dropping the watch mark here would stall an output THE SWITCH DOES NOT
  // EVEN SHOW — and worse, `send` is typically the head of a loop that comes
  // back into this same node, so stalling it can quietly empty the wet path the
  // player is mixing against. There is no accumulator to lose (each frame is a
  // fresh dry/wet blend), which makes this purely an OUTPUT argument — but it
  // is the batch's strongest one, because a second output is invisible from the
  // face.
  function draw() {
    rafId = null;
...
<div class="fader-output" data-testid="fader-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    ...
    <button
      ...
      data-testid="fader-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the mix preview is collapsed and its space reclaimed. FADER keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the mix preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
