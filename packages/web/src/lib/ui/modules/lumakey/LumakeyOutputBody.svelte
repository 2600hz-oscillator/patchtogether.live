<script lang="ts">
  // packages/web/src/lib/ui/modules/lumakey/LumakeyOutputBody.svelte
  //
  // The LUMAKEY dock full-view body: its live key picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `LumakeyCard.svelte` owns the only
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. `grainsOfVision` and
  // `bentbox` argue from ACCUMULATED STATE that empties if the node stops being
  // pulled; LUMAKEY HAS NONE — no time uniform, no ping-pong, no history — so
  // the key is a pure per-pixel function of (input, threshold, softness,
  // invert) and would resume instantly and correctly.
  //
  // What the mark protects is the OUTPUT, and on a KEYER that is the pointed
  // case: this module exists to be composited downstream. Its `out` is the
  // matte/keyed picture something else consumes, so a rack where this face is
  // the only observer and `out` feeds a mixer or a not-yet-opened downstream
  // still gets frames. A SCREEN switch that quietly stopped the key would
  // change what the DOWNSTREAM sees, which is not what the control says it
  // does.
  function draw() {
    rafId = null;
...
<div class="lumakey-output" data-testid="lumakey-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="lumakey-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="lumakey-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the key preview is collapsed and its space reclaimed. LUMAKEY keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the key preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
