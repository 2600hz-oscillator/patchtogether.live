<script lang="ts">
  // packages/web/src/lib/ui/modules/posterbox/PosterboxOutputBody.svelte
  //
  // The POSTERBOX dock full-view body: its live quantised picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `PosterboxCard.svelte` owns the only
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. POSTERBOX is a
  // stateless quantiser: each pixel's colour is reduced to the declared bit
  // depth, optionally dithered, and blended back by MIX — a pure per-pixel
  // function of (input, depth, dither, mix) with no time uniform, no ping-pong
  // and no history. It would resume instantly and identically.
  //
  // What the mark protects is the OUTPUT. This is a chainable mid-graph effect
  // whose `out` is what something downstream is looking at, so a rack where
  // this faceplate is the only observer still gets frames with the screen off.
  function draw() {
    rafId = null;
...
<div class="posterbox-output" data-testid="posterbox-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="posterbox-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="posterbox-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the quantised preview is collapsed and its space reclaimed. POSTERBOX keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the quantised preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
