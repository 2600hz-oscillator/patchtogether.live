<script lang="ts">
  // packages/web/src/lib/ui/modules/destructor/DestructorOutputBody.svelte
  //
  // The DESTRUCTOR dock full-view body: its live mangled picture plus the
  // SCREEN ON/OFF switch the 2026-08-18 owner ruling requires of every video
  // module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `DestructorCard.svelte` has NEVER
  ...
  // requires. The picture is what makes the four faders legible: every one of
  // them is a DEGRADATION AMOUNT whose result is only describable as a look
  // (channel smear, scanline depth, banding), so "how much decay is this?" is
  // not answerable from four dials. Recorded here so nobody later "restores
  // parity" by deleting either.
  //
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. `destructor.ts`'s
  // `FRAG_SRC` declares no time uniform, no ping-pong and no history: its
  // scanline grid is a function of `vUv.y` alone, NOT of a clock, so the glitch
  // is a pure per-pixel function of (input, shift, scanline, posterize, mangle)
  // and would resume instantly and correctly. Do not copy `vdelay`'s or
  // `milkdrop`'s comment, which argue from state that empties.
  //
  // What the mark protects is the OUTPUT, and DESTRUCTOR is a chain effect that
  // exists to be looked at THROUGH something else — its `out` typically runs on
  // into a mixer or the rack video output. So a rack where this face is the
  // only observer still gets frames. A SCREEN switch that quietly stopped the
  // mangle would change what the DOWNSTREAM sees, which is not what the control
  // says it does.
  function draw() {
    rafId = null;
...
<div class="destructor-output" data-testid="destructor-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="destructor-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="destructor-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the mangled preview is collapsed and its space reclaimed. DESTRUCTOR keeps rendering: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the mangled preview off to collapse it and reclaim the vertical space. The glitch goes on rendering downstream either way.'}
