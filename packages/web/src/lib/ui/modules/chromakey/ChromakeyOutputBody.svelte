<script lang="ts">
  // packages/web/src/lib/ui/modules/chromakey/ChromakeyOutputBody.svelte
  //
  // The CHROMAKEY dock full-view body: the live composite picture plus the SCREEN
  ...
  // RENDERING.
  //
  // ⚠ THE REASON HERE IS THE PULL, AND IT IS THE SHARPEST VERSION OF IT IN THIS
  // BATCH. CHROMAKEY holds no history at all — a per-pixel key decision over its
  // `fg` and `bg` textures, no ping-pong, no ring, no time uniform — so nothing
  // is lost by pausing and `feedback`'s accumulator comment must not be copied
  // here.
  //
  // But it is the pull root for TWO upstream chains rather than one. A lapsed
  // mark idles whatever feeds `fg` AND whatever feeds `bg`, so the blast radius
  // of getting this wrong is both branches of the composite, not one.
  function draw() {
    rafId = null;
...
<div class="chromakey-output" data-testid="chromakey-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="chromakey-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="chromakey-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the composite is collapsed and its space reclaimed. CHROMAKEY keeps compositing and keeps feeding OUT: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the composite off to collapse it and reclaim the vertical space. CHROMAKEY goes on compositing and feeding OUT either way.'}
