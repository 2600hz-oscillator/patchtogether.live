<script lang="ts">
  // packages/web/src/lib/ui/modules/onetonine/OneToNineOutputBody.svelte
  //
  // The ONETONINE dock full-view body: its live monitor picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `OneToNineCard.svelte` owns the only
  // live preview this module has ever had, and promotion is exactly what stops
  // that card rendering: `migrated(type)` becomes true and
  // `DockFullView.svelte` mounts `<ModuleShell>` instead. On a VIDEO ROUTER
  // that loss is worse than on most modules — the four selectors are
  // bit-identically symmetric and their only observable difference is WHAT COMES
  // OUT, so a faceplate with no picture asks the player to route blind.
  //
  // The route is the `fullViewBody` shell-extension slot, and the shape below is
  // `FreezeframeOutputBody`'s deliberately: same key, same overlay geometry,
  // same watch-mark handling. Copying it is the point — a second spelling of
  // this control is how `previewCollapsed` would fork.
  //
  ...
  // would silently re-open every preview collapsed before the promotion.
  // Absent ⇒ false ⇒ ON.
  ...
  //
  // ⚠ THIS IS THE SHARPEST CASE IN THE BATCH, because the picture the switch
  // hides is NOT the module's product. ONETONINE has NINE outputs (out1..out9,
  // one clean crop per grid cell) and a SEPARATE monitor surface — the grid +
  // digits overlay that exists purely so an operator can see which cell feeds
  // which output. What this preview shows is the MONITOR. What downstream
  // consumers pull are the nine crops.
  //
  // So dropping the watch mark would stall NINE outputs the switch does not
  // show, in order to hide one diagnostic surface — a control labelled SCREEN
  // silently unpatching a nine-way splitter. That is the #2015 defect at its
  // most severe, and the reason the collapsed branch still calls `markWatched`.
  //
  // There is no accumulator to lose (each crop is a fresh sample of the input
  // sub-rect), so this is purely an OUTPUT argument — but the arity is what
  // makes it bite.
  function draw() {
    rafId = null;
...
<div class="onetonine-output" data-testid="onetonine-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="onetonine-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="onetonine-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the monitor preview is collapsed and its space reclaimed. ONETONINE keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the monitor preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
