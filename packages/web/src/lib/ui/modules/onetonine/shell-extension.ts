// packages/web/src/lib/ui/modules/onetonine/shell-extension.ts
//
// The ONETONINE SHELL EXTENSION — the module-owned end of the extension seam
// (#1512), on the `fullViewBody` slot.
//
// `onetonineDef.face.extension: 'onetonine'` declares this file — the id IS this
// directory's name, and the non-eager glob in
// `$lib/ui/workflow/shell-extensions.ts` is the one resolver. ModuleShell loads
// it lazily and never imports a onetonine component itself, which keeps
// `module-shell-import-guard` green.
//
// ⚠ WHY (#1928): promotion stops BOTH surfaces rendering `OneToNineCard.svelte`, so a
...
import OneToNineOutputBody from './OneToNineOutputBody.svelte';

export default {
  fullViewBody: OneToNineOutputBody,
