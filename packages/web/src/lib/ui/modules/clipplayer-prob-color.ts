 ...
 *   - a deeper PURPLE (`hsl(280 …)`) as a PER-NOTE override's probability drops;
 *   - a deeper ORANGE (`hsl(30 …)`) as the CLIP DEFAULT drops (no per-note key).
...
export function noteProbCellFill(clip: NoteClipRecord, step: number, midi: number): string {
  ...
  // SOURCE-AWARE hue: orange for a clip-default note, purple for a per-note
  // override (when p < 1 the source is never 'none', so this is exhaustive).
  return probColorBucket(clip, ev) === 'orange'
    ? `hsl(30 90% ${l}%)` // clip default → orange ramp
    : `hsl(280 72% ${l}%)`; // per-note override → purple ramp
}
