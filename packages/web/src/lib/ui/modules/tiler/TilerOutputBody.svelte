<script lang="ts">
  // packages/web/src/lib/ui/modules/tiler/TilerOutputBody.svelte
  //
  // The TILER dock full-view body: its live tiled picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `TilerCard.svelte` owns the only
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. TILER re-samples the
  // input into a grid every frame — a pure function of (input, tile) with no
  // clock, no ping-pong and no history — so a stalled pull costs it nothing to
  // rebuild.
  //
  // What the mark protects is the OUTPUT: this is a chainable effect whose
  // `out` feeds whatever is downstream, and SCREEN is a control about the
  // PREVIEW, not about whether the module runs.
  function draw() {
    rafId = null;
...
<div class="tiler-output" data-testid="tiler-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    ...
    <button
      ...
      data-testid="tiler-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the tiled preview is collapsed and its space reclaimed. TILER keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the tiled preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
