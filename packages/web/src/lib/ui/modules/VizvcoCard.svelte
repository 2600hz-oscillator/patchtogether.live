<script lang="ts">
  // VizvcoCard — VIZVCO is the analog-VCO sister with a built-in
  // wavefolder + a mono-video scope output. Same control surface as
  // AnalogVcoCard plus a Fold knob and a `scope` source handle.
  import { Handle, Position, type NodeProps } from '@xyflow/svelte';
  import Fader from '$lib/ui/controls/Fader.svelte';
  import { patch } from '$lib/graph/store';
  import { vizvcoDef } from '$lib/audio/modules/vizvco';
  ...
  let tune = $derived(node?.params.tune ?? vizvcoDef.params[0]!.defaultValue);
  let fine = $derived(node?.params.fine ?? vizvcoDef.params[1]!.defaultValue);
  let fmAmount = $derived(node?.params.fmAmount ?? vizvcoDef.params[2]!.defaultValue);
  let pw = $derived(node?.params.pw ?? vizvcoDef.params[3]!.defaultValue);
  let foldAmount = $derived(node?.params.foldAmount ?? vizvcoDef.params[4]!.defaultValue);
...
<div class="card">
  ...
  <header class="title">VIZVCO</header>
  ...
  <Handle type="target" position={Position.Left} id="foldAmount" style="top: 128px; --handle-color: var(--cable-cv);" />
  <span class="port-label left" style="top: 50px;">pitch</span>
  <span class="port-label left" style="top: 86px;">fm</span>
  <span class="port-label left" style="top: 122px;">fold</span>
  ...
  <Handle type="source" position={Position.Right} id="scope"    style="top: 200px; --handle-color: var(--cable-mono-video);" />
  ...
  <span class="port-label right" style="top: 194px;">scope</span>
  ...
  <div class="fader-row">
    ...
    <Fader value={foldAmount} min={0}    max={1}    defaultValue={0}   label="Fold"            curve="linear" onchange={setParam('foldAmount')} readLive={readLive('foldAmount')} />
  </div>
</div>
