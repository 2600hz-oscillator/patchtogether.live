<script lang="ts">
  ...
  // affordance) is the #1928 defect this slot exists to prevent. `ColorizerCard.svelte`
  ...
  // node stops being pulled. COLORIZER HAS NONE: there is no time uniform, no
  // ping-pong and no accumulator in its fragment source — the tint is a pure
  // per-pixel function of (input texture, tintR, tintG, tintB), so it would
  // resume instantly and correctly. What the mark protects is the OUTPUT, and
  // this module is the clearest case of it in the batch: COLORIZER is the
  // MONO→COLOUR adapter, taking `mono-video` in and emitting full `video` out,
  // so it sits in the MIDDLE of a chain by construction rather than at its end.
  // Its typical upstream is EDGES — the module directly beside it in this same
  // batch, whose `out` is exactly the `mono-video` this one consumes. A rack
  // where this face is the only observer and `out` runs into an OUTPUT node
  // still gets frames.
  function draw() {
    rafId = null;
...
<div class="colorizer-output" data-testid="colorizer-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="colorizer-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="colorizer-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the tint preview is collapsed and its space reclaimed. COLORIZER keeps rendering and keeps feeding OUT: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the tint preview off to collapse it and reclaim the vertical space. COLORIZER goes on rendering and feeding OUT either way.'}
