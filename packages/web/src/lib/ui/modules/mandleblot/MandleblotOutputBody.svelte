<script lang="ts">
  // packages/web/src/lib/ui/modules/mandleblot/MandleblotOutputBody.svelte
  //
  // The MANDLEBLOT dock full-view body: the live fractal picture plus the SCREEN
  ...
  // RENDERING.
  //
  // ⚠ MANDLEBLOT IS TIME-ANIMATED, which separates it from its three batch-mates
  // and is worth stating because the obvious reading of "a fractal is a still
  // picture" is wrong here. Its fragment shader takes a `uTime` uniform and adds
  // `uTime * 0.1 * uColorCycle` into the hue, and `color_cycle` ships at 1 — so
  // at the shipped defaults the palette CYCLES CONTINUOUSLY at rest.
  //
  // It still holds no accumulator: every frame is computed from scratch out of
  // (centre, zoom, iterations, rotation, colour, time), so pausing loses no
  // history and the `feedback` comment must not be copied here.
  //
  // What the mark protects is the pull. MANDLEBLOT is a SOURCE — its only input
  // is a CV — so it is the ORIGIN of whatever is downstream, and a lapsed mark
  // would not stall a preview, it would MUTE the generator every consumer is
  // sampling.
  function draw() {
    rafId = null;
...
<div class="mandleblot-output" data-testid="mandleblot-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="mandleblot-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="mandleblot-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the fractal is collapsed and its space reclaimed. MANDLEBLOT keeps rendering and keeps feeding both outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the fractal off to collapse it and reclaim the vertical space. MANDLEBLOT goes on rendering and feeding both outputs either way.'}
