...
<div class="mod-card drummergirl-card">
  ...
  <PatchPanel nodeId={id} {inputs} {outputs}>
    <div class="fader-row">
      <Fader value={pitch}  min={-36}   max={36}  defaultValue={0}    label="Pitch"  units="st" curve="linear" onchange={set('pitch')}  readLive={live('pitch')} />
      <Fader value={tone}   min={0}     max={1}   defaultValue={0.3}  label="Tone"              curve="linear" onchange={set('tone')}   readLive={live('tone')} />
      <Fader value={shape}  min={0}     max={1}   defaultValue={0.3}  label="Shape"             curve="linear" onchange={set('shape')}  readLive={live('shape')} />
      <Fader value={decay}  min={0.001} max={0.5} defaultValue={0.15} label="Decay"  units="s"  curve="log"    onchange={set('decay')}  readLive={live('decay')} />
      <Fader value={volume} min={0}     max={2.0} defaultValue={1.0}  label="Volume"            curve="linear" onchange={set('volume')} readLive={live('volume')} />
    </div>
  </PatchPanel>
</div>
