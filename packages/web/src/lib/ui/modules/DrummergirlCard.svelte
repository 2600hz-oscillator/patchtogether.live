...
<div class="mod-card drummergirl-card">
  ...
  <PatchPanel nodeId={id} {inputs} {outputs}>
    <div class="fader-row">
      <NeonFader value={pitch}  min={-36}   max={36}  defaultValue={0}    label="Pitch"  units="st" curve="linear" onchange={set('pitch')} moduleId={id} paramId="pitch"  readLive={live('pitch')} />
      <NeonFader value={tone}   min={0}     max={1}   defaultValue={0.3}  label="Tone"              curve="linear" onchange={set('tone')} moduleId={id} paramId="tone"   readLive={live('tone')} />
      <NeonFader value={shape}  min={0}     max={1}   defaultValue={0.3}  label="Shape"             curve="linear" onchange={set('shape')} moduleId={id} paramId="shape"  readLive={live('shape')} />
      <NeonFader value={decay}  min={0.001} max={0.5} defaultValue={0.15} label="Decay"  units="s"  curve="log"    onchange={set('decay')} moduleId={id} paramId="decay"  readLive={live('decay')} />
      <NeonFader value={volume} min={0}     max={2.0} defaultValue={1.0}  label="Volume"            curve="linear" onchange={set('volume')} moduleId={id} paramId="volume" readLive={live('volume')} />
    </div>
  </PatchPanel>
</div>
