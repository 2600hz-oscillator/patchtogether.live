// packages/web/src/lib/ui/modules/sourcery/shell-extension.ts
//
// The SOURCERY SHELL EXTENSION — the module-owned end of the extension seam
// (#1512), on the `fullViewBody` slot.
//
// `sourceryDef.face.extension: 'sourcery'` declares this file — the id IS this
// directory's name, and the non-eager glob in
// `$lib/ui/workflow/shell-extensions.ts` is the one resolver. ModuleShell loads
// it lazily and never imports a sourcery component itself, which keeps
// `module-shell-import-guard` green.
//
// ⚠ WHY (#1928): promotion stops BOTH surfaces rendering `SourceryCard.svelte`, so a
...
import SourceryOutputBody from './SourceryOutputBody.svelte';

export default {
  fullViewBody: SourceryOutputBody,
