<script lang="ts">
  // packages/web/src/lib/ui/modules/sourcery/SourceryOutputBody.svelte
  //
  // The SOURCERY dock full-view body: its live keyed picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `SourceryCard.svelte` owns the only
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. SOURCERY derives its
  // picture per frame from its two thresholds, the colour skew and the rotation
  // — no time uniform, no ping-pong, no history — so it resumes exactly where
  // an unbroken loop would have been.
  //
  // What the mark protects is the OUTPUT: a rack where this faceplate is the
  // only observer, with `out` running into a mixer or a not-yet-opened
  // downstream, still gets frames while the screen is off.
  function draw() {
    rafId = null;
...
<div class="sourcery-output" data-testid="sourcery-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="sourcery-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="sourcery-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the keyed preview is collapsed and its space reclaimed. SOURCERY keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the keyed preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
