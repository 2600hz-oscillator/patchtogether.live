<script lang="ts">
  // packages/web/src/lib/ui/modules/cellshade/CellshadeOutputBody.svelte
  //
  // The CELLSHADE dock full-view body: its live toon picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `CellshadeCard.svelte` owns the only
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR — and this def says so
  // itself. Its three stages (bilateral smoothing, luma quantisation, Sobel
  // ink) all run per frame from the live input, and the docs state the module
  // "is stateless per frame, so the look tracks the live source with no
  // feedback". No time uniform, no ping-pong, no history: it would resume
  // instantly and identically.
  //
  // What the mark protects is the OUTPUT. CELLSHADE is a chainable `effects`
  // module whose `out` is the toon picture something downstream consumes, so a
  // rack where this faceplate is the only observer still gets frames with the
  // screen off. A control labelled SCREEN must not become a MUTE.
  function draw() {
    rafId = null;
...
<div class="cellshade-output" data-testid="cellshade-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="cellshade-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="cellshade-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the toon preview is collapsed and its space reclaimed. CELLSHADE keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the toon preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
