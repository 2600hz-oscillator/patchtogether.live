// packages/web/src/lib/ui/modules/inwards/shell-extension.ts
//
// The INWARDS SHELL EXTENSION — the module-owned end of the extension seam
// (#1512), on the `fullViewBody` slot.
//
// `inwardsDef.face.extension: 'inwards'` declares this file — the id IS this
// directory's name, and the non-eager glob in
// `$lib/ui/workflow/shell-extensions.ts` is the one resolver. ModuleShell loads
// it lazily and never imports a inwards component itself, which is what keeps
...
//
// INWARDS is the only SOURCE in this batch — no video input at all. Its
// picture animates off `frame.time`, which is why its VRT scene pins
// `__videoEngineFreezeTime` instead of declaring a `freeze` ParamDef.
//
...
import InwardsOutputBody from './InwardsOutputBody.svelte';

export default {
  fullViewBody: InwardsOutputBody,
