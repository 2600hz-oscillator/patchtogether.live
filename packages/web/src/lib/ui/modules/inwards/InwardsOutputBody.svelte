<script lang="ts">
  // packages/web/src/lib/ui/modules/inwards/InwardsOutputBody.svelte
  //
  // The INWARDS dock full-view body: the live ring picture plus the SCREEN
  ...
  // affordance) is the #1928 defect this slot exists to prevent. `InwardsCard.svelte`
  ...
  // RENDERING.
  //
  // ⚠ INWARDS IS A SOURCE, WHICH CHANGES WHICH HALF OF THE ARGUMENT APPLIES.
  // The three siblings in this batch are effects with an input; this one has NO
  // video input at all — it synthesises its rings from time and three params, so
  // "the module keeps rendering" is the whole of its contract rather than a
  // pass-through detail. If the watch mark lapsed, a rack whose only INWARDS
  // observer is this faceplate would stop producing the backdrop/mask every
  // downstream node is sampling, and the switch labelled SCREEN would have
  // silently become a MUTE.
  //
  // ⚠ IT DOES ANIMATE, AND IT STILL NEEDS NO ACCUMULATOR ARGUMENT. Its
  // fragment source reads `uTime` (`phase = r * uDensity - uTime * uSpeed`), so
  // unlike EDGES / COLORIZER the picture genuinely moves at rest — Speed
  // defaults to 0.5. But `frame.time` is a WALL CLOCK
  // (`performance.now() - startTime`), not a per-draw counter, so skipped draws
  // cost no phase: the render is a pure function of (wall clock, params) and
  // resumes exactly where an unbroken loop would have been. That is precisely
  // why this module's VRT scene pins `__videoEngineFreezeTime` rather than
  // needing a `freeze` ParamDef — see its roster entry in `_shell-faces.ts`.
  // Nothing to rebuild on resume; what the mark protects is the OUTPUT.
  function draw() {
    rafId = null;
...
<div class="inwards-output" data-testid="inwards-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="inwards-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="inwards-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the ring preview is collapsed and its space reclaimed. INWARDS keeps rendering and keeps feeding OUT: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the ring preview off to collapse it and reclaim the vertical space. INWARDS goes on rendering and feeding OUT either way.'}
