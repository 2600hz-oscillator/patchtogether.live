<script lang="ts">
  ...
  // module.
  //
  // The mechanics below are `ChromaOutputBody`'s deliberately: same node-data
  ...
  //
  // ⚠ AND ON THIS MODULE THE LOSS WOULD NOT BE RECOVERABLE BY RESUMING, which
  // separates it from `chroma` (a stateless per-pixel grade that resumes
  // identically) and puts it in `vdelay`'s class instead — one step worse.
  // FRAMETABLE'S SUBJECT IS AN ACCUMULATOR: a 60-layer ring the draw step
  // advances exactly once per frame, so every frame the node is not pulled is a
  // frame that never enters the history. `frametableDef.pullExempt` exists for
  // precisely this reason and says so in its own comment — *"a gap from a
  // paused-while-unwatched period would be a visible seam the instant you MORPH
  // back through it"*. Turning the SCREEN off must not punch that gap; keeping
  // the mark alive is what stops it.
  //
  // The upstream half matters too: FRAMETABLE sits MID-CHAIN on one video input,
  // so a lapsed mark would stop the whole chain feeding `video_in` from being
  // pulled — a control labelled SCREEN acting as a mute on everything behind it.
  function draw() {
    rafId = null;
...
<div class="frametable-output" data-testid="frametable-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="frametable-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="frametable-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the scanned picture is collapsed and its space reclaimed. FRAMETABLE goes on recording every input frame into the 60-frame ring, so switching it back on shows the LIVE scan with no gap in the history.'
        : 'SCREEN — turn the scanned picture off to collapse it and reclaim the vertical space. The 60-frame ring keeps filling and OUT keeps feeding either way.'}
