<script lang="ts">
  // packages/web/src/lib/ui/modules/mirrorpool/MirrorpoolOutputBody.svelte
  //
  // The MIRRORPOOL dock full-view body: its live picture plus the SCREEN
  ...
  // `migrated('mirrorpool')` true, and neither surface renders
  // `MirrorpoolCard.svelte` after that — so a toggle authored only on the card
  // is deleted by the promotion meant to keep it. That is what `spirographs`
  // shipped and #1930 repaired. There is no generic shell affordance to fall
  // back on (`previewCollapsed` appears in zero shell files), so it arrives
  // through the `fullViewBody` extension slot, the route `backdraft`,
  // `videoOut` and `spirographs` already take.
  //
  ...
  // ⚠ SCREEN OFF STOPS THE COPY, AND KEEPS THE WATCH MARK. This is the half of
  // the ruling that is easy to lose, and it is MEASURED rather than assumed:
  //
  //   * `blitOutputForPreview` IS the "someone is watching" signal — it calls
  //     `markWatched` itself (`video/engine.ts:1632`), and its own header says
  //     "because the blit IS the watch mark, no markWatched" on that path.
  //   * A node is a pull root only while its mark is younger than
  //     `WATCH_TTL_MS = 1500` (`engine.ts:674, 1100`).
  //
  // So a SCREEN-OFF state that simply stops calling the blit — whether by
  // halting the rAF loop or by keeping the loop and skipping the blit — stops
  // marking the node watched, and 1.5 s later it drops out of the pull set.
  // The toggle would then be a PRODUCER KILL SWITCH on any rack where nothing
  // downstream is watching, and switching back ON would show a stale frame
  // while it spun up: exactly the #1720/#1721 class the owner ruling names when
  // it says the module KEEPS RENDERING.
  //
  // Marking directly is the supported route — `markWatched` is public
  // precisely "so bespoke presentation paths (and tests) can mark directly"
  // (`engine.ts:994-997`). It is one `Map.set` per frame against a blit plus a
  // downscale, so SCREEN OFF still reclaims essentially all of the cost, which
  // is what the switch is for. A hard `renderLease` is deliberately NOT used:
  // the engine reserves that for surfaces that outlive their card's viewport,
  // and a soft mark is what lets this decay naturally once the dock closes and
...
<div class="mp-output" data-testid="mirrorpool-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="mirrorpool-face-canvas"
      ></canvas>
    {/if}
    <button
      type="button"
      ...
      data-testid="mirrorpool-face-screen-toggle"
