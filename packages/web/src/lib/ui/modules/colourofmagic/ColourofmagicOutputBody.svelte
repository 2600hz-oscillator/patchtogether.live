<script lang="ts">
  ...
  // module carries a screen on/off toggle. On a FACE that control cannot live
  // on the card: promotion makes `migrated()` true and stops BOTH surfaces
  // rendering `ColourofmagicCard`, so a toggle left there is deleted by the
  // promotion meant to preserve it. That is not hypothetical — it is what
  // shipped on spirographs, and this is that pattern applied rather than a new
  // one invented.
  //
  ...
  //
  // ⚠ WHAT THIS MODULE SHOWS IS NOT FIXED. Unlike spirographs, colourofmagic
  // has TWENTY-TWO outputs and a `preview` param selecting which one the
  // preview blits. That selector is promoted into the faceplate HERO, so the
  // picture here and the control that aims it are adjacent but separately
  // owned — this component reads the engine's already-selected preview output
  // and does not itself know which tap it is showing.
  ...
  // ⚠ COLLAPSING STOPS A COPY, NEVER A PRODUCER — the #2015 class, and here it
  // is structural rather than careful. The picture is produced by the VIDEO
  // ENGINE's own module instance, which the engine owns and advances; this
  // component only READS it through `blitOutputForPreview`. So SCREEN OFF stops
  // the blit and nothing else: the shader keeps running, downstream patches
  // keep receiving frames, and the first frame after switching back ON is
  // current by construction rather than the stale one the #1720/#1721 class
  // showed.
  function draw() {
    rafId = null;
...
<div class="com-output" data-testid="colourofmagic-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="colourofmagic-face-canvas"
      ></canvas>
    {/if}
    <button
      type="button"
      ...
      data-testid="colourofmagic-face-screen-toggle"
