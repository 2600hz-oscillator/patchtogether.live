<script lang="ts">
  ...
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `ShapegenCard.svelte` owns the only
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. SHAPEGEN carries no
  // time uniform, no ping-pong and no history: the figure is a pure function of
  // (its three raster inputs, size, rotate, solids), so a stalled pull costs it
  // nothing to rebuild — it would resume exactly where an unbroken loop was.
  //
  // What the mark protects is the OUTPUT. This module is a GENERATOR whose
  // picture is the whole point of patching it: its `out` is typically feeding a
  // keyer, a mixer or an OUTPUT, none of which are necessarily open while the
  // player is looking at this faceplate. Letting the watch mark lapse would
  // make a control labelled SCREEN behave as a MUTE on everything downstream.
  function draw() {
    rafId = null;
...
<div class="shapegen-output" data-testid="shapegen-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="shapegen-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="shapegen-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the shape preview is collapsed and its space reclaimed. SHAPEGEN keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the shape preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
