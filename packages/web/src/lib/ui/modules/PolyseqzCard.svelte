...
<div class="mod-card poly-card">
  ...
  <PatchPanel nodeId={id} {inputs} {outputs}>
    ...
    <div class="fader-row">
      <NeonFader value={bpm}        min={30}  max={300} defaultValue={90}  label="BPM"  curve="linear"   onchange={set('bpm')} moduleId={id} paramId="bpm"        readLive={live('bpm')} />
      <NeonFader value={length}     min={1}   max={128} defaultValue={8}   label="Len"  curve="discrete" onchange={set('length')} moduleId={id} paramId="length"     readLive={live('length')} />
      ...
      <NeonFader value={gateLength} min={0.1} max={0.95} defaultValue={0.6} label="Gate" curve="linear"  onchange={set('gateLength')} moduleId={id} paramId="gateLength" readLive={live('gateLength')} />
      <NeonFader value={humanize}   min={0}   max={1}   defaultValue={0}   label="Hum"  curve="linear"   onchange={set('humanize')} moduleId={id} paramId="humanize"   readLive={live('humanize')} />
    </div>
