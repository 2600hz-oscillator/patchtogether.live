<script lang="ts">
  // packages/web/src/lib/ui/modules/tempest/TempestOutputBody.svelte
  //
  // The TEMPEST dock full-view body: its live well picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `TempestCard.svelte` owns the only
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. TEMPEST rebuilds its
  // whole vertex set from `tempest-core` every frame — rim ring, pit ring,
  // radial lanes and the claw — from (rim, shape) alone. There is no ping-pong,
  // no history ring and no time integration, so a stalled pull costs it nothing
  // to rebuild and it would resume exactly where an unbroken loop was.
  //
  // What the mark protects is the OUTPUT. This module is a SOURCE with no video
  // input: its `out` is the whole reason to patch it, and a rack where this
  // faceplate is the only observer would otherwise stop feeding whatever the
  // well is routed into. A control labelled SCREEN would be behaving as a MUTE.
  function draw() {
    rafId = null;
...
<div class="tempest-output" data-testid="tempest-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="tempest-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="tempest-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the well preview is collapsed and its space reclaimed. TEMPEST keeps routing and keeps rendering all four outputs: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the well preview off to collapse it and reclaim the vertical space. All four outputs go on rendering either way.'}
