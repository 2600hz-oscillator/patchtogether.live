<script lang="ts">
  // packages/web/src/lib/ui/modules/videoMixer/VideoMixerOutputBody.svelte
  //
  // The V-MIXER dock full-view body: its live composite picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `VideoMixerCard.svelte` has NEVER
  // drawn a preview — it is a title, a PatchPanel and four faders — so this
  // body is not a PORT of a card affordance, it is an ADDITION the ruling
  // requires. The picture is the whole point on a MIXER: four faders that
  // SUM into one frame have no per-channel observable, and the composite is the
  // only surface on which "which sources am I actually seeing, and is it
  // clipping to white?" is answerable at all. Recorded here so nobody later
  // "restores parity" by deleting either.
  //
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. `mixer.ts`'s
  // `FRAG_SRC` declares no time uniform, no ping-pong and no history — it is a
  // weighted sum of four samplers, and its own header records that binding its
  // OWN output as a spare sampler was rejected precisely because that would be
  // a feedback loop — so the composite is a pure function of (four inputs, four
  // amounts) and would resume instantly. Do not copy `vdelay`'s or `milkdrop`'s
  // comment, which argue from state that empties.
  //
  // ⚠ AND THIS IS THE MODULE WHERE A LAPSED MARK IS WIDEST. V-MIXER is a JOIN:
  // it is the node where up to four independent chains meet, and its `out` is
  // the single frame everything downstream of the join sees. A SCREEN switch
  // that quietly stopped the sum would black out FOUR upstream chains' visible
  // result at once, which is not what the control says it does.
  function draw() {
    rafId = null;
...
<div class="video-mixer-output" data-testid="video-mixer-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="video-mixer-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="video-mixer-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the composite preview is collapsed and its space reclaimed. V-MIXER keeps rendering: switching it back on shows the LIVE mix, not a stale frame.'
        : 'SCREEN — turn the composite preview off to collapse it and reclaim the vertical space. The mix goes on rendering downstream either way.'}
