<script lang="ts">
  ...
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `FourPlexVidCard.svelte` owns the only
  ...
  //
  // ⚠ IT MATTERS MORE HERE THAN ON A GENERATOR, and that is worth stating.
  // 4PLEXVID renders ALL FOUR outputs every frame regardless of patch state
  // (the def's own architecture note), so downstream consumers of out2..out4
  // depend on this node staying in the pull set even when nobody is looking at
  // the OUT 1 preview. Dropping the watch mark here would stall three outputs
  // the switch does not even show.
  function draw() {
    rafId = null;
...
<div class="fpv-output" data-testid="fourplexvid-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    {#if !previewCollapsed}
      <canvas
        ...
        width={480}
        height={360}
        data-testid="fourplexvid-face-canvas"
        data-node-id={nodeId}
      ></canvas>
    {/if}
    <button
      ...
      data-testid="fourplexvid-face-screen-toggle"
