<script lang="ts">
  // RuttetraCard — UI for the RUTTETRA Rutt-Etra-style scanline OUTPUT.
  ...
  // SCOPE — RUTTETRA shares "compact display + controls" footprint
  ...
  function draw() {
    ...
    if (ctx2d) {
      ...
      // RUTTETRA / OUTPUT cards on one engine each render independently.
      try {
...
<div class="card video" data-testid="ruttetra-card" data-node-id={id}>
  ...
  <div class="fader-grid">
    <Fader value={p('hRamp')}     min={-1} max={1}   defaultValue={ruttetraDef.params.find((x) => x.id === 'hRamp')!.defaultValue}     label="H"     curve="linear" onchange={setParam('hRamp')} />
    <Fader value={p('vRamp')}     min={-1} max={1}   defaultValue={ruttetraDef.params.find((x) => x.id === 'vRamp')!.defaultValue}     label="V"     curve="linear" onchange={setParam('vRamp')} />
    <Fader value={p('intensity')} min={0}  max={1}   defaultValue={ruttetraDef.params.find((x) => x.id === 'intensity')!.defaultValue} label="Z"     curve="linear" onchange={setParam('intensity')} />
    <Fader value={p('lines')}     min={8}  max={240} defaultValue={ruttetraDef.params.find((x) => x.id === 'lines')!.defaultValue}     label="Lines" curve="linear" onchange={setParam('lines')} />
    <Fader value={p('spacing')}   min={0}  max={0.95} defaultValue={ruttetraDef.params.find((x) => x.id === 'spacing')!.defaultValue}  label="Gap"   curve="linear" onchange={setParam('spacing')} />
