...
/** The DEF's own defaults — never re-typed here (CLAUDE.md: ONE place). */
function defaults(): KickdrumEnvelopeParams {
  const v = (id: string): number => {
    const p = kickdrumDef.params.find((q) => q.id === id);
    if (!p) throw new Error(`kickdrum has no param '${id}'`);
    return p.defaultValue;
  };
  return {
    tune: v('tune'),
    pitch_amt: v('pitch_amt'),
    pitch_time: v('pitch_time'),
    tension: v('tension'),
    sub_decay: v('sub_decay'),
    body_decay: v('body_decay'),
    click_len: v('click_len'),
    sub_level: v('sub_level'),
    body_level: v('body_level'),
    click_level: v('click_level'),
  };
}
...
describe('kickdrum hero — the TAIL figure is measured, not drawn', () => {
  ...
  it('NEGATIVE CONTROL — doubling SUB DEC moves the tail; the caption follows', () => {
    ...
    expect(kickdrumHeroCaption(base)).not.toBe(kickdrumHeroCaption(longer));
  });
  ...
  it('a fully muted voice has no tail rather than a NaN caption', () => {
    ...
    expect(kickdrumHeroCaption(silent)).toContain('0 ms');
  });
...
describe('kickdrum hero — the PITCH SWEEP half of the caption', () => {
  ...
  it('NEGATIVE CONTROL — P AMT moves the start pitch AND the printed semitones', () => {
    ...
    expect(kickdrumHeroCaption(deeper)).toContain('+36 st');
    expect(kickdrumHeroCaption(base)).toContain('+24 st');
  });

  it('NEGATIVE CONTROL — TUNE moves the target Hz the caption ends with', () => {
    const base = defaults();
    expect(kickdrumHeroCaption(base)).toContain('→ 50 Hz');
    expect(kickdrumHeroCaption({ ...base, tune: 38 })).toContain('→ 38 Hz');
  });

  it('the caption is the mock sentence, with every number live', () => {
    expect(kickdrumHeroCaption(defaults())).toBe('tail ≈ 398 ms · +24 st → 50 Hz');
  });
});
...
describe('kickdrum hero — the plotted geometry', () => {
  ...
  it('a zero-depth sweep plots FLAT — there is no chirp to draw', () => {
    ...
    expect(kickdrumHeroCaption({ ...defaults(), pitch_amt: 0 })).toContain('· 0 st →');
  });
...
describe('kickdrum sidebar — the signal flow is the DSP chain', () => {
  it('lists the mock stages, in the mock order', () => {
    expect(KICKDRUM_FLOW.map((s) => s.id)).toEqual([
      'SUB',
      'BODY',
      'CLICK',
      'DRIVE·HARD',
  ...
  it('marks TRANSLATE as the one PARALLEL stage — it taps the RAW sub', () => {
    const parallel = KICKDRUM_FLOW.filter((s) => s.parallel).map((s) => s.id);
    expect(parallel).toEqual(['TRANSLATE']);
    // The claim is the def's own, so it cannot rot into a decorative flag.
    expect(kickdrumDef.docs!.explanation!).toContain('PARALLEL branch');
  });

  it('the legend covers every stage kind the flow actually uses', () => {
    const kinds = new Set(KICKDRUM_FLOW.map((s) => s.kind));
    for (const l of KICKDRUM_LEGEND) expect(kinds.has(l.kind)).toBe(true);
    expect(KICKDRUM_LEGEND.length).toBeGreaterThan(0);
  });

  it('every stage carries a note (an unexplained box is decoration)', () => {
    for (const s of KICKDRUM_FLOW) expect(s.note.length).toBeGreaterThan(10);
  });
});
...
describe('kickdrum sidebar — the STEREO CROSSOVER number is the worklet’s', () => {
  it('the DSP source still splits at the frequency the picture draws', () => {
    ...
    // The worklet states the split in its own words; if that changes, the
    // sidebar's "mono ‹ 120 Hz" is a lie and this goes red.
    expect(src).toContain(`${KICK_CROSSOVER_HZ} Hz`);
  });

  it('NEGATIVE CONTROL — WIDTH drives the spread the picture shows', () => {
    expect(kickdrumCrossover(0).spread).toBe(0);
    expect(kickdrumCrossover(0.2).spread).toBeCloseTo(0.2, 6);
    expect(kickdrumCrossover(1).spread).toBe(1);
    expect(kickdrumCrossover(4).spread, 'clamped, not extrapolated').toBe(1);
  });

  it('labels say which side of the split is which', () => {
    const c = kickdrumCrossover(0.2);
    expect(c.monoLabel).toContain('mono');
    expect(c.wideLabel).toContain('M/S');
    expect(c.hz).toBe(KICK_CROSSOVER_HZ);
  });
});

describe('kickdrum presets — five voices that SELECT', () => {
  it('is exactly the mock roster, in the mock order', () => {
    expect(KICKDRUM_PRESETS.map((p) => p.label)).toEqual([
      'DEEP CLUB',
      'TECHNO PUNCH',
      '909 CLASSIC',
      'SUB BOOM',
    ...
    expect(KICKDRUM_PRESETS.map((p) => p.note)).toEqual([
      '50 Hz',
      'hard',
      '62 Hz',
      '38 Hz',
  ...
  it('every stamped key is a DECLARED param and every value is inside its range', () => {
    const byId = new Map(kickdrumDef.params.map((p) => [p.id, p]));
    for (const preset of KICKDRUM_PRESETS) {
  ...
  it('the notes are not decoration — each names its own TUNE or character', () => {
    const byLabel = new Map(KICKDRUM_PRESETS.map((p) => [p.label, p]));
  ...
  it('the five voices are AUDIBLY different where the roster says they are', () => {
    ...
    // player hears and what the hero caption prints.
    const tails = KICKDRUM_PRESETS.map((p) =>
      Math.round(kickdrumTailMs({ ...defaults(), ...(p.values as Partial<KickdrumEnvelopeParams>) })),
    );
    expect(new Set(tails).size, `tails: ${tails.join(', ')}`).toBe(KICKDRUM_PRESETS.length);
  ...
  it('lookup is total on an unknown id (a stale saved slot)', () => {
    expect(kickdrumPreset('909-classic')?.label).toBe('909 CLASSIC');
    expect(kickdrumPreset('nope')).toBeUndefined();
    expect(kickdrumPreset(undefined)).toBeUndefined();
  });
});
