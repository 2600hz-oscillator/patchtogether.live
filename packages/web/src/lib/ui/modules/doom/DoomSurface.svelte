...
<script lang="ts">
  // DoomSurface — THE ONE DOOM SURFACE, mounted by both the legacy card
  // (`DoomCard.svelte`, a thin bridge) and the v2 faceplate body
  // (`doom/DoomBody.svelte`). It is a MOVE, not a rewrite: every line below
  // came out of `DoomCard.svelte` unchanged except where the `variant` prop
  // gates card-only chrome, so the two surfaces cannot diverge and there is
  // exactly one keyboard map, one session adoption and one blit loop.
  //
  // ⚠ WHY THE MOVE IS STRUCTURALLY REQUIRED BY THE PROMOTION, stated here
  // because it is the whole argument for touching DOOM at all. Promotion sets
  // `migrated('doom')`, and from that moment neither surface renders
  // `DoomCard.svelte` on the default shell — `DockFullView` mounts
  // `<ModuleShell>` instead. Everything DOOM's multiplayer needs lives in this
  // component's `onMount`: `nodeDoomSession.adopt` (the pump that feeds the
  // lockstep barrier), the awareness/nodes/edges observers, the window-level
  // keyboard capture, the `__doomCards` hook every DOOM spec reads, and the
  // canvas blit. A face body that did not carry them would ship a promoted
  // DOOM that is a black tile with no game in it, while every registry gate
  // stayed green — the exact failure `module-surfaces` names. The gibribbon
  // (`gibribbon/GibribbonScreen.svelte`) and nibbles (`nibbles-game-actions.ts`)
  // promotions took the same route for the same reason.
  //
  // ⚠ NOTHING ABOUT THE GAME IS RE-TIMED HERE. `runtime.runTic()` still runs
  // where it always ran — inside `surface.draw`, in `video/modules/doom.ts` —
  // so DOOM's game clock is still its frame clock and this file does not
  // introduce a second one. No lockstep, netcode, consistency or budget code is
  // touched by the move.
  //
  ...
  interface Props {
    /** The graph node id. On the card this is `NodeProps.id`; on the faceplate
     *  it is the ONLY prop the `fullViewBody` slot gets. */
    id: string;
    /** SvelteFlow's node payload. Card-only — the faceplate slot has none, and
     *  the one thing it feeds (the OUTPUT FIT / Volume row) is card-only too:
     *  on the face those two params are ranked cells the shell renders. */
    data?: { node?: ModuleNode };
    /** Which surface is mounting this. `'card'` keeps the legacy chrome —
     ...
     *  mounted so a collapsed screen never drops a player out of a netgame. */
    previewCollapsed?: boolean;
    /** The CARD's own control row (OUTPUT FIT + Volume), rendered where the
     *  card has always drawn it. Card-only by construction: the faceplate
     *  passes none, because the shell renders those two params as ranked cells
     *  from `face.order`. See the render site for why it lives on the card. */
    controlsRow?: Snippet;
  }
  let {
    id,
  ...
  // ⚠ `fillMode` / `audioGain` AND THEIR SETTERS LIVE ON THE CARD, not here —
  // see the `controlsRow` render site below. They are card CHROME (the
  // faceplate renders both params as ranked shell cells), and the card↔def
  // agreement ledgers scan `*Card.svelte` only, so moving them would have
  // emptied those gates' subject set for this module without fixing anything.
  ...
  // ── WHAT THE FACE PAINTS, AND WHY IT IS LESS THAN THE CARD ────────────────
  //
  // The identity row is the module's MULTIPLAYER INDICATOR and functional
  // parity keeps it — a player must be able to see which marine they are, and
  // whether they are hosting, playing or spectating. But the CARD paints that
  // twice: a short BADGE (`P2`, `HOST`, `SPEC`) and a full derived SENTENCE
  // beside it (`Player 2 — alice (you)`, `Spectating — joining as Player 3
  // next map`, `2 rack-mates · host: remote · player 2`). At rest, beside the
  // thing it describes, that sentence is exactly the shape the
  // no-resting-derived-text ruling refuses, and `face-resting-text-source`
  // structurally cannot see it here (GAMES.md §1: text inside a bespoke
  // `fullViewBody` is invisible to that gate, and DOOM has no dock baseline
  // either — see its FACES_WITHOUT_SCENES entry — so NOTHING would catch it).
  //
  // So the face keeps the BADGE and moves the SENTENCE to the accessible name
   ...
   *  and assertable without a resting readout. The card carries it too; it is
   *  additive there. */
  let surfaceLabel = $derived(
    'DOOM video module — click to capture the keyboard, then arrows move, Ctrl fires, Space uses'
      + (mySlot !== null ? `. ${identityLabel}` : myPendingSlot !== null || memberIds.length > 1 ? `. ${specLabel}` : '')
      + (isHost ? '. You are the host for this rack' : '')
      + (memberIds.length > 1 ? `. ${memberIds.length} rack-mates` : '')
      + (loadStatus === 'ready' ? '. WAD loaded' : loadStatus === 'loading' ? '. Loading WAD' : loadStatus === 'error' ? `. Load failed: ${loadError ?? ''}` : '. Not loaded'),
  );

  /** SvelteFlow starts a NODE DRAG on pointerdown anywhere in a node and pans
   *  the canvas from the pane, so a game surface has to say "this press is
   *  mine". ⚠ VARIANT-GATED DELIBERATELY: the CARD is a SvelteFlow node and
   *  dragging it by its body is shipped behaviour — stopping propagation there
   *  would make the DOOM card immovable on the canvas, which is why the card
...
{#snippet gameBody()}
  ...
  <!-- ⚠ THE CARD'S OWN CONTROL ROW, PASSED IN RATHER THAN OWNED HERE. The
       OUTPUT FIT toggle and the Volume knob are CARD chrome: on the faceplate
       those two params are RANKED CELLS the shell renders from `face.order`, so
       painting them here as well would put two controls on the plate for one
       param — which `faces-parity` COUNTS rather than compares, and would pass.
       ⚠ AND KEEPING THE MARKUP IN `DoomCard.svelte` KEEPS IT WHERE THE CARD↔DEF
       AGREEMENT LEDGERS CAN STILL SEE IT. `card-def-agreement.test.ts` and
       `card-range-source.test.ts` scan `*Card.svelte` ONLY; a control that
       slipped out of their reach would take its recorded divergence with it and
       the entry would read as PAID when nothing had been fixed — the
       silently-emptied-subject-set failure `card-media-lifetime.test.ts` has now
       recorded three times. This is one surface for the GAME; the card keeps its
       own chrome. -->
  {@render controlsRow?.()}
...
<!-- role="application" + tabindex="0" + onclick: the surface IS an
     ...
     (which is also good UX — click anywhere to grab the keyboard).
     Key handling is window-level capture (see <script> header) — NOT
     element-level — because xyflow's own keydown listeners fire on the
     document and we need to preventDefault BEFORE they do to keep arrow
     keys from moving the node on the canvas instead of reaching the
     in-game player.

     ⚠ NO TAB-ORDER ADDITION ON THE FACE. `tabindex="0"` here is the SAME
     single stop the card has always had, moved rather than added, and it is
     what `role="application"` requires; Tab remains the faceplate's flip
     gesture because nothing inside this surface is focusable in sequence —
     the keyboard is taken by POINTER (click latches), exactly as on the card. -->
...
<style>
  /* `.doom-fit-row` / `.doom-fit-label` / `.doom-volume` live in
     DoomCard.svelte with the markup they style — Svelte scopes styles per
     component, so a rule here would be pruned as unmatched. */
  .doom-card {
    width: 360px;
    min-height: 320px;
    outline: none;
  ...
  .doom-card.face-variant:focus-within {
    /* The faceplate draws its own focus ring; a second inset outline inside the
       dock pane reads as a nested card. */
    outline: none;
  }
  .doom-card.face-variant .title {
    /* No stripe and no ModuleTitle here, so the row collapses to zero height
       when the identity snippet renders nothing (the resting solo case). */
    min-height: 0;
    padding: 0;
    gap: 6px;
  }
  .doom-card.face-variant .title:empty {
    display: none;
  }
</style>
