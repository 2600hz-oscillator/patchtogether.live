<script lang="ts">
  // packages/web/src/lib/ui/modules/luma/LumaOutputBody.svelte
  //
  // The LUMA dock full-view body: its live graded picture plus the SCREEN
  // ON/OFF switch the 2026-08-18 owner ruling requires of every video module.
  //
  // ⚠ WHY THIS FILE EXISTS (#1928 class). `LumaCard.svelte` has NEVER drawn a
  // preview — it is a title, a PatchPanel and four faders — so this body is not
  // a PORT of a card affordance, it is an ADDITION the ruling requires. The
  // picture earns its place on THIS module more than on most: LUMA ships as a
  // bit-exact IDENTITY (gamma 1 · cntr 1 · post 16 = true bypass · bias 0), so
  // the only observable that distinguishes "graded" from "untouched" is the
  // frame itself. Recorded here so nobody later "restores parity" by deleting
  // either.
  //
  ...
  //
  // ⚠ THE REASON HERE IS THE OUTPUT, NOT AN ACCUMULATOR. `luma.ts`'s `FRAG_SRC`
  // declares no time uniform, no ping-pong and no history — the transfer is a
  // per-texel chain of gamma → contrast → posterize → bias re-applied as a luma
  // RATIO — so the grade is a pure function of (input, four params) and would
  // resume instantly and correctly. Do not copy `vdelay`'s or `milkdrop`'s
  // comment, which argue from state that empties.
  //
  // What the mark protects is the OUTPUT, and LUMA is a MID-CHAIN GRADE: it is
  // patched between a source and whatever consumes it, so its `out` is the
  // reason it is in the rack at all. A rack where this face is the only
  // observer still gets frames. A SCREEN switch that quietly stopped the grade
  // would change what the DOWNSTREAM sees, which is not what the control says
  // it does.
  function draw() {
    rafId = null;
...
<div class="luma-output" data-testid="luma-output-body">
  <div class="preview-wrap" data-preview-collapsed={previewCollapsed ? 'true' : 'false'}>
    ...
    <button
      ...
      data-testid="luma-face-screen-toggle"
      aria-pressed={!previewCollapsed}
      title={previewCollapsed
        ? 'SCREEN is OFF — the graded preview is collapsed and its space reclaimed. LUMA keeps rendering: switching it back on shows the LIVE picture, not a stale frame.'
        : 'SCREEN — turn the graded preview off to collapse it and reclaim the vertical space. The grade goes on rendering downstream either way.'}
