...
const EXTENSION_BODY_ROLES: Readonly<Record<string, BodyRule>> = {
  ...
  // the plate. Nothing had to be removed on promotion either: the surface it
  // replaced never printed a readout.
  //
  ...
  controlSurface: { role: 'control-grid', why: 'the POINTER BOARD — per-source group boxes with live display names, each holding proxied Knobs that read and write the SOURCE node\'s param through resolveSurfaceParam (flat node.params AND nested TOYBOX node.data both land on the live location the source\'s own knobs use), under a passthrough stripe of that source\'s control colour resolved per render and never stored, plus the per-knob ✎ rename (the Electra custom label, an inline <input type="text"> that is deliberately IN the fullViewBody file — the typed-entry parity leg reads only the directly-imported component) and the drag layout (unlocked: absolute boxes persisted to node.data.layout; locked: a flowing wrap — armed by the ranked LOCK cell, which is NOT duplicated here). ⚠ IT IS A CONTROL GRID, NOT A PICTURE: a knob is the surface another module\'s param is turned from, and it mounts no drawing surface and must not grow one — the role predicates GREP RAW SOURCE and cannot tell code from a comment, so this sentence spells the tag out in words; beyond the gate, attest basis membership is derived from CONTENT, so a drawn body would enrol a meta module in the GPU attest. ⚠ EVERY PROXIED KNOB PASSES AN EXPLICIT testid (cs-board-dial-*): Knob.svelte emits control-<paramId> whenever the MIDI-learn key is passed, and faces-parity asserts exact multiset equality against this def\'s params, which is []. For the same reason NO testid in the body starts with control- — the older control-surface-* vocabulary is deliberately not reused. ⚠ ALL PAINTED TEXT IS A CONTROL CAPTION, A SOURCE MODULE\'S NAME, OR EMPTY-STATE COPY: the group labels (names), each knob\'s own caption (a label prop consumed by Knob.svelte), the ✎ affordance, and the empty-state instruction naming the right-click "Send to …" gesture — the module\'s only discovery path, the midiclock licence. No value, no measurement, no state word; the control counts are DERIVED onto the board\'s aria-label and never painted. ⚠ THE PRUNE IS NOT HERE AND THAT IS THE POINT: pruneSurfaceDangling rides the tileBody (node-on-canvas lifetime), because this body mounts only while the dock full view is open and the card\'s $effect was the tree\'s only production caller — the ES-9 card-only-side-effect shape, which EXTENSION_BODY_ROLES itself cannot see (a tileBody is outside this gate\'s subject; controlsurface-face-model.test.ts pins the call at the source and the face e2e proves it with the dock CLOSED). ⚠ IT IS A BODY RATHER THAN A PANEL for the electraControl reason landing on both halves: ShellPanelCell.minWidth is a required NUMBER and this board\'s width is a function of how many sources are bound (360–760 px on the card), and the required probe speaks data/data-rev/text while this surface\'s observable is a param on a DIFFERENT NODE. ⚠ NO SCREEN SWITCH and NO WATCH MARK: the video-screen ruling runs over STRICT_FACES INTERSECT video defs and this is domain meta, and markWatched is a VideoEngine pull-set concept this module has no part in.' },

  // ── CHROMA CONSOLE (2026-09-02) — the first control grid whose cells name a
  //    PARAM'S MEANING rather than proxy another node ────────────────────────
  //
  // ⚠ THE GRID EXISTS FOR A REASON NO OTHER ENTRY IN THIS ROSTER HAS. matrixMix,
  // gamepad, electraControl and controlSurface are grids of things that are not
  // the module's own params at all (edges, a gamepad poll, pointers into other
  // nodes). This module's eight slots ARE its own params and DO have cells in
  // the band below — what the board carries is what those cells structurally
  // cannot: the per-NODE NAME each slot has acquired. `deviceSlotParams` mints
  // eight identical `slot 1`..`slot 8` ParamDefs because an id is permanent
  // while `node.data.assign` is per instance, and a cell caption is
  // `ParamDef.label` with no node input anywhere in the shell.
  //
  // ⚠ IT SATISFIES TWO PREDICATES, gamepad's situation exactly: one `StatusLed`
  // (the MIDI lamp) lives on the surface, so `status-primitive` holds too. The
  // declared role is `control-grid` because the BOARD is what the eight slots
  // are assigned from — the lamp is one indicator beside the two pickers, not
  // the surface's purpose.
  //
  // ⚠ AND THE SPEAKABLE LEG BITES HERE. Each chip paints the assigned control's
  // SHORT name (`chip.name`) while its accessible name is the whole sentence
  // (`chip.detail`) — what it drives, on which CC, and whether the pedal snaps
  // it. Both are built in `./chromaconsole-status-model`, so the painted and the
  // spoken expressions are structurally different rather than accidentally so,
  // which is the fix gamepad's entry records for the same leg.
  linnstrument: { role: 'control-grid', why: 'THREE XY PADS — the LinnStrument\'s R / G / B retained joystick pairs as three DOM pads (the joystick body, three times: jump-to-point, pointer capture, the Y flip, lostpointercapture recovery, double-click re-centre, NO SNAP-BACK per #1963), plus the LINK lamp and one StatusLed per musical region and the three D15 target pickers. ⚠ IT IS A CONTROL GRID, NOT A PICTURE: a pad is the surface a joystick pair is MOVED from, and every drag is a `set_pair` intent dispatched through linnstrument-cell-actions.ts into the runtime\'s ONE selection reducer — the same reducer the hardware finger, the ranked cells and patch hydration feed — so the dot, the CV jack and the device LEDs cannot disagree. It mounts no canvas and must not grow one: the role predicates grep raw source, and beyond the gate attest-basis membership is derived from CONTENT, so a drawn pad would put an audio module\'s every future edit on the GPU-attest path. ⚠ IT IS NOT A FACE CELL: the six `pos_*` axes rank as ordinary knob cells and are the parity-credited controls, so these pads emit NO `control-*` anchor, NO `data-control-params` and NO `data-cell-*` attribute (the joystick redundancy, stated in the def). Selection is shown as the pad border BURNING, a colour mark, never a second toggle over the ranked `sel_*` cell. ⚠ THE SEMANTICS LIVE ON aria-label: each pad\'s live pair and whether it is following the finger or holding are its accessible name (role="application" has no aria-valuetext) and are never painted; the bound source, the session, the voice counts, the arp state and the rejected/stolen counters are StatusLed detail. ALL PAINTED TEXT IS A CAPTION OR AN OPTION NAME: R / G / B, LINK / KEYS / PAD, and the target pickers\' joystick names. ⚠ THE DEVICE ROSTER CANNOT BE A CELL (midi-lane.ts:525-529) and here it is not even a roster: one app-wide LinnStrument source is bound at a time, so the LINK lamp names it. ⚠ NOTHING ONGOING LIVES HERE: the source subscription, the reducer, the CVs, the buses and both arps are the factory\'s and run with this body unmounted. ⚠ NO SCREEN SWITCH and NO WATCH MARK: the video-screen ruling runs over STRICT_FACES INTERSECT video defs and this is domain audio, and markWatched is a VideoEngine pull-set concept this module has no part in.' },
  chromaconsole: { role: 'control-grid', why: 'the DEVICE BINDING plus the SLOT BOARD for the Hologram Chroma Console pedal: the live MIDI output picker, the 16-entry channel picker, the MIDI lamp, the role="alert" problem line, the eight slot chips naming what each automatable slot drives, the ASSIGN mode that swaps those chips for the grouped 29-entry assignment selects, and the five role:action pedal commands (tap tempo, capture, the two gesture-looper commands, the calibration menu). ⚠ IT IS A CONTROL GRID, NOT A PICTURE: the board is the surface the eight slots are ASSIGNED from, and it mounts no drawing surface and must not grow one — the role predicates read raw source and cannot tell code from a comment, so this sentence spells the tag out in words; beyond the gate, WebGL attest basis membership is derived from CONTENT and this def deliberately chose the audio domain to stay out of that basis. ⚠ THE OUTPUT PICKER CANNOT BE A CELL for the reason midiclock states — its roster lives on the engine handle behind requestMIDIAccess(), differs per machine and changes when hardware is plugged in — and THE CHANNEL cannot for a second reason worth separating: it is a fixed 16-entry roster, but it lives on the device HANDLE rather than the graph, so a ShellSelectorCell value, a pure function of the node, would paint a number that never changed. ⚠ CONNECT, PUSH ALL AND THE EIGHT SLOT VALUES ARE NOT DUPLICATED HERE: all ten are ranked cells that reach the lane, and a body carrying them too would be a second implementation of controls the face already owns. The one exception is stated rather than hidden: a slot assigned to one of the pedal\'s named-range SELECTORS (role enum — the four module switches, the six bypasses) is operable here as a real Segmented, because its states are named ranges and no honest knob exists for them; that control passes an EXPLICIT testid so the second surface does not claim to be the param\'s cell under faces-parity\'s exact-multiset leg. ⚠ ALL PAINTED TEXT IS A CAPTION, A NAME OR AN EMPTY-STATE INSTRUCTION: the two picker captions, the device names in the output roster, ch 1..ch 16, the SLOTS heading, each chip\'s assigned-control name, each pedal command\'s descriptor label, and the pre-connect hint (the midiclock licence). ⚠ THE `·snap` MARKER IS A NAME, NOT A READOUT, and it is the entry\'s one judgement call: it is a property of the ASSIGNED CONTROL, identical at every value, marking the two controls (RATE, TIME) the pedal quantizes to tempo subdivisions and cannot be told not to. ⚠ NO VALUE IS PAINTED ANYWHERE. The card\'s per-slot formatControlValue readout is DELETED under the 2026-08-17 ruling, and so is its stale-slot COUNT — the stale SIGNAL is kept per chip, which says WHICH slot rather than how many, with the sentence on aria-label. ⚠ NO COUNTER, NO ACTIVITY DOT, NO ELAPSED TIME: the pre-promotion surface recorded that its resting render must be byte-stable for a committed VRT baseline and that the same deletions stop it implying it knows what a receive-only pedal holds; BINDERS §2.1 would permit a non-text activity dot and it is refused here. ⚠ NO SCREEN SWITCH and NO WATCH MARK: the video-screen ruling runs over STRICT_FACES INTERSECT video defs and this is domain audio, and markWatched is a VideoEngine pull-set concept this module has no part in.' },
  ...
  // ── CLIPPLAYER (2026-09-02) — the launcher's DECK, and the roster's first
  //    body whose module also owns six PANEL CELLS ──────────────────────────
  //
  // ⚠ `control-grid` IS THE HONEST ROLE, and BOTH other predicates were checked
  // rather than eliminated by default. `picture` is false: this body mounts no
  // drawing surface and must never grow one (the predicates GREP RAW SOURCE and
  // cannot tell code from a comment, so the tag is spelled out here in words;
  // beyond the gate, attest basis membership is derived from CONTENT, so a
  // drawn body would enrol an audio def in the GPU attest for nothing).
  // `status-primitive` genuinely HOLDS — the body imports StatusLed and paints
  // four of them — and it is still not the declared role, on the clockedRunner
  // and livecode precedent: the role names what the body IS, and this is the
  // surface the module is OPERATED from with lamps beside it, not a measurement
  // strip. Almost every element here is a gesture.
  //
  // ⚠ WHAT IS *NOT* IN THIS BODY IS THE POINT OF THE MODULE. The launch grid,
  // the piano roll and the four eight-wide lane rows are PF-14 PANEL CELLS, not
  // body content — a distinction no other entry in this roster has had to make,
  // and one this gate is structurally blind to (it reads `fullViewBody` sources
  // only). What is left here is everything that could not be a cell for a
  // reason the registry states: the TRANSPORT and the tempo nudge write
  // TIMELORDE's params (a param on ANOTHER NODE, the electraControl
  // addressability argument), the clip-undo stack and the monome bind are
  // one-shot gestures with no ParamDef, and the two recorders are node.data
  // modes whose surfaces are buttons.
  clipplayer: { role: 'control-grid', why: 'the PERFORMANCE DECK — the TIMELORDE transport and its ±2 bpm nudge, the clip-undo/redo stack, RST (wrapped in MidiAssignButton so the documented, localStorage-persisted MIDI binding on the `reset` param survives promotion), the panic STOP ALL, the monome GRID bind, the per-lane MUTE/STOP row, BOTH recorders kept visibly apart (the ARRANGER record captures clip LAUNCHES onto a timeline; SONG-REC prints a concrete performance, and its arm stamps this client as the single-writer recorderId through the shared seam), the ARR ⤢ pop-out, and four StatusLeds. ⚠ IT IS A CONTROL GRID, NOT A PICTURE: it mounts no drawing surface and must not grow one (the role predicates read raw source; and a drawn body would enrol an audio def in the GPU attest). status-primitive also HOLDS of this source and is deliberately not the declared role, on the clockedRunner/livecode precedent — the role names what the body IS, and nearly every element here is a gesture. ⚠ THE MODULE\'S ACTUAL SURFACE IS NOT IN THIS FILE and this gate cannot see it: the 8x8 launch grid, the piano roll and the four eight-wide lane rows (mono, rate, arm, scene repeats) are PF-14 PANEL CELLS registered in shell-cells.ts, which is why a body that looks small belongs to a very large module. ⚠ THREE RESTING READOUTS WERE DELETED, NOT HIDDEN, and each was a derived value painted outside any control: the per-lane ASSIGNED-MODULE count chips, the MAX track-cap badge and the SONG "{n} notes - {n} bars" line. All three are StatusLed detail now, reaching aria-label and title only; the TIMELORDE bpm joins them as a fourth, because the tempo belongs to the rack transport and this module does not own it. The automation OVERRIDE dot keeps its click (re-enable every suspended param at once) — a lamp that is also a button is still a lamp. ⚠ FOUR CONTROL FAMILIES WERE DELETED FROM THE DEF in the same diff, and this body is where three of them landed: auto-assigned, auto-cap and auto-override are pure or conditional READOUTS with no gesture a faces-parity probe could drive on a fresh node, and a family that cannot be probed is a cell nothing can prove is alive. CLR AUTO, the fourth, is a real button and lives in the note-editor panel where the clip it clears is open. ⚠ ALL PAINTED TEXT IS A CONTROL CAPTION OR A MODE NAME: the transport glyph, the undo/redo arrows, RST, STOP ALL, GRID, M/STOP per lane, SES/ARR, SES/SONG, OVR/RPL, ARR, and the four static lamp captions (BPM, ASSIGNED, REC, MAX). No value, no measurement, no state sentence. ⚠ THE ONE ONGOING BEHAVIOUR THE CARD OWNED NEEDED NO MOVE: pruneAutoAssignDangling is a card $effect, but pruneAllAutoAssignDangling() already sweeps EVERY clip player from the Canvas graph-change seam precisely so an assignment is dropped "even when no clipplayer CARD is mounted" — the behaviour was already outside the UI, so re-mounting it here would add a second janitor. The card\'s rAF poll DID move and is here, reading transport state, the automation countdown, the override set and the track-cap flag once per frame and reassigning only on a real change. It is in the DOCK BODY and never in the tileBody, which mounts on every rack boot (the #2314 rule). ⚠ NO SCREEN SWITCH and NO WATCH MARK: the video-screen ruling runs over STRICT_FACES INTERSECT video defs and this is domain audio, and markWatched is a VideoEngine pull-set concept this module has no part in.' },
  livecode: { role: 'control-grid', why: 'the SCRIPT BUFFER and its OUTPUT LOG — a CodeMirror buffer holding the JavaScript that builds and patches the rack, plus the lines the last run printed through log(). ⚠ THIS BODY IS THE MODULE, not a surface beside it: livecodeDef.factory returns a no-op handle, so every evaluation this module ever performed happened inside its card, and migrated(type) stops both surfaces rendering a promoted module\'s card — the promotion moved the run out to livecode-cell-actions.ts, which the ranked RUN cell and this body both call rather than either owning. ⚠ IT IS A CONTROL GRID, NOT A PICTURE: the buffer is the surface the module is operated from, and it mounts no drawing surface and must not grow one (the predicates read raw source; and a drawn body would enrol an audio def in the GPU attest). ⚠ THE OUTPUT LOG IS THE SCRIPT\'S PRODUCT, NOT DERIVED STATE ABOUT A CONTROL. This module has no ports at all, so the log is literally its only output and deleting it would delete a documented API function\'s destination — and it is ABSENT AT REST, because node.data.lastRun is unset until a run happens, so the resting plate paints no log and no empty-state placeholder (the card\'s "output log appears here after Run" is GONE, not relocated: the plate has other content, so the empty-state-instruction licence midiclock and es9 hold does not reach it). ⚠ ALL OTHER PAINTED TEXT IS THE USER\'S OWN DOCUMENT plus one static lamp caption (RUN). The card\'s `OK - {n} mutations applied` and its line:col error sentence are StatusLed detail now, reaching aria-label and title only. ⚠ THE SEMANTICS LIVE ON aria-label: the buffer and the log are named groups whose accessible names say what each region IS, built in the script and never painted. ⚠ IT IS A BODY RATHER THAN A PANEL by the probe discriminator: ShellPanelProbe REQUIRES an element to click or drag, and neither a text document nor a console is one. ⚠ NO RESIZE GRIP, unlike the card, for the videoOut reason. ⚠ NO SCREEN SWITCH and NO WATCH MARK: domain audio, and markWatched is a VideoEngine concept this module has no part in.' },
  // ── JOYSTICK (2026-09-01, wave 3) — the roster's first body whose entire
  // surface is ONE 2-D control ──────────────────────────────────────────────
  //
  // ⚠ `control-grid` IS THE HONEST ROLE, checked rather than defaulted.
  // `picture` is false — the pad is DOM (crosshairs + a dot), it mounts no
  // canvas and must never grow one (attest basis membership is derived from
  // CONTENT, and a drawn body would enrol an audio utility in the GPU attest).
  // `status-primitive` is false — no StatusLed, and nothing here is a
  // measurement. The third role's own definition is what fits: "a DOM control
  // surface that is neither a preview of something nor a measurement of
  // something, but the surface the module is operated FROM" — a one-cell grid
  // is still the surface the module is operated from.
  //
  // ⚠ THE BODY IS DELIBERATELY NOT A FACE CELL. The two-ordinary-cells
  // fallback (owner decision 2026-08-31) ranks pos_x/pos_y as plain knob
  // cells — those are the parity-credited controls — and this pad is the
  // module's own redundant surface, so it must NOT emit `control-*` anchors,
  // `data-control-params` or `data-cell-*` attributes.
  // `joystick-face-model.test.ts` pins that, alongside the #1963 no-snap-back
  // drag contract this body carries forward unchanged.
  // ── MOOG 956 (2026-09-02) — the roster's first body that is ONE 1-D
  // control, and its `tileBody` twin ────────────────────────────────────────
  //
  // ⚠ `control-grid` IS THE HONEST ROLE, checked rather than defaulted, the
  // joystick way. `picture` is false — the strip is DOM (a bordered box and a
  // positioned wiper), it mounts no canvas and must never grow one (attest
  // basis membership is derived from CONTENT, and a drawn strip would enrol an
  // audio utility in the GPU attest). `status-primitive` is false — no
  // StatusLed, and nothing here is a measurement. The third role's own
  // definition fits: "the surface the module is operated FROM".
  //
  // ⚠ THE GATE THIS ENTRY CANNOT SEE, named rather than left implicit: this
  // extension also fills `tileBody`, and a `tileBody` is outside this file's
  // subject entirely (`extensionsWithBody` greps for `fullViewBody:`). The two
  // slots mount the SAME `Moog956RibbonStrip`, so what is verified here is
  // true of the lane strip by construction — but only by construction, and
  // `moog956-face-model.test.ts` is where the tile half is actually pinned.
  moog956: { role: 'control-grid', why: 'the RIBBON — the module\'s whole instrument, and the one thing no ranked cell can express: ONE POINTER STROKE that writes `pos` and raises `gate` together, slides with the gate standing, and on release drops the gate while LEAVING the pitch (the hardware wiper keeps its voltage; the def promises the patched VCO stays on the last note). Press captures the pointer, `lostpointercapture` and `pointercancel` both end the gesture, and unmount releases the gate — pointer capture protects a MOVING pointer, not a DELETED element, and this body unmounts mid-hold whenever the dock pane closes. ⚠ IT IS A CONTROL GRID, NOT A PICTURE: the strip is the surface the pitch is played from, it mounts NO canvas and must not grow one — the role predicates GREP RAW SOURCE and cannot tell code from a comment, so this sentence spells the tag out in words; beyond the gate, attest-basis membership is derived from CONTENT, so a drawn strip would put an audio utility\'s every future edit on the GPU-attest critical path. ⚠ IT IS NOT A FACE CELL: `pos` ranks as a FADER cell and `gate` as a momentary PAD, and those are the parity-credited controls, so this strip emits NO `control-*` anchor, NO `data-control-params` and NO `data-cell-*` attribute — any of them would double-count `pos` in faces-parity\'s exact multiset, a failure that only appears in the browser lane. The redundancy (strip above, four cells below) is the twotracks/joystick shape and is stated in the def. ⚠ IT PAINTS NO TEXT AT ALL: no captions, no values, no state words. The card\'s `{n} st` pitch readout is this promotion\'s named deletion (owner-decisions 2026-08-31 item 11) and the live value is the strip\'s `aria-valuetext` — `role="slider"` genuinely has one, unlike the `role="application"` pad one entry down, which had to fall back to `aria-label`. The gate lamp survives as a COLOUR MARK rather than a word: the wiper burns while the finger is down. ⚠ THE MEANING IS ON `aria-label`, per the resting-text ruling: this body names itself as a group ("956 ribbon controller — press and slide to play"), which is the instruction a strip with no affordances of its own cannot otherwise give, and the strip inside it carries its own name and value. ⚠ IT IS A BODY RATHER THAN A PANEL by the width discriminator: `ShellPanelCell.minWidth` is a required NUMBER and this control\'s PRECISION is its width (≈ 8 px per semitone across the default 2-octave span), so it wants the plate rather than a band column. ⚠ NO SCREEN SWITCH and NO WATCH MARK: the video-screen ruling runs over STRICT_FACES INTERSECT video defs and this is domain audio, and markWatched is a VideoEngine pull-set concept this module has no part in.' },
  joystick: { role: 'control-grid', why: 'the XY PAD — the module\'s real instrument, carried forward unchanged: jump-to-point on pointerdown, pointer capture, the Y flip (drag up = +y), rAF-coalesced tracked commits per axis with a flush on pointerup, lostpointercapture recovery, double-click re-centre to the (0,0) defaults, and NO SNAP-BACK (#1963 "1 - persist" — the position is node.params and survives release, eviction and reload). ⚠ IT PAINTS NO TEXT AT ALL: no captions, no values, no state words — the card\'s x/y decimal readout row is DELETED on this surface (owner-decisions item 11), and the live values are the pad\'s aria-label (role="application" has no aria-valuetext), the same place XyPad.svelte put them when #2038 deleted the generic pad\'s row. ⚠ IT MOUNTS NO CANVAS and must not grow one — WebGL attest basis membership is derived from CONTENT, and a drawn pad would put an audio utility\'s every future edit on the GPU-attest critical path. ⚠ NO SCREEN SWITCH and NO WATCH MARK: the video-screen ruling runs over STRICT_FACES INTERSECT video defs and this is domain audio, and markWatched is a VideoEngine pull-set concept this module has no part in.' },
  ...
  // module always painted. What is FORBIDDEN is a score or lives row rendered
  ...
  doom: { role: 'picture', why: 'the MARINE\'S POV — the 640x400 DOOM framebuffer blitted by the SHARED DoomSurface component both surfaces mount, plus everything a player needs to reach the game: the WAD load gesture, the click-to-capture-keyboard hint, the multiplayer identity badge, the Single Player / Host Multiplayer choice, the guest Join button, the arbiter\'s New Game dialog and the SCREEN switch. ⚠ IT IS THE MODULE\'S IDENTITY AND ITS ONLY INSTRUMENT AT ONCE: doom ranks exactly TWO cells (audioGain, fillMode) and both are trims on the OUTPUT — without this body a promoted DOOM could not be booted, focused, played, hosted or joined by any route, because the card that owned all of it is no longer rendered on the default shell. ⚠ THE BODY\'S ONLY PAINTED TEXT IS CONTROL CAPTIONS AND EMPTY-STATE COPY: the SCREEN caption, the load / loading / load-failed overlays and the "waiting for host" line (each REPLACED by the surface the moment that surface exists — the samsloop NO SAMPLE LOADED shape), the New Game field captions, and the short identity BADGE (P2 / HOST / SPEC). DOOM\'s health, armour, ammo and status-bar face are painted INSIDE the framebuffer by the game itself — the module\'s own artwork, and the settled precedent the GAMES.md ruling cites without naming. ⚠ THE CARD\'S TWO DERIVED SENTENCES ARE NOT CARRIED: "Player 2 — alice (you)" and "2 rack-mates · host: remote · player 2" are a labelled derived value at rest beside the thing they describe, so the face keeps the badge and moves both to the surface\'s aria-label (the GAMES.md §1.1 remedy). ⚠ SCREEN OFF IS SAFE, AND FOR A DIFFERENT REASON THAN EVERY SIBLING HERE — this is the one module whose GAME CLOCK IS ITS FRAME CLOCK. frogger, gibribbon and pong step on the shared scheduler clock and could not be stopped by a preview toggle if you tried; DOOM ticks inside `surface.draw`, so what makes OFF safe is that the collapse removes the card-style 2-D putImageData COPY of a frame the VIDEO ENGINE has already produced, never the engine draw that produces it. A peer that stopped ticking would starve every other peer\'s lockstep barrier (#345 semantics), so this is load-bearing rather than tidy. ⚠ ONLY THE CANVAS IS INSIDE THE COLLAPSE: unlike gibribbon, this body does NOT unmount its surface component, because that would drop the node session adoption, the awareness observers, the keyboard capture and the Join / New Game affordances — a host reclaiming dock space would have taken the lobby down with it. ⚠ NO WATCH MARK, MEASURED: `VideoEngine.isPullExempt` names DOOM in its own comment — a non-empty audioSources map makes it a pull ROOT unconditionally — and no earlier surface marked it either (its blit reads extras.snapshotFramebuffer() directly, not through the engine), so adding one here would be new behaviour dressed as parity.' },
  ...
  modtris: { role: 'picture', why: 'the TETRIS WELL — a 10x20 stack plus the NEXT-piece preview and the LN / LV counts, painted every rAF from the engine snapshot by the module\'s own exported `drawModtris`, plus its SCREEN switch. ⚠ IT IS THE MODULE\'S IDENTITY, NOT A PREVIEW OF SOMETHING ELSEWHERE: the well IS how you read what the gate outputs are doing — modtris is a gate-driven ACCUMULATOR whose output rate is a function of the stack you have built — and before promotion it existed only on a surface the shipping shell does not mount (modtris is not in NON_SHELL_LANE_TYPES, not a CARD_PRODUCER and not in HEADLESS_MOUNT_LANE_TYPES, so its lane tile was a blank placeholder). ⚠ THE COUNTS ARE PAINTED INTO THE CANVAS BY THE GAME and are allowed as the game\'s own artwork; the face adds NO chrome row of its own, and the body\'s only DOM text is the SCREEN caption. They reach the a11y tree through role="img" + aria-label on the well frame — and that frame sits OUTSIDE the collapse guard, so the accessible name genuinely tracks the game while the picture is off (frogger\'s puts it inside, so the same claim in that file is false there). ⚠ SCREEN OFF IS UNUSUALLY SAFE HERE for the same measured reason as frogger\'s: the game runs on the shared SCHEDULER CLOCK subscribed in the module\'s FACTORY — not in this component, not on rAF, and not gated on the AudioContext (the clock is a Web Worker setInterval) — so collapsing the well stops a drawModtris call and NOTHING else: pieces keep falling, lines keep clearing and LINE/OVERFILL keep firing. `skifree`, one module away in the same family, does NOT have that property. ⚠ NO WATCH MARK: markWatched is a VideoEngine pull-set concept and this is domain audio with two gate outputs.' },
  // ── NIBBLES — the game group's fourth body, and its only VIDEO one ────────
  //
  // ⚠ THE CLAIM THAT SEPARATES IT FROM ITS THREE SIBLINGS IS ABOUT WHERE THE
  // GAME TICKS, and copying frogger's or modtris' paragraph onto it would be
  // wrong. Those two step on the shared SCHEDULER CLOCK inside their factory,
  // so their SCREEN switch is safe by construction. NIBBLES ticks inside
  // `surface.draw` — `dt = frame.time - lastDrawTimeS` accumulates into
  // `tickAccumS` and drives `advanceGame()` — so a collapse that stopped the
  // DRAW would stop the game and silence every output. It does not, and TWO
  // independent things keep that true: the module is structurally pull-exempt
  // (`VideoEngine.isPullExempt` returns true for a handle with a non-empty
  // `audioSources` map, and names NIBBLES in its own comment), and the body
  // renews the watch mark in BOTH screen states, ABOVE the collapse branch, so
  // the module's pull-root status cannot become a function of whether anyone is
  // looking at the picture.
  nibbles: { role: 'picture', why: 'the 320x200 SNAKE GAME SCREEN — the module\'s own CPU-rasterised framebuffer (dark-slate board, red pellet, lime head, green body, every other scanline dimmed ~15%) read as ImageData off `read(node, "snapshot")` and putImageData-ed every rAF, plus its SCREEN switch, its 1x-4x SCALE switch, its RESET button and the ARROW KEYS that are the module\'s only manual steering. ⚠ IT IS THE MODULE\'S IDENTITY AND ITS PLAYING INTERFACE AT ONCE — role="application" with tabindex="-1" (focusable by CLICK, absent from the tab order, so Tab stays the faceplate FLIP gesture and this promotion adds no tab stop inside the plate); `pushDirection` refuses while AUTO is on, so the bot and the hands cannot fight. ⚠ THE CANVAS PAINTS NO TEXT AT ALL, MEASURED rather than asserted: `paintFrame` draws a background, a one-pixel border ring, the food cell, the snake back-to-front and a scanline darken — no fillText, no glyph table, no font. So unlike frogger and modtris there is NO in-canvas HUD, and the card\'s `LEN {n}` chrome row is DELETED by the resting-text ruling rather than relocated; the length, the alive/dead state and the driver reach the a11y tree through aria-label on the screen frame, built by the pure `nibblesScreenLabel`. The card\'s tip sentence is deleted as prose and folded into docs.explanation for right-click ANNOTATE. The body\'s only DOM text is three control captions: SCREEN, RESET, and the scale button\'s own option NAME. ⚠ SCREEN OFF KEEPS EVERYTHING RUNNING, AND HERE THAT MATTERS MORE THAN ON ANY SIBLING, because the GAME TICKS INSIDE surface.draw (tickAccumS / advanceGame) rather than on the scheduler clock: a collapse that stopped the draw would stop the snake and silence the three gates, the length CV and both square waves. It cannot — this component only READS the snapshot, the draw belongs to the VideoEngine, the module is structurally pull-exempt through its non-empty audioSources map, AND the loop renews markWatched in both screen states above the collapse branch so the pull-root status is independent of the picture. ⚠ IT MUST STAY 2-D: nibbles.ts is in the WebGL attest basis and this file is deliberately outside it, so a GL context here would enrol every future face edit in the real-GPU attest. ⚠ AND THE ZOOM SCROLLS RATHER THAN WIDENING THE PLATE — the card does the opposite (width: max-content plus an inline 320*scale), so its 4x zoom makes the card 1280 px wide; the plate here is sized to the 1x preview and everything above it pans inside its own overflow box.' },
  ...
  // is the interesting part — the pre-promotion surface printed TWO resting
  // readouts (`SCENE n/41` and a live speed multiplier like `2.4x`), and
  // neither is
  ...
  // handles. ⚠ NOT on the surface, deliberately: the old
  ...
  tvLibrarian: { role: 'picture', why: 'the STATION TUNER: a live 16:9 picture of the module\'s OWN engine output with its SCREEN switch, and beneath it the two-level browse this module exists for — a clickable equirectangular world map or a country dropdown, then that country\'s channel roster with random / next. \u26a0 THE PICTURE IS THE ENGINE OUTPUT, NOT THE NODE-OWNED <video>, and that is a choice with two consequences rather than a copy of loopback\'s constraint. The output is what `gain` scales and what downstream modules receive, so the one ranked control on this face is visible in the body, which the card\'s raw-element preview structurally cannot show. \u26a0 THE BODY IS LOAD-BEARING, NOT ADDITIVE. tvLibrarian left DOM_SOURCE_LANE_TYPES when its stream became node-owned (LEG-02 P3, #2209), so there is no <HeadlessSourceHost> and under the shell NO card is mounted anywhere — without this surface a promoted tvLibrarian could not be tuned at all. \u26a0 THE ROSTER IS A BODY RATHER THAN A `selector` CELL BY NECESSITY: ShellSelectorCell.options is a pure synchronous (node) => SelectorOption[], and both rosters here are runtime network fetches against a third-party dataset (famelack, hotlinked at runtime, never bundled) with their own loading, error and empty states — and persisting that payload into node.data to make it expressible would sync a volatile third-party blob into every saved rack. \u26a0 WHAT IT PAINTS AS TEXT: the control captions on its own buttons (map / list / random / next / SCREEN), the country NAME as a section label over the list it heads, and one station NAME per roster row with its geo badge and language tag — option names INSIDE the control that selects them, the picturebox per-slot-filename precedent. \u26a0 NOTHING DERIVED IS PAINTED: the card\'s NOW PLAYING label — the station name restated OUTSIDE every control, beside the picture — is DELETED on BOTH surfaces and lives on the picture\'s aria-label; the roster\'s highlighted row, scrolled into view on tune, is the painted answer, and that scroll is what makes the removal safe rather than merely compliant. \u26a0 THE TRANSIENT OVERLAYS STAY (tuning\u2026, stream unavailable \u2014 skipping, and the empty-state placeholder \'pick a country, then a channel\'): outcomes and a placeholder naming the surface\'s own condition, the samsloop NO SAMPLE LOADED shape, each replaced the moment a stream exists. \u26a0 THE ONE TEXT WITH NO DECLARED ROLE IS THE LEGAL DISCLAIMER AND THE FAMELACK / IPTV-ORG ATTRIBUTION, kept because the dataset licence requires it and refused by no gate because body text is this roster\'s stated blind spot — named here as the only fleet text whose justification is legal rather than design, and an open owner question whether that is a permitted resting-text role. peertube carries the same shape.' },
  videobox: { role: 'picture', why: 'the FILE PLAYER: a live picture of the module\'s OWN engine output with its SCREEN switch, and around it the gestures this module exists for — the drop target, the Choose/Pick-another file <label>s (the Chromium showOpenFilePicker path ported verbatim so remembered handles keep working), the one-click re-allow and the re-link overlays, the Play/Pause transport, the seek slider and the corner resize over the card\'s own width/height keys. ⚠ THE PICTURE IS THE ENGINE OUTPUT, NOT THE NODE-OWNED <video> — the tvLibrarian pair of reasons: the output is what `gain` scales and what downstream receives, which the card\'s raw-element preview structurally cannot show. The engine frame is blitted AS-IS (owner decision 2026-08-31 §3: the anamorphic 16:9-into-4:3 upload is a separate platform defect, not letterboxed around here). ⚠ THE BODY IS LOAD-BEARING, NOT ADDITIVE: videobox left DOM_SOURCE_LANE_TYPES in LEG-02 P1 (#1511), so under the shell NO card is mounted anywhere — without this surface a promoted videobox could not load a file at all. ⚠ WHAT IT PAINTS AS TEXT: control captions on its own buttons (Choose video…, Play/Pause, SCREEN, the re-allow button naming the file it re-grants, the re-link label naming the file to re-pick with its size · duration inside that same control), and the FILENAME as the loaded file\'s own caption — the picturebox per-row-filename precedent. ⚠ ONE DERIVED READOUT IS DELETED, NOT HIDDEN: the card\'s `0:04 / 2:00` time line, a derived value painted outside every control. Position survives on the seek slider and its aria-valuetext, and the loaded-file identity lives on the picture\'s aria-label. ⚠ TRANSIENT OUTCOMES STAY: the load-error line (an outcome, replaced on the next load) and the drop-hint placeholder naming this surface\'s own empty condition (the samsloop NO SAMPLE LOADED shape).' },
  peertube: { role: 'picture', why: 'the FEDIVERSE BROWSER: a live 16:9 picture of the module\'s OWN engine output with its SCREEN switch, and beneath it the search-and-pick this module exists for — a rate-limited search box over Sepia Search, the results roster with thumbnails and channel@host·duration, the ↻ next gesture and the Play/Pause transport with its playhead bar. ⚠ THE PICTURE IS THE ENGINE OUTPUT, NOT THE NODE-OWNED <video> — the tvLibrarian pair of reasons: the output is what `gain` scales and what downstream receives, which the card\'s raw-element preview structurally cannot show. ⚠ THE BODY IS LOAD-BEARING, NOT ADDITIVE, and more so than videobox\'s — videobox\'s body rescues a file picker, this one rescues the only way to NAME a video. peertube left DOM_SOURCE_LANE_TYPES in LEG-02 P3 (#1511) and is in neither half of HEADLESS_MOUNT_LANE_TYPES, so under the shell NO card is mounted anywhere: without this surface a promoted peertube could not be searched at all. ⚠ THE ROSTER IS A BODY RATHER THAN A `selector` CELL BY NECESSITY: ShellSelectorCell.options is a pure synchronous (node) => SelectorOption[], and this roster is a runtime Sepia Search fetch with its own loading, error and empty states — and persisting that payload into node.data to make it expressible would sync a volatile third-party blob into every saved rack. ⚠ WHAT IT PAINTS AS TEXT: the control captions on its own buttons (↻ next / Play / Pause / SCREEN), the search box\'s placeholder, and one video TITLE plus its channel@host·duration per roster row — option names INSIDE the control that selects them, the picturebox per-slot-filename precedent. ⚠ NOTHING DERIVED IS PAINTED: the card\'s peertube-now-playing label — the video name restated OUTSIDE every control — is DELETED on BOTH surfaces and lives on the picture\'s aria-label. ⚠ IT IS SOURCED FROM selectionLabel RATHER THAN FROM A HIGHLIGHTED ROSTER ROW, which is the one place this differs from tvLibrarian\'s identical deletion: PEERTUBE_PROFILE.autoLoadCatalogue is FALSE, so a reloaded rack restores the selection with an EMPTY catalogue and no row to highlight; the controller publishes selectionLabel from node.data, which survives that state. ⚠ THE TRANSIENT OVERLAYS STAY (loading…, display unavailable — skipping, and the empty-state placeholder ‘search, then pick a video’): outcomes and a placeholder naming this surface\'s own condition, the samsloop NO SAMPLE LOADED shape. ⚠ TWO TEXTS HAVE NO DECLARED RESTING ROLE AND STAY ANYWAY, and this is the cohort tvLibrarian\'s entry named: the PeerTube / Sepia Search legal disclaimer (an attribution obligation, refused by no gate because body text is this roster\'s stated blind spot) and the per-video attribution ANCHOR to https://<host>/w/<uuid>, which is a navigational CONTROL rather than a readout — the only route from a playing federated video to the creator\'s page, and now the only place the instance host is named at all (the card\'s dead "instance (optional)" input, which nothing ever read, is deleted in the same diff).' },
  mappy: { role: 'picture', why: 'the PROJECTION MAPPER\u2019s composite preview with its CORNER-PIN OVERLAY, and the surface a promoted mappy is actually aligned on. \u26a0 THE PICTURE IS ALSO THE CONTROL, the quadralogical shape: the frame carries a draggable quad per LIVE surface \u2014 pin a corner, drag the quad bodily, focus one \u2014 writing node.data.surfaces[i].corners through the same pure hit-test and in-place Yjs seam the full-window MAP editor uses. \u26a0 WHAT IS HERE BECAUSE IT CANNOT BE A CELL: the SIX INDEPENDENT per-surface FIT/CROP and RESET pairs (a controlFamilies template is ONE cell with no per-member index, so a six-member roster of independent booleans has no cell shape at all), the MAP \u2932 button that mounts the full-window editor, and the export OUTCOME line \u2014 the shell paints a status line for a FILE cell and nothing for an ACTION cell, so the export needs a perceivable home. \u26a0 AND THE EDGES OBSERVER, hand-carried: a ydoc.getMap(\u2019edges\u2019).observeDeep bridge that lives in NONE of the three shared mappy seams, without which live[], the hit-test roster and the editor\u2019s connected prop never see a cable patched while the pane is open. \u26a0 WHAT IT PAINTS AS TEXT: control captions on its own buttons (MAP \u2932, export map, import map, FIT/CROP, reset, SCREEN), the per-surface INn name inside the button that focuses it with its \u25cf video / \u25cb grid state \u2014 the picturebox per-row-filename shape \u2014 the transient map I/O outcome (an outcome, replaced on the next press, and the only thing distinguishing a REJECTED foreign file from a dead button), and the empty-state hint naming this surface\u2019s own condition (the samsloop NO SAMPLE LOADED shape). Nothing DERIVED is painted: how many surfaces are live and how many are fed lives on the picture\u2019s aria-label, and the surface COUNT is a ranked cell with its own roster readout. \u26a0 SCREEN OFF KEEPS THE WATCH MARK, and on this module that is not a nicety: mappy is a mid-chain compositor feeding a projector, markWatched happens INSIDE blitOutputForPreview, and a collapsed branch that stopped blitting would make a control labelled SCREEN black out the stage (#2015).' },
  videovarispeed: { role: 'picture', why: 'the VARISPEED FILE PLAYER: a live picture of the module\'s OWN engine output with its SCREEN switch, and around it every gesture this module exists for — the drop target, the Choose/Pick-another file <label>s (the Chromium showOpenFilePicker path ported verbatim so remembered handles keep working), the one-click re-allow and re-link overlays, the Play/Pause transport, the seek slider, the LOOP\u21941-SHOT toggle, the crop add/edit/remove row with its aspect-locked dragged overlay, and the 7-slot ASSET BANK. \u26a0 THE PICTURE IS THE ENGINE OUTPUT, NOT THE NODE-OWNED <video> — the videobox/tvLibrarian pair of reasons: the engine frame is what the CROP output windows and what downstream receives, letterboxed exactly as they see it, which a raw-element preview structurally cannot show. \u26a0 THE BODY IS LOAD-BEARING, NOT ADDITIVE, AND MORE SO HERE THAN FOR ANY OTHER VIDEO FACE: videovarispeed is in neither DOM_SOURCE_LANE_TYPES nor CARD_PRODUCER_LANE_TYPES, so there is no headless host and under the shell NO card is mounted anywhere — and no ParamCellKind mounts an <input type=file> or a draggable rectangle, so without this surface a promoted videovarispeed would be a video player with no way to be given a video. \u26a0 THE 7-SLOT BANK IS AN ALWAYS-VISIBLE SECTION RATHER THAN THE CARD\'S RIGHT-CLICK SHEET, because right-click is claimed PER-CONTROL by ControlContextMenu (MIDI-learn / automation): an affordance whose only opener belongs to another owner has no opener. \u26a0 WHAT IT PAINTS AS TEXT: control captions on its own buttons (Choose video…, Play/Pause, LOOP/1-SHOT, add/edit/remove crop, SCREEN, the re-allow button naming the file it re-grants, the re-link label naming the file to re-pick), one scale-degree tag and one filename per bank row (the picturebox per-row-filename precedent), and the FILENAME as the loaded clip\'s own caption. \u26a0 TWO NON-CAPTION TEXTS, EACH ARGUED: the SPEED MULTIPLIER beside the scrubber, because the knob\'s law is an asymmetric analog-clock face (\u22124\u00d7 \u2026 +1\u00d7 \u2026 +4\u00d7) whose reading an angle cannot give, kept body-side rather than through a ParamDef.format that would cost a real-GPU re-attest for a readout (owner decision 2026-08-31 \u00a78); and the START-past-END warning, which is the ONLY diagnostic for a transport that genuinely HALTS (resolveWindow returns hasWindow:false, the controller pauses the element every frame, and Play does nothing with no other explanation on screen). \u26a0 ONE DERIVED READOUT IS DELETED, NOT HIDDEN: the card\'s `0:04 / 2:00` time line, the same one videobox deleted. Position survives on the seek slider and its aria-valuetext, and the clip identity lives on the picture\'s aria-label. \u26a0 AND THE WATCH MARK IS RETAINED FOR A REASON THE STATELESS BODIES DO NOT HAVE: this module has TWO video outputs, and CROP is a second pass over its own frame, so a collapsed state that stopped marking would idle BOTH while the element went on decoding (#2015).' },
  ...
  // ── ARCHIVIST — the THIRD and last card-owned-source promotion ────────────
  archivist: { role: 'picture', why: 'the INTERNET ARCHIVE BROWSER: a live 16:9 picture of the module\'s OWN engine output with its SCREEN switch, and under it everything this module exists for — the media-type picker, the search term, the two year bounds, Search, ↻ next, the transport (play/pause, ±10s, a random-position jump and a seek bar), the attribution anchor to the item\'s archive.org details page, and a CLEAN OUT lamp. ⚠ SAME STRUCTURAL REASON AS `cameraInput` AND `loopback`: promotion moves the real card into `<HeadlessSourceHost>`, parked at `left:-9999px` with `pointer-events: none`, so the card is MOUNTED (which is what keeps the three node-owned elements attached and a loaded item playing) but nothing on it is CLICKABLE, and this body is the only surface a player can reach. ⚠ THE CASE IS STRICTLY HARDER HERE THAN FOR EITHER SIBLING, and that is the thing not to carry over by analogy: both of those lose a way to START a source that would otherwise already be streaming, whereas a fresh archivist has NO ITEM AT ALL — `node.data.item` is null until a search writes one and the factory searches nothing on its own — so an unreachable search box does not degrade the module, it makes it a media source that can never be given any media. ⚠ THERE IS NO RESULTS ROSTER TO ARGUE ABOUT, which is where this differs from peertube and tvLibrarian: archivist fetches a page of up to 50 docs and picks a RANDOM one, and the player\'s second draw is the ↻ next button. So no `ShellSelectorCell.options` argument arises at all — what could not be cells here is a free-text TERM, two YEAR bounds and four transport ACTIONS. ⚠ THE PICTURE IS BLITTED FROM THE ENGINE AND NONE OF THE THREE NODE-OWNED ELEMENTS IS ADOPTED: a DOM node has one parent and the card holds all three `nodeMedia` leases, so adopting one here would steal it from the mount that owns the attach. ⚠ AND THE BLIT IS WHY A *VIDEO* ITEM SHOWS NOTHING, which is correct rather than a defect to chase: archive.org serves video without CORS, so `archivist.ts` deliberately never textures a video element and the FBO holds the idle pattern. The item still plays and scrubs and its PLAYING/PLAYHEAD/ENDED jacks still fire; the CLEAN OUT lamp is what says so. ⚠ WHAT IT PAINTS AS TEXT: control captions on its own buttons (Search, ↻ next, Play/Pause, −10s/+10s, ⤭, SCREEN), the four media-type option NAMES inside the select that chooses them, the search box\'s placeholder, the item TITLE inside the attribution anchor that opens it (the picturebox per-slot-filename shape), and two transient overlays plus an empty-state placeholder naming this surface\'s own condition (the loading line, the search failure, and ‘Search the Internet Archive / pick a type + term, press Enter’ — the samsloop NO SAMPLE LOADED shape). ⚠ NOTHING DERIVED IS PAINTED: the card\'s `0:04 / 2:00` line is DELETED on BOTH surfaces (position lives on the scrubber and its `aria-valuetext`, the deletion videobox and videovarispeed already made) and so is its `Internet Archive · {type}` line (the type restated the picker two rows up; the source is the module\'s whole identity). Both live on the picture\'s aria-label. ⚠ THE `play-only (no clean output)` WARNING IS KEPT AS A `StatusLed`, NOT DELETED, and for videovarispeed\'s START-past-END reason: it is the ONLY account a player has of a patched `video` jack delivering the idle pattern, so removing it would leave a dead-looking cable with no explanation of itself. As a lamp its caption is STATIC and the sentence rides on aria-label/title, so no derived text is painted. ⚠ SCREEN OFF KEEPS THE WATCH MARK — archivist is a pure SOURCE feeding `image`, `video` AND `audio_l`/`audio_r`, so a lapsed mark would idle the picture every consumer samples — and SCREEN OFF CANNOT REACH THE PLAYBACK AT ALL, a stronger guarantee than the fleet\'s ordering-dependent one: the element, its `play()`, the 100 ms playhead pump and every gate/CV write are the off-screen CARD\'s, and this body never touches them.' },


  // ── TOYBOX — the four-layer compositor: a PICTURE whose body is also the
  //    module's only surface (2026-09-02, the last unfaced module besides doom)
  toybox: { role: 'picture', why: 'the FOUR-LAYER COMPOSITOR\'S WHOLE CONSOLE: a live picture of the module\'s own engine output with its SCREEN switch, a persistent LAYER band (four layer chips with populated dots and per-layer randomize locks, the KIND select, and the active layer\'s own editor for whichever of gen / frag / obj / image / video it holds), and three tabs under it — CV-MOD (six routed modulation inputs, each with a source-kind badge, a target+param pair addressed by the graph\'s own display names, an attenuverter, an offset and an always-on inline scope), COMBINE GRAPH (the editable node editor: a 17-button ADD row, click-to-patch bezier cables, click-to-delete edges, per-node lock badges, a right-click contextual menu, a canvas whose height persists to node.data.combineView, and the selected node\'s params beneath), and PRESETS (bundled + saved presets, a seeded RANDOM roll that honours locks, REVERT, SAVE, zip EXPORT/IMPORT and the saved list). ⚠ IT IS A PICTURE AND ALSO THE MODULE\'S ONLY SURFACE, which is a harder case than the card-owned-source cohort below rather than an easier one. archivist, cameraInput and loopback each keep their real card MOUNTED in <HeadlessSourceHost>; toybox is in NEITHER half of HEADLESS_MOUNT_LANE_TYPES, so promotion stops ToyboxCard.svelte mounting anywhere at all. There is no parked card to fall back on, and a body that failed to mount would ship a compositor with no way to pick a layer, a shader, an op node or a route. ⚠ IT IS A HOST, NOT A SECOND CONSOLE: the body renders $lib/ui/modules/toybox/ToyboxConsole.svelte with layout=face — the SAME component the console has always been, hosted rather than forked. One component tree, every zone a snippet — so a control cannot exist on one surface and be missing from the other. The body owns exactly two things of its own: the SCREEN button and the previewCollapsed key behind it. ⚠ THE ARRANGEMENT IS OWNER-SPECIFIED (2026-08-28) — keep today\'s capabilities, restyled, with cv-mod / combine graph / presets on three tabs below a screen that turns on and off — so the tab rail is the card\'s two section-collapse toggles restyled, not a redesign. It is BODY-INTERNAL markup and imports no shell tab model: dockTabPlan\'s DOCK_TAB_MIN_BANDS governs param BANDS and this face declares zero. ⚠ NO KNOB HERE EMITS control-*. Knob.svelte derives that testid from the MIDI-learn key, and faces-parity asserts exact multiset equality against this def\'s params, which is []. The face passes Knob\'s own testid override (toybox-dial-*) so the twenty knobs keep their MIDI bindings while emitting no control- id. ⚠ WHAT IT PAINTS AS TEXT is control captions, option names and landmark names: the layer chips (L1..L4), the lock glyphs, KIND/CONTENT/MODEL/MATCAP/SURFACE/MAP/SOURCE/PRESET captions, every knob\'s own label, the three tab names, the 17 op-kind names on the ADD row, the combine graph\'s unique node display names (L1, LUMA 1, DSYNC 2, OUT), IN1..IN6, the per-jack source-kind badge (the port-annotation class — it names what a cable is delivering), the loaded FILENAME of a custom shader / OBJ / image / video (the picturebox per-slot-filename shape, which disambiguates which source a layer holds), and empty/armed invitations bound to an actionable state (LAYER N is empty — pick a KIND above; patched feed — wire a video source into VID A; armed: … click an input dot). ⚠ NOTHING DERIVED IS PAINTED AT REST, and that is true of the card it replaces too rather than something this promotion had to repair: Knob renders its numeric readout only while dragging or hovering, so no value sits under any of the twenty knobs on either surface, and the only measured quantity anywhere is the picture itself plus the six scope traces. ⚠ SCREEN OFF KEEPS THE WATCH MARK, and here that is load-bearing rather than fleet boilerplate: the combine roster includes FEEDBACK, FRAMEDELAY, EXQUISITE and DATAMOSH, all of which accumulate history BETWEEN frames, and out is a real cable other modules sample. The console\'s renewWatchMark runs every rAF on the face whatever the switch says, so OFF stops a GL readback into a surface nobody can see and changes nothing about what is produced.' },
  ...
  mandleblot: { role: 'picture', why: 'the Mandelbrot explorer\'s live fractal and its SCREEN switch. ⚠ TIME-ANIMATED despite holding no accumulator: `uTime * 0.1 * uColorCycle` cycles the palette and `color_cycle` ships at 1, so it is never still at rest. Its only input is a CV, making it the ORIGIN of its chain — a lapsed mark would mute the generator every consumer samples. ⚠ It used to paint a derived magnification readout; this body deliberately does not, and that finding moved into the def\'s `docs.controls.zoom`.' },
  ...
  // prompt is indistinguishable from a broken button, and the pre-promotion
  // surface recorded a one-line hint swap that users did not register.
  //
  // ⚠ NOT ON THE SURFACE, and this is the entry's interesting half: that
  // surface's TWO readout rows. `STATE — RUN / STOP` is the deleted hero strip's
  ...
  // states, which is what the pre-promotion `{#if modeLine !== null}` did.
  //
  ...
  // engine factory, one worklet, one transport and one body, so a second copy
  // of this surface would be a second place for it to drift.
  //
  ...
  textmarquee: { role: 'picture', why: 'the RICH-TEXT EDITOR that is this module — a `contenteditable` document plus its toolbar (align left/centre/right, bold, italic, underline, the per-selection TEXT colour, a 12-entry FONT family picker and the SIZE range), the layer BACKGROUND swatch, the live OUT preview and the SCREEN switch. ⚠ THIS BODY IS NOT A PREVIEW BEING RESCUED, IT IS THE MODULE\'S ONLY WRITER: all four params (ScrlX/ScrlY/PosX/PosY) move the ribbon and none of them says what it READS, which is node.data.richText — thirteen of the card\'s fifteen data-testids are that writer and ZERO of them are expressible as a face cell, because they act on a live DOM Selection, or are `<input type="color">`, or are the document itself. `ShellEntryCell` (#1509) is real and shipped and is emphatically NOT the seam: it parses ONE SCALAR per cell. ⚠ AND RE-HOSTING THE EDITOR IS A SEMANTIC CHANGE UNLESS IT IS STOPPED — the sharpest finding in this promotion. `serializeEditor` reads getComputedStyle, so the editor\'s CASCADE is part of the persisted document: the card\'s `.editor` rule set color:#ffffff and white-space:pre-wrap, and `.dock-ext-body` sets neither and inherits the faceplate\'s var(--text,#eef1f5). A body that copied the markup would have stamped #eef1f5 onto every untouched run, `bold:true` under any ancestor at font-weight>=600 and `center` under any centred ancestor — written into Y.Doc-persisted data, synced to collaborators, rasterized into the video texture and read back by every other reader of that run. The serializer is therefore extracted to $lib/graph/textmarquee-editor (outside lib/video/**, so it costs no attest) and BOTH surfaces stamp EDITOR_BASE_STYLE on the element explicitly. ⚠ IT IS NOT THE PRODUCER: the rasterize-and-push half moved to $lib/ui/media/extras-producers on NODE lifetime in #1720, so a saved rack shows your text with no UI mounted and textmarquee stays correctly out of CARD_PRODUCER_LANE_TYPES; this body writes the MODEL and blits. ⚠ SCREEN OFF KEEPS THE WATCH MARK, on acidwarp\'s argument in its strongest form: textmarquee is a pure SOURCE with no video input, so a lapsed mark would not stall a preview of somebody else\'s picture, it would MUTE the generator every downstream node is sampling. ⚠ WHAT IT PAINTS AS TEXT IS ALL CONTROL CAPTIONS — the six toolbar glyphs, the TEXT / FONT / SIZE / BG swatch labels, the twelve font NAMES inside the select that chooses them, and SCREEN ON/OFF — plus one empty-state badge, TYPE TEXT…, which names the surface\'s own condition (the samsloop NO SAMPLE LOADED shape) and tells the player that the "textmarquee" word in the picture is the factory placeholder rather than their document. ⚠ ZERO RESTING READOUTS WERE DELETED because the card painted none: no value, no measurement, not even a number beside the SIZE slider. ⚠ IT MUST STAY 2-D: textmarquee.ts is in the WebGL attest basis and this file is deliberately outside it, but resolveWebglBasis() step (2) sweeps lib/ui/modules/**/*.svelte by CONTENT, so a getContext("webgl") here would enrol it and put every future face edit on the real-GPU attest critical path.' },

  // ── PAINTER (2026-09-02) — the roster's first DRAWING SURFACE, and the
  //    first body whose canvas is the SOURCE rather than a copy of one ─────
  //
  // ⚠ EVERY OTHER `picture` ENTRY BLITS. Their canvases are 2-D copies of an
  // engine frame that already exists, which is why "SCREEN OFF stops the copy
  // and keeps the watch mark" is the fleet sentence. This canvas is UPSTREAM of
  // the engine: its pixels are uploaded into painter's texture and blitted 1:1
  // into the output FBO, so there is no copy to stop — what OFF stops is the
  // EDITOR, and the node-lifetime producer takes the binding straight back. The
  // role still holds (it mounts a `<canvas>`), and the difference is written
  // down here because a reader checking the fleet sentence against this body
  // would not find it.
  //
  // ⚠ THE TEXT AUDIT: the card painted NO derived value — no size number
  // beside the slider, no colour hex, no op count — so this promotion deletes
  // ZERO resting readouts, and the body adds none. The one measurement that
  // exists (how many ops are in the log) is on the frame's aria-label, and the
  // "N ops / synced" chrome row is refused by name.
  painter: { role: 'picture', why: 'the MS-PAINT EDITOR that is this module — nine tools (pencil, brush, eraser, fill, pick, line, rect, ellipse, text), the 28-colour Win95 swatch grid with left-click FOREGROUND / right-click BACKGROUND, the SIZE range, the FILL toggle, the text-stamp field, UNDO/CLEAR, the drawing canvas and the SCREEN switch. ⚠ THIS BODY IS NOT A PREVIEW BEING RESCUED, IT IS THE WHOLE MODULE: painterDef declares `params: []` AND `inputs: []`, so `face.order` is empty, the faceplate has ZERO bands and there is no cell of any kind for the extension to sit above. painter is in none of DOM_SOURCE_LANE_TYPES / CARD_PRODUCER_LANE_TYPES / HEADLESS_MOUNT_LANE_TYPES, so promotion stops PainterCard.svelte mounting anywhere and a body that failed to mount would ship a drawing module nobody can draw on. ⚠ THE CANVAS IS THE SOURCE, NOT A BLIT OF ONE — see the note above this entry. ⚠ ITS SETUP LIVES IN AN $effect KEYED ON THE ELEMENT, NOT IN onMount, and that is load-bearing rather than stylistic: the SCREEN guard DESTROYS and RECREATES the canvas, and onMount does not re-run, so the sizing, the 2-D context and the engine binding would all stay pointed at a detached element — a blank editor, strokes into nowhere, and ops still committing. ⚠ THE EXTRAS LEASE FOLLOWS THAT ELEMENT: while the canvas exists the body CLAIMS the node binding and pushes its own surface (an in-progress stroke must show on OUT before the op commits); on teardown it RELEASES, and $lib/ui/media/extras-producers re-pushes its own replay of the same op log immediately. There is deliberately no setPaintCanvas(null) anywhere — that would revert the node to the white placeholder, which is the #1720 bug. Both replay the same deterministic log, so the lease changes WHICH canvas is bound and never WHAT is on it. ⚠ THE GESTURE ARITHMETIC LIVES OUTSIDE THIS BODY, in $lib/ui/modules/painter/paint-surface.ts, because a stroke drawn here and the same stroke drawn on any second surface must serialise to a byte-identical PaintOp — the log is valid either way, so a divergence would sync two different pictures to two peers with every gate green. ⚠ SCREEN OFF PUTS THE WHOLE PAINT SET AWAY (toolbar, canvas and palette together) rather than collapsing a preview well beside surviving controls: here the picture IS the instrument, so a toolbar left behind with nothing to draw on would be chrome for a surface that is gone. The output is UNAFFECTED, and the watch mark is RENEWED IN BOTH STATES by the body\'s one rAF — painter is texture-only, so computePullActiveSet can skip it outright, and with the lane tile scrolled out from under an open dock its setCardVisibility(false) demotes the mark and every stroke made after that would sit on this canvas without ever reaching OUT. ⚠ THE Y.Doc-SYNCED COLLAPSE IS SELF-UNDOING: previewCollapsed syncs, so a peer can collapse the module\'s only input device for everyone — and the SCREEN button renders OUTSIDE the collapse, so any peer restores it with one click, which is why painter takes the fleet switch rather than a NO_SCREEN_SWITCH exemption. ⚠ ALL PAINTED TEXT IS A CONTROL CAPTION: the nine tool glyphs (each with its own title/aria-label), SIZE, FILL, UNDO, CLEAR, SCREEN ON/OFF and the stamp field\'s own placeholder. What the USER paints on the canvas, stamped text included, is the module\'s ARTWORK — the GAMES.md in-canvas ruling verbatim: the face paints no text, the player does. ⚠ IT MUST STAY 2-D: painter.ts is in the WebGL attest basis and this file is deliberately outside it, but resolveWebglBasis() step (2) sweeps lib/ui/modules/**/*.svelte by CONTENT, so a getContext("webgl") here would enrol it and put every future face edit on the real-GPU attest critical path.' },
};
 ...
 * A body whose picture comes from a SHARED surface component — so that every
 * host is a mount of ONE renderer instead of two renderers drifting against one
 * DSP — is the shape
