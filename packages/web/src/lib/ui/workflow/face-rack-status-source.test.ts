...
const EXTENSION_BODY_ROLES: Readonly<Record<string, BodyRule>> = {
  ...
  cameraInput: { role: 'picture', why: 'the capture source\'s live preview, its SCREEN switch, the runtime-enumerated DEVICE PICKER and a capture lamp. ⚠ The picker and the lamp are here because neither CAN be a face cell — the device list comes from `enumerateDevices()` at runtime so it is not a `ParamDef` and not expressible as an `options` roster either, and `controlCell` renders a `static` cell as a dead dashed label by design. ⚠ The picture is BLITTED from the engine and the node-owned `<video>` is never adopted: a DOM node has one parent, and adopting it would steal the element from the card that owns `getUserMedia`. The only TEXT is the device NAME (a name, not a measurement) and the lamp carries its state in `aria-label` — the cvBuddy precedent.' },
