...
const EXTENSION_BODY_ROLES: Readonly<Record<string, BodyRule>> = {
  ...
  //
  // ⚠ IT IS ALSO THE FIRST ENTRY WHOSE MODULE IS A MIDI BINDER. The three
  // shipped binders in this cohort (midiclock, ptzcam, midiCvBuddy) are all
  // `status-primitive` bodies — a picker, some lamps, an error line. This one is
  // those PLUS a canvas, so the declared role is `picture` and the lamp is a cell
  // on it rather than the surface's purpose. Both predicates would hold (the body
  // imports `StatusLed` too); the gate checks the DECLARED role only, and this
  // roster's own header says the roles are not exclusive by intent.
