  ...
  const CARD_SOURCE_FACED: Readonly<Record<string, CarriedAffordances>> = {
    archivist: {
      why:
        ...
        + 'and the transport are ACTIONS rather than values, and `controlCell` renders a `static` '
        + 'cell as a dead dashed label by design. ⚠ THE TILE SLOT IS FILLED TOO, because '
