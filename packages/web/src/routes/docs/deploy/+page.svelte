<svelte:head>
  <title>deploy — patchtogether.live docs</title>
...
<table>
  <thead>
    <tr><th>tier</th><th>web URL</th><th>CF Pages project</th><th>Fly Hocuspocus app</th><th>Neon branch</th><th>beta gate</th></tr>
  </thead>
  <tbody>
    <tr>
      <td>prod</td>
...
<ul>
  <li><strong>PR open / push</strong> → preview at <code>pr-N.patchtogether-live-autotest.pages.dev</code>. Inherits the autotest Clerk env so auth round-trips work.</li>
  <li><strong>merge to main</strong> → autotest + dev fan-out, in parallel. Both stay in sync.</li>
  <li><strong>version bump on main</strong> → prod deploy. Detected by diffing <code>package.json:.version</code> against <code>HEAD~1</code>.</li>
  <li><strong>workflow_dispatch</strong> → manual override. Requires the latest CI run on the chosen branch to be green.</li>
</ul>
...
<ul>
  <li><code>pg.Client</code> over TCP — fails. Workers' <code>node:net</code> shim returns <em>"proxy request failed"</em>; <code>pg</code>'s socket layer doesn't speak <code>cloudflare:sockets</code>.</li>
  <li><code>@neondatabase/serverless</code> WebSocket <code>Pool</code> — fails. CF's egress proxy 403s the outbound WebSocket handshake.</li>
  <li><code>@neondatabase/serverless</code> HTTP <code>neon</code> template tag — works. <code>fetch()</code> under the hood.</li>
</ul>
<p>
  ...
  server on Fly is regular Node — it uses <code>pg.Pool</code> over TCP without trouble.
</p>

<h2>Beta gate</h2>
<p>
  Basic-auth wrapper enforced in <code>hooks.server.ts</code>. Off in prod, on in autotest + dev.
  <code>/api/health</code> bypasses the gate (live-smoke + uptime probes), and the
  <code>/docs/*</code> tree is also exempt so contributors can read the docs without a
  credential prompt. Browser auth + session flows still work behind it because Playwright
  passes credentials via <code>use.httpCredentials</code>.
</p>
...
<p>
  Hocuspocus binds 1235, not 1234 — BitwigStudio reserves 1234 for OSC on this machine.
