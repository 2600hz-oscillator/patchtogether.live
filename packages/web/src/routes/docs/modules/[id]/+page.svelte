...
{#if data.face}
  <!-- THE CARD — a clean screenshot of the real rendered card, generated by the
       VRT card-face pipeline. The primary visual; every control is documented in
       the authored "controls" table below (not a raw-testid legend). -->
  <h2>the card</h2>
  <div class="face-wrap" data-testid="module-face">
    <img
      class="face-img"
      src={data.face.src}
      alt={`The ${mod.label} card`}
      loading="lazy"
    />
  </div>
{:else}
