...
<section class="docs-hero">
  ...
  <div class="docs-sub">Multiplayer browser-native modular synthesizer. Patches are CRDT-shared; audio runs locally.</div>
</section>

<p>
  Performers patch a shared rack live in their browser. The patch graph is the canonical state &mdash; audio (today)
...
<p>
  ...
  <a href="https://hocuspocus.dev" rel="noopener">Hocuspocus</a> server on Fly.io brokers updates between
  rackspace participants and persists snapshots to Neon Postgres. Auth is Clerk; an HMAC-derived
  invite code (<code>/r/[id]?invite=...</code>) lets anonymous users join without an account.
...
<h2>DSP pipeline</h2>
<p>Two flavors of audio module sit behind a uniform <code>AudioModuleDef</code> registry:</p>
<ul>
  <li><strong>Faust &rarr; WASM</strong>. Most generators / filters / effects are
  ...
  <li><strong>TS AudioWorklets</strong>. Modules that need clock arithmetic, lookahead schedulers,
    or buffer state in JS land &mdash; LFO, Wavetable VCO, TIMELORDE, CHARLOTTE&apos;S ECHOS &mdash; ship as
...
<p>
  ...
  Workers &mdash; see <a href="/docs/deploy">deploy notes</a>). Hocuspocus on Fly runs Node
...
<p>Three tiers, fan-out from a single repo:</p>
<ul>
  <li><strong>prod</strong> &mdash; <code>patchtogether.live</code>. Gated on a
    <code>package.json:.version</code> bump in a merge commit.</li>
  <li><strong>autotest</strong> &mdash; <code>autotest.patchtogether.live</code>. Auto-deploys on every
    push to <code>main</code>. Beta-gated (basic-auth <code>beta:robotsonly</code>).</li>
  <li><strong>dev</strong> &mdash; <code>dev.patchtogether.live</code>. Same as autotest, separate Hocuspocus
...
<ul>
  <li><a href="/docs/modules">Module catalog</a> &mdash; every module, its I/O, params, source link.</li>
  <li><a href="/docs/testing">Testing</a> &mdash; unit / ART / E2E layers.</li>
  <li><a href="/docs/deploy">Deploy + ops</a> &mdash; the 3-tier flow, Workers&harr;Postgres caveats.</li>
</ul>
