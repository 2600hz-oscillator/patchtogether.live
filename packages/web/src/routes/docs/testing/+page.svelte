<script lang="ts">
  let { data } = $props();
  let artScenarios = $derived(data.artScenarios);
  let artBaselines = $derived(data.artBaselines);
  let vrtImplemented = $derived(data.vrtImplemented);
  let e2eSpecs = $derived(data.e2eSpecs);
</script>

<svelte:head>
  <title>testing — patchtogether.live docs</title>
...
<p>
  ...
  <strong>VRT</strong> → <strong>E2E</strong>. The PR gate (<code>flox activate -- task ci</code>) runs
  typecheck + unit + ART + E2E. VRT is not yet implemented (see below).
</p>

<h2>Unit</h2>
<p>
  Vitest, fast, scoped per workspace. Lives next to the code (<code>foo.ts</code> ↔ <code>foo.test.ts</code>).
  Run all unit tests:
</p>
...
<p>
  Offline render of compiled DSP through <code>node-web-audio-api</code> →
...
<h3>Scenarios ({artScenarios.length})</h3>
<table>
  <thead><tr><th>group</th><th>spec</th></tr></thead>
  <tbody>
    {#each artScenarios as s (s.path)}
      <tr>
        <td>{s.group}</td>
...
{#if artBaselines.length > 0}
  <h3>Baselines ({artBaselines.length})</h3>
...
{:else}
  <p class="banner">No ART baselines committed yet — most ART scenarios assert on FFT/spectrum properties rather than byte-exact PCM.</p>
{/if}
...
<h3>Specs ({e2eSpecs.length})</h3>
<ul>
  {#each e2eSpecs as s (s)}
    <li><code>{s}</code></li>
  {/each}
</ul>
