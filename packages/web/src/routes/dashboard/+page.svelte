<script lang="ts">
  ...
  // ONE create card, ONE rack kind. (There were two, selecting the deleted
  // `racks.mode` column; the shell is now the only shell.)
...
<div class="root">
  ...
  <main>
    ...
    <!-- ⚠ THE "Saved groups" SECTION IS GONE with the GROUP! module: the
         library it listed (`saved_groups`, `/api/saved-groups`) is deleted, so
         the dashboard's only surface is the rackspace list again. -->
  </main>
</div>
