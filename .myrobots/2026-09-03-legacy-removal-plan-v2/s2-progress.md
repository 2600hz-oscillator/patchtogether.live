...
| … | NEW QUEUE (surfaced by upstream promotions, RED on default at HEAD): control-surface.spec.ts:196 + electra-control.spec.ts:146 use the `rack` fixture but drive card DOM — controlSurface/electraControl were promoted on main AFTER S0 classified these files; drain 13 re-points all their tests | in progress |
