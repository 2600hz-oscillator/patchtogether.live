...
| 39 | drain 15 — the TIMELORDE family + trails (4 files, 13 tests): tap = the DOCK's `shell-cell-timelorde-tap` (⚠ NOT on the tile — it ranks only bpm/running/mute; the dock paints every face page at once, no tab click). ⚠ TWO in-page dispatch traps: the shell ACTION CELL fires on POINTERDOWN (Button.svelte), a synthetic `.click()` never reaches it; and its handler calls `setPointerCapture` BEFORE the trigger, which THROWS on a synthetic pointer — tapInPage shims the capture call for its no-CDP-between-taps driver. ⚠ PRODUCT PARITY PORT: the Space-taps-tempo shortcut lived ONLY in TimelordeCard's window listener (dead on the shell — the headless copy reads xyflow selection as never-selected) while the tap cell's title PROMISES it; Canvas now owns a window Space listener for the selected timelorde (⚠ selection is XYFLOW-INTERNAL — Canvas dropped bind:nodes, so read `flowApi.getNodes()`, never `flowNodes`). External-clock test retitled + rewritten: the face never disables the tap (documented, face-tap.ts) — the surviving claim is the follower RE-OWNS the bpm (tap ~300 never sticks). transport-state: the card's one-strip `data-transport-state` → the tile readout PAIR (`readout-running` RUNNING/STOPPED × `readout-muteOutputs` GATES LIVE/MUTED = the same four states, def vocabulary) + the engine read; ⚠ FACE DELTA measured: `control-running` does NOT step aside when transport-slaved (the card hid RUN) — the legibility the hiding protected is what the readouts assert. timelorde-video: display = dock `timelorde-face-canvas`, mode = the display frame's aria-label narration mapped to video/wizard/off; videoOut read = the tile thumb. trails: dock `trails-face-*` (near-parity testids) + `shell-cell-trails-connect`; resting hint = `.hint`/status under the LINK lamp. ⚠ MY OWN TRAP ×2: perl rewrites silently emptied `${...}` interpolations — grep for `data-id=""`/`data-pane-node=""` after EVERY scripted edit | DONE — 13/13 once, 9/9 + 45/45 REPEAT=3, positive control (four-state setParam suppressed → poll red; restored → green), 1 orphaned wait row accepted (the converted 400ms), lint 0, typecheck 0, e2e tsc 0, --list 3045/502 |
| 40 | drain 16 — the VST pair (vst-bridge 2 tests serial + vst-lane-autowire 1): the face body is `fullViewBody` (dock pane, per-node `vst-face-*-{id}` testids); the card's THREE text rows live VERBATIM in the lamps' aria-label sentences (vst-status-model.ts) — status → BRIDGE lamp (/mock-vst-bridge/), mounted-name → PLUGIN lamp, state-size → the PLUGIN lamp's clause, whose face wording is "state travels with the patch" (not the card's "state saved in patch"). ⚠ PRODUCT FIX (proxy-identity class, THIRD sighting): VstBridgeFaceBody's `persisted` was a bare `$derived(node?.data?.vst)` — the PLUGIN lamp kept saying "nothing is saved in the patch yet" over a populated node.data.vst (measured); now pumped by `nodeVersion(nodeId)` (the TimelordeDisplayBody cardVersion pattern). Red-before/green-after on the shipping lamp = the control | DONE — 3/3 once, 9/9 REPEAT=3, typecheck 0, e2e tsc 0 |
| 41 | drain 17 — coverage-group-1-sinks (6 tests) + coverage-groups-6-7-8-9 (27): three legacy boots flipped; sticky's per-type class KEPT (sticky is NON_SHELL_LANE — the card still renders on the default shell; S4 burn-down reconciles the test with the module); cameraInput/picturebox mount asserts → `:has([data-shell-type])`. ⚠ EIGHTEEN of 6789's tests were SILENTLY RED AT HEAD — every `video-out-canvas` reader: the card canvas does not mount on the default shell (MEASURED via a direct probe: count 0 after spawn; videoOut promoted upstream after S0 classified the file). The file-local `readVideoOutCanvasStats` now reads the videoOut TILE THUMB (the fleet seam). ⚠ TRIAGE LESSON: `tail -N` on a line-reporter run EATS the failed-list — always grep for `N (passed|failed|flaky|skipped)`; two earlier runs of this file were misread as green. ⚠ Desktop-load contention (owner active, load 13, WindowServer pegged) also flakes video batches — run video files --workers=1 and re-check under load before blaming a spec | DONE — 33/33 once, 99/99 REPEAT=3 (workers=1), positive control (thumb locator broken → integration red; restored → green), e2e tsc 0, --list 3045/502 |
| 43 | WAVE-2 FLIP MEASUREMENT (the honest instrument, 64 leg-only files + 6 mixed, preview @4752, workers=2, 26.3m): **50 passed / 205 failed / 8 skipped / 3 did-not-run — 61 of 64 red.** The wave-1 easy pool is EXHAUSTED; what remains is the deep per-module card-DOM population. KEPT FLIPPED (each verified + REPEAT=3, 21 passed ×run + park-skips): clipplayer-play-every, instrument-exposed-port-patching, workflow-surfaces. REVERTED to legacy — the successor's per-family queue with MEASURED failing-test counts: archivist 3 (`archivist-card` testid), b3ntb0x 3, backdraft 5, backdraft-full-output 5, bentbox 3, blood-mount 1, cellshade 4, clipplayer-edit-launch 2, clipplayer-pitch-probability 2, colourofmagic 1, control-color 3, dx7 4, edges 3, frametable 2, gamepad 17, grains-of-vision 1, lushgarden 2, mapper 2, mappy-export-import 2, mappy-output 7, multi-video-playback 2, onetonine 1, outlines 3, outlines-render-smoke 1, peertube 3, picturebox-gif 2, picturebox-sync 1, present-second-display 2, quadralogical 4, quadralogical-assign 2, recorderbox 1, render-worker-acidwarp 4, render-worker-toybox 3, sourcery 1, tempest-render-smoke 1, textmarquee 3, tiler 2, tv-librarian 2, tv-librarian-audio 1, twotracks 31, twotracks-perfzip 1, varispeed-multislot-persist 1, vfpga-p3-composite 8, video-audio-output 2, video-fullscreen 5, video-fullscreen-multimonitor 3, video-preview-gate 2, video-pull-eval 3, videobox-output 2, videobox-performance-bundle 1, videobox-upload-perf 1, videocube 2, videocube-assign 1, videovarispeed-crop 1, videovarispeed-output 5, videovarispeed-perfzip 2, videovarispeed-switch 1, warrensvisions 4, wavesculpt 10, workflow-channel-columns 7, workflow-mode 1. Dominant sampled shapes: `<type>-card` testids, card canvases, card chrome. MIXED-6 findings: node-source-hls's legacy goto belongs to a LEGACY-SUBJECT test and freezeframe-screen-toggle's to a "LEGACY CARD surface" describe — both RESTORED untouched (S5 escape-hatch arms, the face-clipplayer class); matrixmix/unpatch-patch-point/video-full-frame/video-hide-controls carry real card legs (queue; measured legs: matrixmix GRAY-✕ fanout accept, unpatch rear-card menu, full-frame ×4, hide-controls ×5). EXCLUDED by classification (not failures): grouping-phase1/2/3 + saved-groups-wiring + sticky-note (S4 burn-down), per-module.spec + per-module-per-port-* + io-spec-consistency (budget-coupled sweep phase; ⚠ inputs sweep asserts VERBATIM CARDS via rack-session's LEGACY_RACK_URL worker option), card-control-overflow (family c), workflow-dock (has a legacy-subject test), workflow-dock-collab (@collab), face-* legacy arms | DONE — e2e tsc 0, --list 3045/502 |
| 44 | drain 18 — the MAPPY pair (9 tests; mappy faced on main 2026-09-02, near-parity): export/import = the dock cells (`shell-cell-mappy-export-map` / `-import-map` — the FILE input IS the cell testid), status = `mappy-face-map-status` (data-status-kind carries), count readout `mappy-count-n` DIED → the SURFACES segmented radio (aria-checked) + legend-2; editor opens from `mappy-face-open-editor` (SAME MappyEditor, every `mappy-editor-*` testid unchanged); overlay geometry = `mappy-face-canvas` + `mappy-face-handle-1-<i>`; composite reads = the videoOut TILE THUMB (node-scoped — every video tile paints one). ⚠ SPEC-PREMISE DEFECT FOUND, measured at the ENGINE seam (`vid.outputTexture` + readPixels — the shell-agnostic instrument): the tests' `tintMix: 1` means PURE TINT on the reworked CHROMA (documented "1 = pure tint") — a FLAT colour fill, variance 0 at chroma.out AND mappy.out on BOTH boots; the legacy run stayed green only because the CARD canvas letterboxed the flat frame and the variance assert measured ITS OWN BLACK BARS (ask why green is green, again). tintMix 0.6 restores real structure through the blend. ⚠ MY OWN TRAP #3: a scratchpad copy of the rewritten file was taken AFTER arming a temporary legacy-boot control — restoring it silently re-armed legacy and cost three diagnostic loops chasing "missing thumbs" that were just cards-in-lane; diff the goto FIRST when a rewrite mysteriously regresses | DONE — 9/9 once, 27/27 REPEAT=3, positive control (import upload suppressed → count poll red; restored → green), e2e tsc 0, --list 3045/502 |
| 45 | drain 19 — vfpga-p3-composite (8) + the QUADRALOGICAL pair (6): vfpga-p3 mirrors the already-proven p4 shapes (preset SELECTOR cell + `.val` readout, videoOut tile thumb node-scoped, `:has(shell-type)` counts; `video-out-card` → shell-type count; luma-plane saturation POLLED — the thumb blit lags a rewire, one-shot reads caught the previous colourful wiring intermittently). quadralogical: the screen body is dock-only; the card's bespoke 2-axis menu died — the face's ASSIGN x/y buttons open the SHARED ControlContextMenu (`ctx-surface-*` / `ctx-electra-*-row-*-knob-*` / `-clear`); pad/diamond/dot → `quadralogical-face-{quadrants,diamond,puck}`; pixel reads were already engine-FBO (shell-agnostic); mount-smoke `video-out-canvas` counts → the v-out SHELL TILE (render-paused tests can't rely on a thumb blit). ⚠ TWO PRODUCT FIXES (both measured red-before/green-after): (1) the joystick FIELD's pointerdown had no target guard — a LEFT CLICK on the x/y assign buttons YANKED the puck to the button's corner (destructive commit) AND captured+prevented the pointer so the click never synthesised: the menu was UNREACHABLE by left click while the buttons' titles promise "right-click or click" (now: presses on buttons inside the field are ignored by the drag handler); (2) the face's send/assign dropped the card's "QUAD X"/"QUAD Y" preset names (the generic seam sends unnamed) — the body now wraps the shared handlers with the card's two-call sequence (setBindingName / setSlotName). ⚠ Probe lesson: a plain-onclick button that responds to dispatchEvent('click') but NOT to a real Playwright click = something upstream captured the pointer — that divergence IS the defect, not a test problem | DONE — 14/14 once, 24/24 + 18/18 REPEAT=3, controls = the two product fixes (menu unreachable → reachable; names absent → present), lint 0, typecheck 0, e2e tsc 0, --list 3045/502 |
| 46 | drain 20 — the VIDEOVARISPEED cluster (5 files, 10 tests; near-parity face): the transport body is `fullViewBody` carrying the card's `videovarispeed-*` testids VERBATIM incl. the state attributes (now on `videovarispeed-face-body`); every interaction pane-scopes (openVvsPane, idempotent, called from the load helpers so reloads/clears re-open). DIED WITH THE CARD: the right-click multi-panel reveal + `videovarispeed-multi-close` + the lingering ctx-overlay dance (the 7-slot bank is ALWAYS-ON in the dock face), the START/END range inputs (the switch spec's setWindow was SETUP, not subject → store-seam param write; the window GESTURE stays pinned by samsloop-window's knob recipe), and the perfzip contextmenu gestures. picturebox side: file input = dock `picturebox-face-file-input`; ⚠ `data-has-image` lives on the face CANVAS, not the body root — aim there. ⚠ PRODUCT FIX (proxy-identity, 4th sighting, CONTROLLED: un-pumped build reads `data-has-image="false"` over live bytes in the doc; pumped build flips to "true" — the revert WAS the control): PictureboxAssetsBody's six node.data leaf deriveds now ride the nodeVersion pump. ⚠ perfzip/persist reloads close the pane with the cleared node — REOPEN after restore | DONE — 10/10 once, 30/30 REPEAT=3, positive control (slot upload suppressed → persist red; restored → green), typecheck 0, e2e tsc 0, --list 3045/502 |
| 47 | drain 21 — videobox trio + videocube pair (5 files, 11 tests): videobox mirrors varispeed (state attrs on `videobox-face-body`, dock picker/play, `videobox-video` registry-owned so it exists on the shell; performance-bundle reopens BOTH panes after the rack clear, and picturebox's `data-has-image` reads the FACE CANVAS). videocube: presence → shell-type/`videocube-output-body`; the three card joystick pads → SHELL XyPads whose testid is `control-<xParamId>` (slice=control-slice_rx@tab-slice, view=control-view_rot_x@tab-view, scan/spread=control-scan@tab-reader — videocube's dock is TABBED, inactive pages display:none); the card's zoom-out drag dance died. ⚠ XyPad ANATOMY (measured, saved me from shipping a false product claim): the per-axis ASSIGN buttons are the pad's SIBLING `.xy-assign` group OUTSIDE the drag surface — no destructive-click defect there (the quadralogical body's inline layout was the outlier; an XyPad guard I drafted was DEAD CODE and got reverted). videocube-assign drives `following-sibling::div[.xy-assign] .xy-assign-btn` per axis → the shared ctx menu → cs-board proxies/dials; the card's left-click-opens claim survives NATURALLY (button outside pad) incl. a no-write assert | DONE — 11/11 once, 33/33 REPEAT=3, positive control (play click suppressed → upload-perf red; restored → green), typecheck 0, e2e tsc 0, --list 3045/502 |
| 48 | drain 22 — the SINGLES cohort (10 files, 31 tests: sourcery, tempest-render-smoke, colourofmagic, grains-of-vision, onetonine, tiler, cellshade, warrensvisions, outlines, outlines-render-smoke): the wave-2 1–4-failing tail, all card-DOM presence/readback shapes → shell-type counts, `control-<param>` reads, videoOut tile-thumb pixel reads. Per-file notes: cellshade's dead `cellshade-bits-readout` → `control-bits` aria-valuetext (/6/); onetonine's toHaveCount(2) card assert was nonsense on the shell (o1/o9 are separate nodes) → split into two count(1) asserts; warrensvisions/outlines/tiler composites read the tile thumb; a batch regex pass over-rewrote canvas READ locators into module-shell (getContext errors) — repaired by re-targeting only evaluate sites. ⚠ colourofmagic '22 outputs': only face page 1's controls mounted — NOT a bug: the module DECLARES `face.bandFocus` on PREVIEW (colourofmagic.ts:744) — the face focuses one colorspace block's band per the PREVIEW selection, and PASS (preview=0, `showAllOn: [0]`) is the deliberate show-all state. The spec now drives preview→0 through the store seam and then asserts one representative control per block (bias_r/bias_db/bias_h/bias_yiq_i/bias_ycc_cb) — the same drive-to-showAll move the faces-parity sweep makes. Red-before/green-after on that drive = the control. Wait-ledger: one orphan removed (videocube joystick row from drain 21's committed rewrite) via lint:waits:accept | DONE — 31/31 once, 93/93 REPEAT=3 (workers=1), lint PASS, e2e tsc 0, --list 3045/502 |
| 49 | drain 23 — the dock-body sextet (6 files, 11 tests: textmarquee, multi-video-playback, tv-librarian, tv-librarian-audio, picturebox-gif, picturebox-sync @collab): all five modules' interactive surfaces are `fullViewBody` (dock-only) — every rewrite opens the pane via `__openDockFullView` and pane-scopes (testids repeat pane-to-pane; panes are LRU'd so each sequential load opens its own and finishes inside it). textmarquee: editor/toolbar in the pane (`textmarquee-editor-body`), card-count asserts → shell tiles. tv-librarian: picker+tuner in the pane, `data-stream-state` on `tv-librarian-face-body`; `tv-video` is REGISTRY-owned (node-hls-source-registry) so raw-element probes stay document-level. picturebox: face file input, `data-has-image` on the face CANVAS; the card's painted 'gif'/'synced' state word is DELETED (resting-text ruling) — the fact moved to the canvas aria-label ('animated gif preserved frame-for-frame' vs 'synced at W×H'), asserted there. picturebox-sync opens the pane on BOTH collab pages. ⚠ INSTRUMENT LESSON (multi-video + picturebox-gif): the lane tile thumb is VISIBILITY-GATED BY DESIGN (VideoTileThumb's IntersectionObserver — an offscreen tile stops blitting), so the card-era offscreen-canvas pixel probes CANNOT re-point at thumbs for nodes laid out beyond the viewport (measured: out2/out3 frozen at luma 6.3 while uploadCount advanced). All pixel probes moved to the ENGINE seam (`vid.outputTexture(producer, port)` + readPixels — probe the PRODUCER: varispeed 'video', mixer 'out', picturebox 'out'); the dock is Escape-closed after loads so local visual runs see the lane. Positive control: vv3's play write suppressed → uploadCount guard red (deltas {49,49,48,0}); restored → green | DONE — 11/11 once, 33/33 REPEAT=3 (workers=1), lint PASS, e2e tsc 0, --list 3045/502 |
| 50 | drain 24 — the browse-body quintet (5 files, 15 active tests: archivist, peertube, edges, frametable, blood-mount; edges' thickness leg stays its pre-existing #106 fixme): archivist's shared BrowseControls re-prefix to `archivist-face-*` in the dock pane, state attrs 1:1 on `archivist-face-body` (has-item/media-type/clean-output/is-playing), `archivist-image`/`-video` stay document-level (registry-owned, node lifetime). peertube: picker+preview pane-scoped, identity read off `peertube-face-preview` aria-label. ⚠ PRODUCT ADD (parity attr, archivist pattern): `data-has-selection` stamped on `peertube-face-body` — the card carried it, the body did not; the spec observes the live false→true transition. edges: readEdgeStats → engine seam on `edg:out` (interior 70% crop kept); `.svelte-flow__node-edges` per-type class GONE on the shell — presence = data-id + module-shell. frametable: already engine-seam; the card mode-row/preview asserts died with the card; the Load-table FILE CELL is `shell-cell-frametable-file-input` (family id, NOT key-minus-braces) with status span `<testid>-status` ('loaded N frames WxH'); ⚠ the cell input is HIDDEN behind its label — scrollIntoViewIfNeeded waits on visibility FOREVER, setInputFiles works directly (dx7 recipe). blood-mount: boot state rides `data-blood-status` on `blood-face-frame` (idle/loading/error/ready) — the three visibility probes collapse to one attribute poll; data-missing → `blood-face-data-missing` in the pane. ⚠ chainNodes' videoOut id is `v-out` (hyphenated) — a `vout` locator matched nothing | DONE — 15/15 once, 45/45 REPEAT=3 (+3 fixme skips), typecheck 0, lint PASS, e2e tsc 0, --list 3045/502 |
| 51 | drain 25 — the CRT/keyer/colour quintet (5 files, 13 active tests: b3ntb0x, bentbox, control-color, lushgarden, mapper; lushgarden+mapper each keep their #1847-parked fixme byte-identical — the parks phase owns those bodies): b3ntb0x/bentbox t1 were already DRS engine-seam — only presence asserts moved to shell tiles. bentbox resize: the face handle sizes the PREVIEW in the dock pane writing the SAME data.width/height keys — the spec polls the STORE (the pane's max-width:100% can clamp the painted wrap) and allows the visual clamp; ⚠ hover() BEFORE the drag — a boundingBox taken below the pane fold hands mouse.down to whatever paints at those coordinates (measured: store never written). control-color: menu = the SHARED NodeContextMenu (right-click `.tile-kind`); the card's colour dot (ModuleTitle) died — the shell's at-a-glance receipt is the SURFACE lane tile's SVG swatch (`cs-tile-swatch-cs-1-adsr-1`, fill=#RRGGBB passthrough) + the dock board stripe `cs-board-stripe-<mod>-<param>`; test 2 stays lane-only (the dock overlay would block tile right-clicks). lushgarden: [GATED] badge died — the latch it painted is the `growPatched` engine probe (already asserted); preview reads moved to `lushgarden-face-canvas` in the pane. mapper: readKeyStats → engine seam `map:out` (interior 70% kept). Positive control: mapper's key edge suppressed → 'shows the colour VIDEO' red; restored → green | DONE — 13/13 once, 39/39 REPEAT=3 (+6 park skips), lint PASS, e2e tsc 0, --list 3045/502 |
| 52 | drain 26 — the presenting/pull-eval sextet (6 files, 15 active tests: video-audio-output, twotracks-perfzip, present-second-display, video-preview-gate, video-pull-eval, recorderbox; recorderbox's #105-quarantine fixme kept byte-identical): video-audio-output + twotracks-perfzip were locator flips (varispeed pane recipe / shell-tile presence). present-second-display: the present gesture right-clicks `videoout-face-canvas` in the dock pane — the SAME shared VideoCanvasContextMenu (`ctx-present-*`) the card mounted; popup blit pipeline unchanged. ⚠ video-preview-gate SEAM FINDINGS (both measured): (1) `previewGateStats()` decisions are the LEGACY CARD LOOP's instrument (`blitOutputForPreview`) — the shell's thumb releases its loop at its own IntersectionObserver BEFORE any engine decision is recorded (undefined IS the refusal; asserted as undefined-or-skip:offscreen + cardVisible false), and the face bodies present via `blitOutputToDrawingBuffer` (ungated funnel) so the dock-lease leg asserts BLIT CALLS continue, not a decision entry; `animation()` now takes a full node-scoped selector (thumbs repeat per-tile). (2) video-pull-eval's presenting leg: Canvas.svelte holds a HARD LEASE PER OPEN VIDEO PANE by design (Canvas.svelte:2877 — 'for as long as the faceplate is open'), so an open pane leases REGARDLESS of data.fullFrame — chased as a lease leak for three probe loops until the acquire stack (Array.map in the Canvas $effect) named the second holder; the leg now opens the pane as THE presenting surface and the release control is CLOSING it (lease gone → skipped), with the body's own fullFrame/fs/present leases left to the fullscreen/full-frame specs. recorderbox: face prefix `recorderbox-face-*`, badges are StatusLed lamps (`-no-encoder` data-lit), folder row 1:1 incl '(chosen on record)'. Positive control: video-audio-output's aout edge suppressed → terminal peak 0.0000 while scope read 0.5132 (the documented false-confidence gap, visible) → restored green | DONE — 15/15 once, 45/45 REPEAT=3 (+3 park skips), lint PASS, e2e tsc 0, --list 3045/502 |
| 53 | drain 27 — the BACKDRAFT pair (2 files, 12 tests): backdraft.spec's pixel probes moved to the SINK's engine surface (`vid.outputTexture('v-out')` — 'chain reaches OUTPUT' preserved: it is the sink's own texture, not bd's FBO; ⚠ readPixels rows are BOTTOM-origin, the mirror grid flips y so the kept-half check keeps its visual meaning). Card anatomy that died: the mounted-but-hidden `backdraft-canvas` 0×0 ghost, the rack-tier geometry (rack-sizing.test.ts owns it), the handles-hidden-in-full-frame CSS. CLK badge → the shell's `face-override-badge-delay` (param-override-badges.ts — SAME shared predicate), which lives on the DOCK ladder's 'loop' TAB (probed: zero delay cells in the lane; inactive pages display:none → click faceplate-tab-loop first). ⚠ mirror-gate leg re-pointed to BEHAVIOR: the store write-back was the CARD's rAF reflection (its comment said so) and no shell surface re-persists the engine latch — the spec now measures X-symmetry appearing on edge 1 and leaving on edge 2 with the same grid instrument (anti-vacuity floor 8; measured base asym 11.33, folded ~2). backdraft-full-output: the output modes live in BackdraftOutputBody (pane) — ⚠ THE RESTING SURFACE IS REAL there (body's own note), so the card's 0×0 asserts INVERTED to present+visible; full-frame = body-root class + the SAME persisted data.fullFrame + dblclick exit; fullscreen state machine + mutual exclusion + present gating unchanged via the shared menu; the no-lane-display right-click leg now guards the TILE (module menu opens, output menu does not). Positive control: e_out suppressed → 'spatial structure' red; restored → green | DONE — 12/12 once, 36/36 REPEAT=3, lint PASS, e2e tsc 0, --list 3045/502 |
| 54 | drain 28 — render-worker pair + dx7 (3 files, 12 active tests; dx7's #1787 suspected-live-regression fixme untouched): both render-worker outputStats moved to the SINK's engine surface (`outputTexture('out')`/node-scoped — the WorkerProxyHandle claim is unchanged: the sink surface carries what the proxy texture routed downstream). ⚠ SEAM FINDING (#1905 leg, measured): the appended worker-resident chain read BLACK because NOTHING OBSERVED it — the legacy video-out card's blit loop marked its node watched unconditionally, the shell thumb is viewport-gated and the appended pair lands outside the first spawn's fit — so the leg opens the sink's dock pane (the Canvas per-pane lease) before measuring; without an observer the black is about pull-eval, not the defect. dx7: card <select>/ALG display/syx input → the dock's `shell-cell-dx7-preset-select` (aria-label 'preset: X', popup listbox picks — NEVER Escape, the dock hazard), `control-algorithm` picture chip, `shell-cell-dx7-syx-input`; handles counted on the TILE (node chrome survives promotion); the stamp/persistence tests pick WIRE LEAD/BRASS 1 through the popup and re-open the dock after the reload. Positive controls: aw edge suppressed → 'spatial structure' red (restored green); toybox #1905's in-file control (withheld>0, posted=0) ran green | DONE — 12/12 once, 36/36 REPEAT=3 (+3 park skips), lint PASS, e2e tsc 0, --list 3045/502 |
| 55 | drain 29 — workflow-mode + the clipplayer pair (3 files, 13 tests): workflow-mode's two legacy boots flip to bare `/rack`; the M-drawer occupant assert widens to accept the FACE — on the default shell the pinned occupant is the one that gets its faceplate (#1739, DockCardHost mounts module-shell, not the verbatim card); the 'shell flag never changed the dock question' note re-worded for the flipped boot. clipplayer pair: the card-era flow (pad dblclick on the CARD → a 959px editor in xyflow space) died — the face editor band is ALWAYS painted in the dock pane, so the ENTIRE viewport-fitting apparatus (fitEditorInViewport / frameEditorUntilItFits / the on-screen clickLaunch prover, root-caused 2026-08-08) was card-era machinery and is DELETED; pane-scoped scrollIntoViewIfNeeded is the whole story. edit-launch selects lane0/slot0 by dblclicking face pad-0 (clip-key '0' presence replaces the card's `.sel` L1·S1 label); pitch-probability's first cell click both selects and commits (the face-clipplayer recipe — no pad step needed); the NotePanel's `clipplayer-edit-now`/`-edit-queue` and the pitch submenu testids are SHARED components, unchanged. Positive control: NOW launch click suppressed → lane-0-playing poll red; restored → green (pitch-prob's in-file solid-border negative control also ran ×3) | DONE — 13/13 once, 39/39 REPEAT=3, lint PASS, e2e tsc 0, --list 3045/502 |
| 56 | drain 30 — the fullscreen pair (2 files, 8 tests: video-fullscreen, video-fullscreen-multimonitor): all gestures move into the dock panes — right-click the FACE picture (`videoout-face-canvas` / `bentbox-face-canvas`), same shared VideoCanvasContextMenu, state machine on `videoout-fs-wrap` / the NEW `bentbox-face-fs-wrap`. ⚠ PRODUCT ADD (parity, controlled): BentboxOutputBody's preview-wrap — the fullscreen TARGET (fs.setTarget(wrapEl)) — carried NO `class:fullscreen` and no testid while its videoOut/backdraft siblings do; added both (control: wrap locator pointed at the class-less body root → 'entered fullscreen state' red; restored → green). ⚠ ASPECT-SWAP NUANCE: the face's resting buffer is IDLE_BUFFER 320×240 — already 4:3 — so the card-era 'card aspect ≠ engine aspect' precondition is card anatomy; the 4:3 leg now asserts the SIZE swap (idle → engine buffer, fullscreenCanvasDims) + engine aspect, and the 16:9 leg remains the aspect-change discriminator (1366×768 after __videoAspectStore.set). multimonitor: setup returns the pane-scoped face canvas; the requestFullscreen spy + fake getScreenDetails port unchanged | DONE — 8/8 once, 24/24 REPEAT=3, typecheck 0, lint PASS, e2e tsc 0, --list 3045/502 |
| 57 | drain 31 — the mixed-4 (4 files, ~16 active tests: video-full-frame, video-hide-controls, matrixmix, unpatch-patch-point; matrixmix's #1847 fixme untouched): video-full-frame's shared card exercise() split into per-module legs — videoOut body carries `.full-frame` on its root; bentbox's body paints NO class and a FIXED 480×360 canvas, so its observables are the shared persisted key + the menu's own state word ('Full Frame'→'Exit Full Frame', isFullFrame reaches the item label); the redundant VIDEOBOX card leg DELETED (the wave-3 face leg in the same file already drives the same gesture on the same key — post-promotion two tests of one surface is a duplicate, not coverage); handles-hidden asserts died (lane handles untouched by dock modes); the three inline 300ms blit settles converted to waitFrames(4). video-hide-controls: the legacy hide/resize suite (2) + the videoOut CARD-resize pair (2) RETIRED with tombstones — hideControls state + the corner drag SURVIVE on the faced monitor legs in the same file (`<type>-face-resize-handle`), and card-resize.ts keeps its living coverage on bentbox's face; the dblclick corner-trigger cascade shortcut RETIRED (corner triggers are the card PatchPanel variant; the shell's lane rail — class `jacks-trigger`, measured — never matches the resolver and its single click IS the shorter path; cascade coverage lives in unpatch + drilldown specs). matrixmix: axes → the lane tile's ranked selector cells (pickAxis popup, 'ANALOGVCO' label has no space), grid → the dock body (lifted, same testids), confirm dialogs unchanged. unpatch-patch-point: the two '(4)' panel legs flip to the default shell (back-jacks + drill-down are SHARED components on the tile) — titles keep the 'legacy patch panel' wording verbatim because the wait-ledger keys on titles (noted in-file). ⚠ DEFECT-CANDIDATE (pre-existing, NOT this drain's): 'rear card … undo restores it' is red at HEAD on the local preview (deterministic ×5): after the rear-jack unpatch, undo #1 removes the strum's AUTO-WIRE lane wires instead of restoring the poly edge — the unpatch delete appears never to land on the undo stack. The test pre-dates the drain on the default shell and its CI history shows green (26-32s measured runs in its own comments); needs a CI cross-check/bisect before treating as a product break. Positive control: GRAY-✕'s foreign fan-out edge suppressed → kind stayed legalEmpty, red; restored → green | DONE — 39/39 REPEAT=3 on the re-pointed set (+3 park skips; the known head-red leg failed 3/3 as recorded), lint PASS, e2e tsc 0, --list 3039/502 (−6: 1 dup + 5 card-only retirements, manifest above) |
| 58 | drain 32 — WAVESCULPT (1 file, 17 tests): `wavesculpt-canvas` is NODE-owned since S1 (NodeVizSurfaceHost, graph lifetime) so every pixel probe worked untouched; the card CONTROLS re-pointed into the TABBED dock (camera/wavetables/osc-*/voicing/look/output/walls): camera pad = the body's `control-pos_x` XyPad; the second card joystick DIED — zoom/rot are camera-tab FADERS (⚠ element-targeted `track.click({position})`, an absolute mouse.click landed on faceplate chrome and wrote nothing); unison = `control-unison` switch (voicing); the card's click-cycle VIEW/BLINK buttons + painted blink-name readout died — both are SEGMENTED radiogroups on the look tab whose checked segment IS the state (PROXIMITY/BIRDSEYE/SPECTROGRAPH, RIBBONS/SCOPES TRIAL/REALITY BASED COMMUNITY); osc strips = `shell-cell-wavesculpt-osc{n}-preset/-table/-load` (wavetables tab). ⚠ INSTRUMENT REPAIR (the drain EXPOSED it): the BLINK-modes pixel leg one-shot grabbed after a fixed 5 frames while the corner-trace scope-data path warms ASYNC of the render loop — green only while the file's early tests failed fast at their card asserts; once they passed on the shell (each booting the full renderer) the machine was busier at that point in the file and the grab landed before the data, deterministically (3/3). Now grabUntilLit: stepFrames batches until lit>2000, bounded at 40×5 frames — frames + a condition, never wall-clock. Controls: joystick gate drive suppressed → 'renders traces' red (proves the bounded wait can go red); restored → green; unison/view/blink carry live bidirectional state asserts ×3 | DONE — 17/17 once, 51/51 REPEAT=3, lint PASS, e2e tsc 0, --list 3039/502 |
| 59 | drain 33 — workflow-channel-columns (1 file, 12 active tests) + a product fix: the file's hand-typed geometry was the LEGACY lane's (COLUMN_W 765) — ⚠ MEASURED on the flipped boot: a drop at legacy col-2's x landed in SHELL column 4, col-3's in column 8 (the shell lays columns at SHELL_COLUMN_W=225), so every membership assert was wrong by construction; the spec now IMPORTS SHELL_COLUMN_W/COLUMN_BASELINE_Y from channel-columns.ts (one pitch scalar drives the whole layout, send rail included). The free-card DRAG leg's baseline−800 drop Y is ABOVE the compact shell band's top (measured: no-op at −800, ch5 at −300) → −300. The MIDI-OUT-BUDDY leg re-points card <select>/violet badge → the dock's `shell-cell-midi-out-buddy-connect`/-channel cells + the DeviceBody's warn LANE lamp. ⚠ PRODUCT FIX (proxy-identity, FIFTH sighting, controlled): the body's `sendChannel`/`laneChannel` derive from `savedData` — pumped by nodeVersion but the IDENTITY-EQUAL data proxy cuts re-derivation downstream of it, so the LANE lamp could never light (pre-fix build measured lit=0 over midiOutChannel=11; `void cardV` at the leaf deriveds → lit=1). The {legacy,shell} viewport pair collapsed to the shell arm (−1 test). Positive controls: the geometry itself (wrong pitch → 7 reds, right pitch → green) + the lamp's red-before build | DONE — 12/12 once (+1 ES-9 capability skip), 33/33 REPEAT=3, typecheck 0, lint PASS, e2e tsc 0, --list 3038/502 |
| 60 | drain 34 — GAMEPAD (1 file, 17 tests) + a product fix: the whole mapping surface moved into the dock board (`gamepad-body-gp`, node-suffixed testids). Translations: `.status.on` → the PAD lamp data-lit + the empty-state prompt; calibrate-start → `gamepad-calibrate-{left,right}-gp`; the calib-mode panel → COMPLETE/CANCEL presence, the right-mode text → the RIGHT stick pad's `.armed` class; calibrated badges → CAL L/R lamps data-lit; the remap BANNER → the armed target's own `.armed` class; the save/preset/garbage status toast → the MAPPING lamp's latched aria-label detail; save/load/preset ids node-suffixed. ⚠ The wavesculpt card-dot leg died AS DESIGN: the face pad shows the PARAM while CV moves the PICTURE (WavesculptOutputBody's own 'do not fix the pad to follow CV' owner-listed note) — the full-range engine read is the surviving claim. ⚠ The remap arm→stimulus BASELINE RACE (gamepad-face's documented recipe) applies to every arm: 3 frames before the stimulus, else the press is captured INTO the baseline (measured: 3 of the re-pointed remap tests red without it). ⚠ PRODUCT FIX (the drain-11 Esc class, 4th consumer): the body's armed-remap Escape was a BUBBLE-phase window listener, so one Esc cancelled the remap AND let Canvas close the whole dock out from under the player (measured — and it made the cancel assert pass VACUOUSLY, the state dying with the unmounted body); now capture-phase + stopPropagation while armed only, and the spec asserts the pane SURVIVES the press. Positive control: the binding press suppressed → detector poll red; restored → green; gamepad-face.spec re-run green (3/3) after the body edit | DONE — 17/17 once, 51/51 REPEAT=3, typecheck 0, lint PASS, e2e tsc 0, --list 3038/502 |
| 61 | drain 35 — TWOTRACKS (1 file, 29 active tests; wave-2's biggest at 31 measured legs) + a product fix: full translation into the TABBED dock (a-transport/a-tape/a-tone, b mirror, mix). mode/overdub/monitor = SWITCHES (aria-checked; the card's labelled cycle buttons + 'tape/loop tape' text died); filterMode + lofi = SEGMENTED radiogroups (checked segment IS the state; the ERROR red-highlight class died — the param write is asserted instead); transports + SAVE TAPE = `shell-cell-twotracks-*` ACTION cells; the A/B percentage readout + the card's 17-knob census (face census 21 — the four START/END loop faders rank as cells) + the transport LED bank + the tape-info duration all died as deleted readouts; the card's 1× rate-reset button → the shell-wide KNOB DBLCLICK default reset (Knob.svelte). Two card-only SAVE-gating tests RETIRED (the face save is a trigger cell with no disabled state; bufLen round-trips in perfzip, export seam audited by the sweep's file-export probe). The reel scrubbers port UNCHANGED onto `twotracks-face-canvas-a/-b` — the body preserves the card's two-gesture seam split. ⚠ PRODUCT FIX (the body's own `nodeData()` rule, unapplied to params — the absent-key derived class): `endA = $derived(node?.params.end_a ?? default)` memoises the DEFAULT off the absent key with nothing to invalidate on, so the START clamp read end=1 forever after an external end_a write — MEASURED: store-write end_a=0.3, drag START to 0.85 → wrote 0.85 straight past END (stale after 300ms too — stale-FOREVER, the rack-mate case). All four loop params now read FRESH (`paramFresh`), the paint staying live off the meter pump's per-frame state. That clamp red→green IS the family control | DONE — 29/29 once, 90/90 REPEAT=3 (with perfzip), typecheck 0, lint PASS, e2e tsc 0, --list 3036/502 (−2 retirements, manifested) |
| … | card-DOM rewrites remaining (rackLegacy 3→1 correction: joystick + live-glyphs resolved in drain 24; save-group-and-naming is the LAST rackLegacy consumer): ⚠ save-group-and-naming: DO NOT rewrite — owner ruling 1 DELETES groups entirely; its group half dies in the group sub-slice (S4), reconcile there. Remaining S2 work = wave-2's reverted 61-file queue (row 43) + mixed-4 + sweeps + helper flips + family (c) + parks + DOOM | pending |
