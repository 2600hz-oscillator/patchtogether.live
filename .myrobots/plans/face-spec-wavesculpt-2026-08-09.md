...
## 12. WHAT THE BUILD MEASURED THAT THIS SPEC GOT WRONG (2026-08-10)

Appended when the face was built, against the code as it stood on `19fe6cc8`.
Corrections only — every number below was re-measured, and each one is now
pinned by a test so it cannot drift back.

1. **⚠ "BLUE is zero across |rot| < 0.370 — 74 % of the ROT knob's travel"
   (§2-A) IS WRONG BY 2×. It is 36.7 %.** The dead WINDOW is 0.74 wide; `rot`
   spans −1..+1, so the FRACTION is 0.74 / 2.0. A 401-point sweep measures
   36.66 %. The window itself reproduces exactly (dark at 0.36, live at 0.38).
   Everything else in §2-A — the spawn gain table, `BLUE = 0.0000e+0`, the
   0.973-margin blind test, the VRT scene sitting on a zero — reproduced
   verbatim. Pinned in `wavesculpt-face-model.test.ts`.

2. **§8-E (`ShellColorCell`) WAS NOT A BLOCKER — the primitive already exists.**
   `face.paramCells` takes `'grid' | 'color'`, `ColorField` shipped, and
   `param-cell-coverage.test.ts` carried a `color` entry written naming this
   very module ("wavesculpt has no `face` yet"). The face declares
   `paramCells: { red_color: 'color', … }` and deletes that entry.

3. **§8-C ALREADY LANDED** (#1449, 11 CRT params deleted). 90 params → 79;
   91 cells → 81, not 80, because the hero panel adds a second `ControlFamily`.

4. **§8-D IS HALF LANDED.** `fxType{1..4}`, `video_mode` and `blink_mode` all
   carry `options` today, so they render as named Segmented rows with no work.
   `chord_quality` still does not, and it is NOT free: `params` is code and this
   def is in the WebGL attest basis, so adding the roster moves the GPU hash
   (measured `a93a3aba…` → `ea9cc2b5…`) and demands a trusted-machine
   re-attest. Deferred; the `chord` cluster LABEL carries the meaning instead.

5. **§7-A IS PAID** — `gate1..4` declare `edge: 'gate'` (#1442).

6. **§5-D's LONG BAND LABELS ARE UNUSABLE ON THIS FACE, and the spec's own
   §5-D warning pointed at the wrong risk.** It worried about CLIPPING; the real
   problem is that with 8 bands the face is TABBED, and on a tabbed face
   `bandHeaderPlan` suppresses the in-band label — so the label is only ever the
   TAB CHIP. Captured with sentence-length labels, three tabs filled the whole
   1220 px pane. Labels are now one or two words and the facts moved to the
   surfaces that paint at rest inside a band: CLUSTER labels (`scope modes only
   — SCALE is dead at RIBBONS`, `inert until alpha_in is patched`, `chord ·
   quality ON = minor`), the sidebar blocks, and the hero panel's caption.

7. **A DEFECT THE SPEC COULD NOT HAVE PREDICTED, found by LOOKING at the first
   baseline.** The hero panel's pointer→plan mapping read
   `getBoundingClientRect()` directly. The viewBox is square and
   `preserveAspectRatio` defaults to `xMidYMid meet`, so in the wide hero bay
   the drawing occupied a ~196 px square in the MIDDLE of a ~780 px element —
   the camera did not follow the pointer. No gate could see it (a panel probe's
   effects are `data`/`text`; a drag writes params). `wavesculpt-face.spec.ts`
   now guards it with the browser's own `getScreenCTM()` as the oracle, and the
   guard is negative-controlled: with the old mapping restored it reads −0.185
   where it wants −0.400.

8. **§2-B's master_gain measurements RE-DERIVE EXACTLY off the post-#1449
   shader** (the wavefold term is gone, so the blend weight is now purely
   `uMasterGain * 0.1`): mid grey 0.500 / 0.487 / 0.497 / 0.522 / 0.556 at gain
   0 / 0.5 / 1 / 1.5 / 2. The doc is corrected; the shader is not (still a
   two-module change).

9. **§9-A's CI ESTIMATE holds and the row is NOT renderer-bound.** Measured
   locally: 20.1 s on a real GPU and **19.5 s under `E2E_SWIFTSHADER=1`** — a
   0.6 s difference, because the cost is protocol round-trips, not pixels. At
   the spec's own `10s + 0.8s/cell` CI model, 81 cells ≈ **75 s**.
