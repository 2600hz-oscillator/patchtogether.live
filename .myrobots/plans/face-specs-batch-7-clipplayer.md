...
| **panels are dock-only** | a panel may only be SELECTED at tier `dock` (`module-face-lint.test.ts:678`), and the FULL lane plate is 6 cells. With 7 params ahead of them the ten panels rank 8–17, so they are dock-only by arithmetic as well as by rule — which is fine, and means the LANE tile can only ever be seven knobs. **That is the argument for leaving `clipplayer` on `NON_SHELL_LANE_TYPES` permanently even after a dock faceplate exists**: a seven-knob lane tile is not a clip launcher, and the list already says so. |
