...
|---|---|---|
| **contract-lock** | **+3 lines total** — `qbrt family qbrt-ping`, `snaredrum family snaredrum-hero`, `karplus family karplus-string` (the hero panel; **0 if the hero-cell revert is taken**); plus **1 MODIFIED** line (qbrt's `ping … edge=trigger`, which is a modification, not an addition — round 2 double-counted it). Fifteen of eighteen cost **zero**: `face` is UI metadata and `contract-signature.ts` has no `face` branch. A `custom` SIDEBAR panel (filter, cloudseed) costs **zero** contract lines — one registry line and one component. | unit |
...
| **faces-parity (REQUIRED)** | **+3 cells total** (qbrt's ping action, karplus's panel, snaredrum's panel). A hero PROMOTES a key; it never adds one, so a hero costs **zero** cells. At the measured ~0.8 s/cell on the SwiftShader runner: **≈ +2.4 s**. | **yes** |
