...
- **GIBRIBBON** analyses audio in-module: a pure fold over its own analyser
  ...
  coupling is what killed the game twice.
...
the old clamp so the change is bit-identical there (proved: all three ART
baselines unchanged). The module states its absent features in its own header
rather than implying parity.

**A renamed module keeps a migration path, and the old string is never reused.**
`RETIRED_TYPE_ALIASES` maps the retired type to the current one, and the
per-alias diagnostic names the *specific* failure a migrated node can still
produce — here, that the replacement analyses audio, so a migrated node with
nothing patched into its input is silent.
...
- Type-alias retirement has no clock running on it. The grace period before an
  alias may be dropped was never started, and dropping one early strands saved
  patches.
- **Game-asset sourcing is out of scope here.** What a module may load, ship or
  redistribute is ADR-007's subject, and DOOM specifically is excluded by name
  from this decision and from any sweep it motivates — nothing there moves
