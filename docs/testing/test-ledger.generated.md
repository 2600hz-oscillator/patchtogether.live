...
## Runtime skips — in-body env gates (81)
...
- `e2e/tests/card-producer-lifetime.spec.ts:716` — (dynamic: ``${type} shows no picture on any video output even with its card mounted, so there is ` + `nothing here to lose: ${di…`)
- `e2e/tests/card-producer-lifetime.spec.ts:743` — (dynamic: ``${type} renders a STATIC picture even with its card mounted on ${liveMounted[0]} ` + `(${fmtChange(mountedMotion)}),…`)
- `e2e/tests/card-producer-lifetime.spec.ts:922` — (dynamic: ``${type} shows no picture on any video output even UNGROUPED, so there is nothing here ` + `to lose: ${digest(beforeS…`)
