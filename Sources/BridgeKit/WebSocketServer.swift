...
//
// COPIED 2026-08-19 from ../patchtogether.es9/Sources/ES9Core/WebSocketServer.swift
// (that repo keeps shipping unchanged until its es9 build migrates here).
// If a bug is found in either copy, fix BOTH.
//
// Deltas from the es9 copy (kept intentionally minimal):
//   - queue labels es9.* -> bridge.*
//   - port 0 binds an ephemeral port; boundPort reports the real one
//     (integration tests bind 0 so parallel runs never collide)
//
...
public final class WebSocketServer: @unchecked Sendable {
    ...
    /// The actual bound port once start() returns — equals `port` unless
    /// `port` was 0 (ephemeral bind, used by tests).
    public private(set) var boundPort: UInt16 = 0
    private let pages: [String: StaticPage]
    private var nextSessionID = 1
    private let stateQueue = DispatchQueue(label: "bridge.ws.server")
    ...
    private func accept(_ conn: NWConnection) {
        ...
        sessions[id] = session
        let queue = DispatchQueue(label: "bridge.ws.conn.\(id)")
