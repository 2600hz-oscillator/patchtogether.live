...
test.describe('GRAINS OF VISION — granular video synthesizer', () => {
  ...
  test('real source chain: A patched → structured non-black OUT + GRAINS tap emits', async ({ page, errorWatch }) => {
    ...
    await expect(page.locator('.svelte-flow__node:has([data-shell-type="grainsOfVision"])'), 'card visible').toBeVisible();
    await expect(page.locator('.svelte-flow__node:has([data-shell-type="grainsOfVision"])')).toHaveCount(1);
    // (the card preview canvas died with the card; the pixel claims below are engine reads)
