...
test.fixme('card transport starts + stops with NO controller connected', { annotation: { type: 'fixme', description: 'FLAKE-PARK #1847 — nondeterministic on CI: 9 recovered-on-retry observations in the 96 h census to 2026-08-18; parked until root-caused' } }, async ({ page, rackLegacy }) => {
...
// Re-enable only on a root cause (#1847); "it passes now" is not one.
test.fixme('the mere presence of a push2Control module does NOT disable card transport', { annotation: { type: 'fixme', description: 'FLAKE-PARK #1847 — nondeterministic on CI: 9 recovered-on-retry observations in the 96 h census to 2026-08-18; parked until root-caused' } }, async ({ page, rackLegacy }) => {
  const transport = await seedClipRack(page, [
...
test('binding then unbinding a (simulated) Push leaves card transport working', async ({ page, rackLegacy }) => {
  const transport = await seedClipRack(page, [
    { id: 'px', type: 'push2Control', position: { x: 520, y: 80 }, domain: 'meta' },
  ]);
