...
const SUBJECTS: readonly Subject[] = [
  ...
  //
  // ⚠ IT SITS AFTER `nibbles` BY ARRIVAL, not by whichever side git happened to
  // take. The two rows were a MERGE CONFLICT and nibbles (#2317) reached main
  // first — the rule nibbles' own note states and peertube's states before it.
  //
  ...
  // because arrival ordering moves rows and this note has already outlived one
  // such move. `face-archivist.spec.ts` measures the MEDIA CLOCK across the toggle
