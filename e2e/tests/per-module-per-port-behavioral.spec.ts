...
const BEHAVIORAL_MODULE_EXEMPT: Record<string, string> = {
  ...
  // FRAMETABLE — video wavetable oscillator (60-frame GPU ring → per-pixel
  // whole-frame SELECT). Same SwiftShader per-frame-WebGL→video-out-canvas timeout
  // class as freezeframe/chroma/fader/mappy: verifying a per-frame WebGL video
  // effect on the software renderer is too slow to land a clean per-port delta on
  // CI. Whole-module exempt. Real coverage: the pure inverse-CDF distribution +
  // freeze/save reducers in frametable-core.test.ts, the per-module-per-port emit
  // sweep (ACIDWARP→video_in), and the bespoke frametable.spec.ts (real source
  // chain → non-blank variance-probed output + FREEZE-holds-while-input-changes).
  frametable: VIDEO_SINK_SWIFTSHADER_NOTE,

  // VIDEOCUBE — the video isomorph of the audio CUBE: 3 60-frame GPU rings →
  // an occupancy-weighted trilinear combine. Same SwiftShader per-frame-WebGL→
  // video-out-canvas timeout class as frametable/freezeframe/chroma (3 rings +
  // a 24-fetch combine is HEAVIER on the software renderer, so even more so).
  // Whole-module exempt. Real coverage: the occupancy-combine + luma-reduction
  // CPU mirror in videocube-core.test.ts, the per-module-per-port emit sweep
  // (ACIDWARP→video_a), and the bespoke videocube.spec.ts (real 3-source chain →
  // non-blank variance-probed video_out + audible audio_out RMS).
  videocube: VIDEO_SINK_SWIFTSHADER_NOTE,
  ...
  // mixmstrs — 8-channel stereo mixer. 111 drivable inputs (20 audio + 91 CV)
  ...
  mixmstrs: 'heavy-fan-out + per-channel-on-summed-mix class: mixmstrs declares 111 drivable inputs (20 audio + 91 paramTarget CV), so the sweep\'s 2-spawn-per-input loop = 222 spawns ≈ 37 min for ONE test — it BLOWS the per-test wall-clock budget (the read times out mid-run; same heavy-budget class as foxy/mandelbulb). On TOP of that, each ch{N}_{eq,comp,send,…}_cv scales ONE channel\'s contribution to the SUMMED masterL the sweep observes, so most single-channel CVs shift the summed RMS below the universal floor (the per-channel-on-mix class, cf. a per-band level scaler on a summed bus). Re-enterable only with BOTH a per-channel sink driver (observe the channel under test, not the sum) AND a way to test a SUBSET of representative ports under budget. Covered by mixmstrs.test.ts (per-channel gain/pan/comp-macro unit math) + the mixmstrs VRT baseline',
...
// near-threshold entries above (cube*, dx7.poly, and
// the existing swolevco/rings families) all straddle a SINGLE universal delta threshold
