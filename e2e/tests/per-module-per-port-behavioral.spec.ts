  ...
  for (const mod of REGISTRY) {
    ...
    test(title, async ({ page }) => {
      ...
      if (!observed) {
        ...
        await page.goto('/rack?shell=legacy'); // give the test SOMETHING to navigate to before asserting
        expect(
          observed,
      ...
      //  goto('/rack?shell=legacy') → spawnPatch(control) → settle → read
      //  goto('/rack?shell=legacy') → spawnPatch(patched) → settle → read → compare
