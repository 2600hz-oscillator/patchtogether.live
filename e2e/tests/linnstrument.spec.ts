...
//            expression LANE — the STATE half, read through
//            `read(node, 'card-api').expression(region)`. The AUDIBLE half
//            (unpatch the pressure path → level modulation disappears while
//            the note audio remains) is owned by
//            linnstrument-expression.spec.ts, at the per-lane expression
//            jacks (F03, owner ruling 2026-09-15 "a build").
...
test('@linnstrument keys → CUBE poly → audible: silence-first, an UNBOUND device is silent, two keys are two pitches, X bends the lane, Z/Y reach the lane, release → silence', async ({
  ...
  //     linnstrument-expression.spec.ts, at the jacks). Z is poly pressure on
  ...
  //     Sample twice, assert on the second: the bent band clears its floor the
  //     instant it appears, while the analyser ring and the voice's last un-bent
  //     cycles can still sit inside the FIRST 400 ms window (measured once under
  //     a loaded runner: the un-bent band read above 0.15 in that window and
  //     under it in the next). The first window absorbs the transition.
  await bandMax(page, 'scp', fA, BAND_ABSENT_WINDOW_MS);
