...
export const EXEMPT_BASELINE_PAIRS = new Set<string>([
  ...
  // above, at the point BEFORE the drain): the 2 CURATED FACE scenes for
  // KARPLUS (compact lane tile + dock full-view faceplate,
  // workflow-shell-faces.spec.ts). darwin baselines captured locally and
  // flake-checked 3×; the linux pair is pending a vrt-update.yml
  // `platform=linux` dispatch on the integrated batch-3 branch, at which
  // point it DRAINS exactly like batch 1's did and the vrt-meta
  // linux-deficit ceiling comes back down by 2.
