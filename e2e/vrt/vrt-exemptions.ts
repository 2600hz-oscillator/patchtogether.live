...
export const STRICT_VRT_MODULES = new Set<string>([
  ...
  // timelorde: TEMPORARILY demoted from the strict lane. The card gained a
  // dot-matrix neon WIZARD graphic (beat-pulse FROZEN to its idle frame under
  // prefers-reduced-motion, so the capture IS deterministic) + a toggle + a
  // gate input row. The darwin baseline was regenerated, but the linux
...
export const EXEMPT_BASELINE_PAIRS = new Set<string>([
  ...
  // TIMELORDE: the card gained a dot-matrix neon WIZARD graphic (data-driven
  // pixel-art bitmap + a beat-pulse) + a wizard show/hide toggle button + a
  // `gate` input row. The pulse is FROZEN to its idle frame under
  // prefers-reduced-motion (which the VRT runner sets) so the capture stays
  // deterministic and TIMELORDE remains in STRICT_VRT_MODULES — but the
  // darwin baseline was regenerated for the new chrome and the linux baseline
  // is pending a `vrt-update.yml` workflow_dispatch on this branch (same
  // pattern as SCALER / OUTLINES above). Functional coverage: the pure
  // timelorde-wizard.test.ts (bitmap→dots, beat-pulse math, gate→on/off) +
  // timelorde.test.ts (gate→wizardOn factory write-through) + the per-module-
  // per-port sweep (the new gate handle + input wire-up).
  'linux/timelorde',
]);
