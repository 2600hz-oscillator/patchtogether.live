...
export const ALLOWED_PERMANENT_EXEMPT: ReadonlySet<string> = new Set([
  ...
  // ⚠ `es9` STAYS 2026-08-25, THOUGH IT WAS PROMOTED IN THAT PR — and the
  // reason is a measurement, not an omission. Its face IS baselined
  // (face-es9-compact / face-es9-dock); its LEGACY card is not, because the
  // card's status row cycles on the bridge worker's reconnect backoff. See the
  // entry in EXEMPT_FROM_VRT, which records the two instruments that disagree
  // about that and which of them was right.
