...
export const VRT_LIVE_SURFACES: Record<string, LiveSurfaceScene> = {
  ...
  // ─────────── workflow-shell-faces.spec.ts — the PF-20 curated faces ───────
  //
  // THE FIRST FACE SCENE IN THIS REGISTRY, and the reason it exists at all.
  // `workflow-shell-faces.spec.ts` opens by asserting its glyphs "render
  // DETERMINISTIC pixels here: no audio flows in these scenes". That is a
  // property of the ROSTER, not of the scene — and analogVco is the roster
  // entry that falsifies it. It is a FREE-RUNNING oscillator: it sounds the
  // instant it spawns, with no gate, no trigger and no note to wait for, so its
  // live-audio glyph draws a real moving saw where all 21 siblings draw the
  // flat centreline an unattached-or-silent tap produces.
  //
  // This is why the face was authored, VERIFIED (every unit gate green) and
  // then DROPPED from batch 3 rather than shipped — the blocker was never the
  // faceplate, it was this one scene.
  'face-analogVco-compact': {
    spec: 'workflow-shell-faces.spec.ts',
    surfaces: [
      {
        // ScopeScreen's host div. Its ONLY out-of-box paint is a
        // `box-shadow: 0 0 0 1px` — spread, no blur — so a 1 px static ring
        // sits outside the mask and stays STRICT, which is correct: the ring is
        // chrome and is perfectly deterministic. Everything non-deterministic
        // is the <canvas>, which is 100%x100% of this element under
        // `overflow: hidden`. This is the timelorde glow trap checked and found
        // NOT to apply, rather than assumed away.
        selector: '[data-testid="shell-glyph"]',
        expectCount: 1,
        why:
          'analogVco is the ONE free-running oscillator in the faces roster, so unlike every ' +
          'other face glyph this one has real audio to draw. The trace is an AnalyserNode tap on ' +
          'the module\'s PRIMARY audio output — `primaryAudioOutPortId` picks the first declared ' +
          'audio port, which is `saw` — read on the shared visibility-gated meter frame (rAF-' +
          'coalesced, shell-glyph-live.ts `createShellGlyphTap`). The 2048-sample window is ~43 ms ' +
          'at 48 kHz and the saw runs at 261.626 Hz (C4, the shipped default), i.e. ~11.2 cycles ' +
          'per window — NOT an integer, so the phase at the window start differs on every capture ' +
          'and the drawn saw lands somewhere new each time. There is no gate to close and no ' +
          'knob to change: suspending the context only LATCHES a random phase rather than ' +
          'producing a repeatable one. ' +
          'GATE RESULT (2026-08-08, darwin, real config, real tolerance, UNMASKED, fresh unmasked ' +
          'baseline, 10 SEPARATE PLAYWRIGHT PROCESSES via scripts/vrt-derive-trials.sh — NOT ' +
          '--repeat-each, per instrument bug C): 1/10 PASS, a 90 % FAILURE RATE, with diffs of ' +
          '227 / 234 / 244 / 272 / 284 / 306-322 px (ratio 0.04-0.05) against an effective budget ' +
          'of 72 px. That supersedes the weaker 254 / 154 / 315 px reading over three consecutive ' +
          'captures that the batch-3 drop note quotes: same conclusion, correct instrument. ' +
          'THE CONTROL, same machine, same session, same 10-process harness, WITH this mask: ' +
          '10/10 PASS — so the instability is the card and not the environment, and the mask is ' +
          'sufficient rather than merely directionally helpful. ' +
          'The DOCK scene of the same face needs no entry and has none — `face.hero.cell` ' +
          'suppresses the shell glyph at the dock, so that capture never samples the oscillator. ' +
          'A scene passing while its sibling fails on one shared cause is the control here.',
        companion: {
          minInkFraction: 0.22,
          minLumaStdDev: 18,
          ...
          minMeanChroma: 22,
          rationale:
            'MASKS 8.7 % of the tile (627 magenta px of 88x82) — by a wide margin the CHEAPEST ' +
            'entry in this registry (mandelbulb 22.6 %, timelorde 41.3 %, the wavesculpt pair ' +
            '84.8 %), because a compact lane tile is mostly frame and knobs and the glyph is a ' +
            '33x19 sliver of it. The RACKLINE frame, the module name, both curated knob columns ' +
            'and their labels all stay in the strict diff, and so does the 1 px ring the ' +
            'box-shadow paints outside the masked element. ' +
            'MEASURED on the 33x19 px glyph through the REAL workflow-shell-faces capture path, ' +
            '4 gate runs: ink 0.6667 / 0.6667 / 0.6635 / 0.6858, stdDev 55.77 / 55.77 / 56.62 / ' +
            '54.98, buckets 12 x4, chroma 68.09 / 68.09 / 68.90 / 69.46 — a spread of 3.4 % on ' +
            'ink and under 3 % on the rest even though the TRACE ITSELF is what moves, which is ' +
            'exactly the property that makes a statistics companion the right replacement for the ' +
            'deleted pixels: the saw sweeps the full 0-1 box on every capture, so how MUCH ink is ' +
            'stable while WHERE it lands is not. Floors are set against the MINIMUM observed, ' +
            'not the mean. ' +
            'Force-killed (opacity:0, so this row is the CARD FACE behind the glyph): ink 0.0000, ' +
            'stdDev 0.83, buckets 1, chroma 9.48. Floors sit at ~a third of live: ink 0.22 ' +
            'against a dead value of EXACTLY zero; stdDev 18 is 21.7x dead; buckets 5 sits ' +
            'between dead 1 and live 12. The chroma floor of 22 is 2.3x the dead 9.48 and it is ' +
            'the one statistic that pins the trace being stroked in the AUDIO DOMAIN HUE rather ' +
            'than merely "something is drawn" — a glyph that lost its `color={spine}` binding and ' +
            'fell back to a grey wash would clear ink, stdDev and buckets and fail only here.',
        },
      },
    ],
  },
